...
- (BOOL)initDaemon {
    NSLog(@"[ZZZZ] 1️⃣ [DEBUG] initDaemon called");

    if (initialized()) {
        NSLog(@"[ZZZZ] 2️⃣ ✅ Daemon already initialized");
        return YES;
    }

    NSLog(@"[ZZZZ] 3️⃣ ❌ Daemon not initialized yet");

    if (![[NSThread currentThread] isMainThread]) {
        NSLog(@"[ZZZZ] 4️⃣ 🧵 initDaemon on background thread → dispatching to main thread");
        ...
        NSLog(@"[ZZZZ] 5️⃣ 🧵 initDaemon on main thread");
        return [self initDaemonInternal];
    }
}

- (BOOL)initDaemonInternal {
    NSLog(@"[ZZZZ] 6️⃣ [DEBUG] initDaemonInternal called");

#if DEBUG
    int flag = LIBJAMI_FLAG_DEBUG | LIBJAMI_FLAG_CONSOLE_LOG | LIBJAMI_FLAG_SYSLOG | LIBJAMI_FLAG_NO_LOCAL_AUDIO;
    NSLog(@"[ZZZZ] 7️⃣ ⚙️ Using debug flags: %d", flag);
#else
    int flag = 0;
    NSLog(@"[ZZZZ] 7️⃣ ⚙️ Using release flags");
#endif

    BOOL result = init(static_cast<InitFlag>(flag));
    NSLog(@"[ZZZZ] 8️⃣ %@ Daemon initInternal result", result ? @"✅" : @"❌");
    return result;
}

- (BOOL)startDaemon {
    NSLog(@"[ZZZZ] 9️⃣ [DEBUG] startDaemon called");

    [self registerSignals];
    NSLog(@"[ZZZZ] 🔟 📶 Signals registered");

    if (![[NSThread currentThread] isMainThread]) {
        NSLog(@"[ZZZZ] 1️⃣1️⃣ 🧵 startDaemon on background thread → dispatching to main thread");
        __block BOOL success;
        dispatch_sync(dispatch_get_main_queue(), ^{
        ...
        NSLog(@"[ZZZZ] 1️⃣2️⃣ 🧵 startDaemon on main thread");
        return [self startDaemonInternal];
    }
}

- (BOOL)startDaemonInternal {
    NSLog(@"[ZZZZ] 1️⃣3️⃣ [DEBUG] startDaemonInternal called");
    BOOL result = start();
    NSLog(@"[ZZZZ] 1️⃣4️⃣ %@ Daemon startInternal result", result ? @"✅" : @"❌");
    return result;
}

- (void)fini {
    NSLog(@"[ZZZZ] 1️⃣5️⃣ [DEBUG] fini called");

    if (![[NSThread currentThread] isMainThread]) {
        NSLog(@"[ZZZZ] 1️⃣6️⃣ 🧵 fini on background thread → dispatching to main thread");
        dispatch_sync(dispatch_get_main_queue(), ^{
            fini();
            NSLog(@"[ZZZZ] 1️⃣7️⃣ 🧹 Daemon finalized on main thread");
        });
    } else {
        fini();
        NSLog(@"[ZZZZ] 1️⃣7️⃣ 🧹 Daemon finalized on main thread");
