...
public final class AdapterService: AdapterDelegate {
    ...
    func sendSwarmFileAndClean(accountId: String, conversationId: String, filePath: URL, fileName: String, parentId: String) {
        ...
        return Single.zip(lookups)
            .map { nameAvatarFlagTriplets in
                ...
                if nameAvatarFlagTriplets.count == 1 {
                    accountType = nameAvatarFlagTriplets.first!.2 ? "jid" : "single"
                } else {
                    accountType = "group"
                }
