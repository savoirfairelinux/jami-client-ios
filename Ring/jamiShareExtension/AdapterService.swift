...
public final class AdapterService: AdapterDelegate {
    ...
    func getConversationInfo(accountId: String, conversationId: String) -> Single<String> {
        ...
        guard let members = adapter.getConversationMembers(accountId, conversationId: conversationId) else {
            let fallback = conversationId
            return .just(fallback)
        }

        // Step 1: Get jamiId from accountId
        guard let accountDetails = adapter.getAccountDetails(accountId),
              let username = accountDetails["Account.username"] as? String else {
            return .just(conversationId)
        }
        let jamiId = username.replacingOccurrences(of: "ring:", with: "")

        // Step 2: Filter out current user's jamiId
        let filteredMembers = members.filter { member in
            guard let uri = member["uri"] else { return false }
            return uri != jamiId
        }

        let lookups: [Single<String>] = filteredMembers.compactMap { member in
            guard let address = member["uri"] else { return nil }
            return lookupUsername(address: address)
                .map { response in
                    let resolvedName = (response.state == .found && !(response.name?.isEmpty ?? true))
                        ? response.name!
                        : self.contactProfileName(accountId: accountId, contactId: address, type: "conversation")
                    return resolvedName ?? conversationId
                }
        }
