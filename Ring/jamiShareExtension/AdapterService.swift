...
public final class AdapterService: AdapterDelegate {
    ...
    func getConversationInfo(accountId: String, conversationId: String) -> Single<(name: String, avatar: String?)> {
        ...
        let lookups: [Single<(String, String?)>] = filteredMembers.compactMap { member in
            ...
            return lookupUsername(address: address)
                .map { response in
                    ...
                    let resolvedName = !profileName.isEmpty
                        ? profileName
                        : ((response.state == .found && !(response.name?.isEmpty ?? true)) ? response.name! : conversationId)
        ...
        return Single.zip(lookups)
            .map { nameAvatarPairs in
                ...
                // If you want to pick the first non-nil avatar from the list (or nil if none)
                let firstAvatar = nameAvatarPairs.compactMap { $0.1 }.first
