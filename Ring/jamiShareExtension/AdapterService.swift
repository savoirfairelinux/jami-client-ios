...
public final class AdapterService: AdapterDelegate {
    ...
    func getGroupMemberProfiles(accountId: String, conversationId: String) -> Single<(members: [GroupAvatarMember], overflowCount: Int)> {
        ...
        let sortedMembers = members.sorted { m1, m2 in
            let a1 = memberAvatarPriority(member: m1, selfJamiId: selfJamiId, accountId: accountId)
            let a2 = memberAvatarPriority(member: m2, selfJamiId: selfJamiId, accountId: accountId)
            return a1 > a2
        }

        let overflowCount = max(sortedMembers.count - maxAvatars, 0)
        let cappedMembers = Array(sortedMembers.prefix(maxAvatars))

        let memberLookups: [Single<GroupAvatarMember>] = cappedMembers.compactMap { [weak self] member in
            guard let address = member["uri"] else { return nil }
            let isSelf = ProfilePathHelper.jamiHash(from: address) == selfJamiId
            let type = isSelf ? "account" : "conversation"

            let profilePhoto = self?.contactProfileAvatar(accountId: accountId, contactId: isSelf ? accountId : address, type: type)
            let profileName = self?.contactProfileName(accountId: accountId, contactId: isSelf ? accountId : address, type: type)
    ...
    private func memberAvatarPriority(member: [String: String], selfJamiId: String, accountId: String) -> Int {
        guard let address = member["uri"] else { return 0 }
        let isSelf = ProfilePathHelper.jamiHash(from: address) == selfJamiId
        let type = isSelf ? "account" : "conversation"
        let contactId = isSelf ? accountId : address

        if contactProfileAvatar(accountId: accountId, contactId: contactId, type: type) != nil { return 2 }
        if let name = contactProfileName(accountId: accountId, contactId: contactId, type: type), !name.isEmpty { return 1 }
        return 0
    }
