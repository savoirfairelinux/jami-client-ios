...
public final class AdapterService: AdapterDelegate {
    ...
    private func nonEmptyName(_ name: String?) -> String? {
    ...
    func getConversationName(accountId: String, conversationId: String) -> Single<(name: String, avatarType: AvatarType)> {
        ...
        return buildGroupName(members: members, selfJamiId: selfJamiId, accountId: accountId)
    }
    ...
    private func buildGroupName(members: [[String: String]], selfJamiId: String, accountId: String) -> Single<(name: String, avatarType: AvatarType)> {
        let selfSuffix = " (\(L10n.Conversation.yourself))"
        let activeMembers = members.filter { ParticipantRole(rawValue: $0["role"] ?? "")?.isActive ?? true }
        let activeCount = activeMembers.count

        let nameLookups: [Single<String>] = activeMembers.compactMap { [weak self] member in
        ...
        return Single.zip(nameLookups)
            .map { names in (buildGroupTitle(resolvedNames: names, totalActiveCount: activeCount), AvatarType.group) }
    }
    ...
    func getGroupMemberProfiles(accountId: String, conversationId: String) -> Single<(members: [GroupAvatarMember], overflowCount: Int)> {
        return Single.deferred { [weak self] in
            ...
            let conversationInfo = self.adapter?.getConversationInfo(forAccount: accountId, conversationId: conversationId) as? [String: String]
            let mode = conversationInfo?["mode"].flatMap { Int($0) } ?? 0
            guard mode > 0 else { return .just(([], 0)) }

            guard let members = self.adapter?.getConversationMembers(accountId, conversationId: conversationId),
                  !members.isEmpty else {
                return .just(([], 0))
            }

            guard let accountDetails = self.adapter?.getAccountDetails(accountId),
                  let username = accountDetails["Account.username"] as? String else {
                return .just(([], 0))
            }
            ...
            let profiles: [MemberProfile] = members.compactMap { member in
                ...
                let parsed = self.contactProfilePhotoAndName(accountId: accountId, contactId: contactId, type: type)
                return MemberProfile(address: address, role: ParticipantRole(rawValue: member["role"] ?? ""),
                                     isSelf: isSelf, photo: parsed.photo, name: parsed.name)
            }
