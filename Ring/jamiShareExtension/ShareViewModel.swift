...
class ShareViewModel: ObservableObject {
                ...
                for (key, indicator) in self.transmissionStatus {
                    ...
                    case .file:
                        if let displayName = interaction.body["displayName"] as? String,
                           ...
                           let fileId = interaction.body["id"] as? String {
                            matched = true
                            messageIdToAssign = fileId
                        }
                    }

                    if matched, let messageId = messageIdToAssign {
    ...
    private func checkAllItemsSent() {
        ...
        if allFinal {
            ...
            self.adapterService.setUpdatedConversations(accountId: indicators.first!.accountid, conversationId: indicators.first!.convid)
