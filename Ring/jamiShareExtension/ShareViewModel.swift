...
class ShareViewModel: ObservableObject {
    ...
    @Published var accountList: [(id: String, name: String, avatar: String)] = []
    @Published var conversationsByAccount: [String: [(id: String, name: String, avatar: String)]] = [:]
    ...
    private func fetchAccountsAndConversations() {
        ...
        for (accountId, conversationIds) in convsByAccount {
            for convoId in conversationIds {
                adapterService.getConversationInfo(accountId: accountId, conversationId: convoId)
                    .observe(on: MainScheduler.instance)
                    .subscribe(onSuccess: { conversationDetails in
                        print("Conversation for account \(accountId): \(convoId) and \(conversationDetails)")

                        // Append (id, name) to the correct account in the dictionary
                        var existingConvs = self.conversationsByAccount[accountId] ?? []
                        existingConvs.append((
                            id: convoId,
                            name: conversationDetails.name,
                            avatar: conversationDetails.avatar ?? "" // or nil if your type allows it
                        ))
                        self.conversationsByAccount[accountId] = existingConvs
                    })
                    .disposed(by: disposeBag)
            }

            adapterService.resolveLocalAccountDetails(accountId: accountId)
                .observe(on: MainScheduler.instance)
                .subscribe(onSuccess: { details in
                    if let accountId = details["accountId"], let accountName = details["accountName"],  let accountAvatar = details["accountAvatar"] {
                        self.accountList.append((id: accountId, name: accountName, avatar: accountAvatar))
                    }
                })
                .disposed(by: disposeBag)
        }
