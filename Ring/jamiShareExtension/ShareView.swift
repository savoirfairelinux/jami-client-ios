        ...
        VStack {
            if let selectedAccountId = selectedAccountId, !selectedAccountId.isEmpty,
               let selectedAccount = viewModel.accountList.first(where: { $0.id == selectedAccountId }) {
                ...
                .accessibilityLabel(L10n.ShareExtension.accessibilitySelectedAccount(selectedAccount.name))
                .accessibilityHint(L10n.ShareExtension.accessibilityOpenAccountSelection)
            } else {
                EmptyView()
            }
        }
...
// Colors from material.io - matching main project
let avatarColors = [
