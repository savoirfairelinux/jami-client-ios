...
struct ShareView: View {
    ...
    let sendFileAction: (String, String, String, String) -> Void
                ...
                for provider in attachments {
                    if provider.hasItemConformingToTypeIdentifier(UTType.plainText.identifier) {
                        ...
                        provider.loadItem(forTypeIdentifier: UTType.image.identifier, options: nil) { (data, _) in
                            if let url = data as? URL {
                                sendAction(convo, account, url.absoluteString, "image")
                            } else {
                                showAlert = true
                            }
                        }
            ...
            if provider.hasItemConformingToTypeIdentifier(UTType.plainText.identifier) {
                ...
                provider.loadItem(forTypeIdentifier: UTType.image.identifier, options: nil) { (data, _) in
                    if let url = data as? URL, let uiImage = UIImage(contentsOfFile: url.path) {
                        DispatchQueue.main.async {
                            contentPreviews.append(ContentPreview(.image(Image(uiImage: uiImage))))
                        }
