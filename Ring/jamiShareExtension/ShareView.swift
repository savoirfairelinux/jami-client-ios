...
struct ShareView: View {
    ...
    init(items: [NSExtensionItem], closeAction: @escaping () -> Void) {
        ...
        self.viewModel = ShareViewModel(sharedItems: items) // Initialize the ShareViewModel
    }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    AccountsSection(
                        ...
                        accountList: viewModel.accountList, // Pass list of AccountViewMdoels
                        conversationsByAccount: viewModel.conversationsByAccount, // Pass conversations map
...
struct AccountsSection: View {
    ...
    // These properties now receive the new ViewModel types
    let accountList: [AccountViewModel] // List of AccountViewModels
    let conversationsByAccount: [String: [ConversationViewModel]] // Dictionary of ConversationViewModels
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            // Get the currently selected AccountViewModel based on selectedAccountId
            if let selectedAccount = accountList.first(where: { $0.id == selectedAccountId }) {
                Menu {
                    Picker("Select Account", selection: $selectedAccountId) {
                        // Iterate directly over AccountViewModel instances
                        ForEach(accountList) { account in
                            Text(account.name).tag(account.id as String?) // Tag with the ID
                        }
                    }
                } label: {
                    HStack(spacing: 12) {
                        // Display account avatar. Will show placeholder until fetched.
                        if let avatarImage = imageFromBase64(selectedAccount.avatar), !selectedAccount.avatar.isEmpty {
                            ...
                            Circle()
                                ...
                                .frame(width: 40, height: 40)
                                .overlay(
                                    Text(String(selectedAccount.name.prefix(1)).uppercased())
                                        .font(.headline)
                                        .foregroundColor(.white)
                                )
                        }

                        // Display account name. Will show "Loading..." then update.
            ...
            if let selectedId = selectedAccountId {
                if let conversations = conversationsByAccount[selectedId], !conversations.isEmpty {
                    VStack(alignment: .leading, spacing: 12) {
                        Text("Conversations")
                            .font(.headline)
                            .padding(.leading, 8) // Alignment
                        
                        // Iterate directly over ConversationViewModel instances
                        ForEach(conversations) { convo in
                            HStack(spacing: 12) {
                                // Display conversation avatar. Will show placeholder then update.
                                if let avatarImage = imageFromBase64(convo.avatar), !convo.avatar.isEmpty {
                                    ...
                                    Circle()
                                        ...
                                        .frame(width: 40, height: 40)
                                        .overlay(
                                            Text(String(convo.name.prefix(1)).uppercased())
                                                .foregroundColor(.white)
                                                .font(.headline)
                                        )
                                }

                                // Display conversation name. Will show "Loading..." then update.
                                Text(convo.name)
                                    .font(.body)
                            ...
                            .onTapGesture {
                                // Call send action with conversation and account IDs
                                sendAction(convo.id, selectedId)
                            }
                        }
                    }
                } else {
                    // Message when no conversations are found for the selected account
                    Text("No conversations found for this account.")
                        .foregroundColor(.gray)
                        .frame(maxWidth: .infinity, alignment: .center)
                        .padding()
                }
        ...
        .onAppear {
            ...
            if selectedAccountId == nil, let firstAccount = accountList.first {
                selectedAccountId = firstAccount.id
            }
        }
        // React to changes in the accountList (e.g., if accounts load after initial empty state)
        .onChange(of: accountList) { newAccountList in
