...
/// Reads the daemon's `<accountPath>/contacts` file and decodes it via the
/// msgpack mirror above. Returns an empty map on any error (missing file,
/// decode failure). Shared between the NSE's production reader and the
/// DriftCheck helper — both must agree on the exact mirror, so they both
