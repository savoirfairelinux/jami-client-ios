...
import { attachmentScheme, exportDocument, linesOf } from '../src/export.js'
...
test('markup a peer typed does not become markup the reader runs', () => {
    // An angle bracket the reader is handed unescaped opens a tag or an
    // autolink; one behind a backslash is the character the peer typed.
    const open = /(^|[^\\])[<>]/

    // Markdown hands HTML straight to whatever renders it, so a tag written as
    // ordinary words is a tag that runs in the reader's application.
    const md = write([{ insert: '<img src=x onerror=alert(1)> and <b>bold</b>\n' }], 'md')
    assert.doesNotMatch(md, open, md)
    assert.match(md, /\\<img src=x onerror=alert\(1\)\\>/)

    // An address between angle brackets is an autolink, so this is the same
    // address the editor refuses to display, arriving by another door.
    assert.doesNotMatch(write([{ insert: '<javascript:alert(1)>\n' }], 'md'), open)

    // A blockquote still opens only where a blockquote can.
    assert.match(write([{ insert: '> quoted\n' }], 'md'), /^\\> quoted/)

    // html says the same thing its own way, and is what it always was.
    assert.match(write([{ insert: '<b>bold</b>\n' }], 'html'), /&lt;b&gt;bold&lt;\/b&gt;/)
})
...
test('a picture with no name goes out as nothing, not as a name', () => {
    // Nothing can be asked for on its behalf, so leaving it in would write an
    // address into the file that no reader, and no application, can follow.
    for (const image of [{}, { id: '' }, { id: 42 }, { id: null }]) {
        const out = exportDocument(new Delta([
            { insert: 'before ' },
            { insert: { image } },
            { insert: ' after\n' },
        ]), 'html')
        assert.deepEqual(out.attachments, [])
        assert.ok(!out.text.includes('jami-attachment-'), JSON.stringify(image))
        assert.ok(!out.text.includes('<img'), JSON.stringify(image))
        assert.match(out.text, /before\s+after/)
    }
})
...
test('the name a picture is left under cannot be written by hand', () => {
    ...
    // And it is drawn from the platform's randomness or from nothing at all: a
    // name from a source somebody could follow is a name they could write.
    const real = globalThis.crypto
    try {
        delete globalThis.crypto
        assert.throws(() => attachmentScheme(), /randomness/)
    } finally {
