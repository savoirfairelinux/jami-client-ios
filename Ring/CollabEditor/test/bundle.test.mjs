 ...
 * Swift side does.
 *
 * The editor lives behind a WKWebView, where a mistake shows up as a blank
 * page and a line in the log. This runs the same file against the same
 * interface so that a bundle that cannot start is a failed build rather than a
 * blank screen.
 */
...
const pageURL = 'jami-collab://editor/editor.html'

/** Stands in for the Swift side of the bridge. */
function makeHost() {
    let announce
    const host = {
        updates: [],
...
function launch() {
    ...
    const dom = new JSDOM(readFileSync(page, 'utf8'), {
        ...
        // The bundle derives attachment URLs from the custom-scheme page URL.
        // jsdom otherwise uses about:blank, which cannot resolve /attachment/.
        url: pageURL,
    })
...
test('a peer writing during a drag does not send the width elsewhere', options, async () => {
    const { dom, host, editor } = await launch()
    ...
    fire(dom, dom.window.document, 'pointermove', 160)

    // The other side writes above the image while the finger is still down,
    // which pushes the image further along the document.
    peer.getText('content').insert(0, 'hello')
    editor.applyUpdate(Buffer.from(Y.encodeStateAsUpdate(peer)).toString('base64'))

    fire(dom, dom.window.document, 'pointerup', 160)

    // The width belongs to the image, wherever it has ended up -- not to
...
test('a version can be restored, and everyone gets it', options, async () => {
    ...
    assert.equal(editor.restoreVersion(), true, 'the restore was not owned up to')
...
test('a restore that changes nothing says so', options, async () => {
    const { host, editor } = await launch()

    const peer = new Y.Doc()
    peer.getText('content').applyDelta([{ insert: 'unchanged' }])
    editor.applyUpdate(Buffer.from(Y.encodeStateAsUpdate(peer)).toString('base64'))

    // The version being read is the document as it stands. Restoring it is not
    // a failure, but nothing happens either, and the application has to be able
    // to tell the two apart before it tells the user the document was restored.
    editor.showVersion(Buffer.from(Y.encodeStateAsUpdate(peer)).toString('base64'))
    host.updates.length = 0

    assert.equal(editor.restoreVersion(), false)
    assert.equal(host.updates.length, 0, 'a restore of nothing was sent')
    assert.deepEqual(host.logs, [])
})

test('a restore with no version being read is refused', options, async () => {
    const { host, editor } = await launch()
...
/** What the editor looks like when it is editable and when it is not. */
function editability(dom, editor) {
    const read = () =>
        dom.window.document.querySelector('.ql-editor').getAttribute('contenteditable')
    editor.setEditable(false)
    const no = read()
    editor.setEditable(true)
    const yes = read()
    assert.notEqual(no, yes, 'editable and not are not told apart')
    return { read, yes, no }
}

test('leaving a version does not hand out the right to edit', options, async () => {
    const { dom, host, editor } = await launch()
    const { read, no } = editability(dom, editor)

    // A document this user may read and not write: a version of it is
    // something to look at, and looking at one must not be a way in.
    editor.setEditable(false)
    ...
    assert.equal(read(), no, 'reading a version made the document editable')
    assert.deepEqual(host.logs, [])
})

test('a version cannot be restored into a document that may not be edited',
     options, async () => {
    const { host, editor } = await launch()

    const peer = new Y.Doc()
    peer.getText('content').applyDelta([{ insert: 'as it stands' }])
    editor.applyUpdate(Buffer.from(Y.encodeStateAsUpdate(peer)).toString('base64'))
    editor.setEditable(false)
    ...
    // Quill refuses the edit either way; what matters is that the refusal is
    // owned up to, rather than reported as a restore that never happened.
    assert.equal(editor.restoreVersion(), false)
    assert.deepEqual(host.updates, [])
    assert.deepEqual(host.logs, [])
})

test('being told the document may be edited again brings it back', options, async () => {
    const { dom, host, editor } = await launch()
    const { read, yes, no } = editability(dom, editor)

    // Said while a version is being read, it takes effect on the way out and
    // not before: what is on screen is the past, and the past is read.
    editor.setEditable(false)
    const past = new Y.Doc()
    past.getText('content').insert(0, 'as it was')
    editor.showVersion(Buffer.from(Y.encodeStateAsUpdate(past)).toString('base64'))
    editor.setEditable(true)
    assert.equal(read(), no, 'the version being read became editable')

    editor.leaveVersion()
    assert.equal(read(), yes, 'the document did not become editable again')
    assert.deepEqual(host.logs, [])
})
...
test('a peer writing during a pinch does not send the width elsewhere',
    options, async () => {
    const { dom, host, editor } = await launch()
    ...
    // The fingers are still down when someone else writes above the picture.
    peer.getText('content').insert(0, 'hello')
    editor.applyUpdate(Buffer.from(Y.encodeStateAsUpdate(peer)).toString('base64'))

    pinch(dom, root, 'touchend', 120)

    // The width belongs to the picture, wherever the picture has been pushed.
    for (const update of host.updates) Y.applyUpdate(peer, decode(update))
    assert.deepEqual(peer.getText('content').toDelta(), [
        { insert: 'hello' },
        { insert: { image: { id: 'att-1', width: 800, height: 600 } },
