...
@objc
final class FilterKeys: NSObject {
...
struct IncomingCallFilter {
    ...
    init(allowFromUnknown: Bool, contactDetails: [[String: String]]) {
        ...
        self.contactURIs = Set(
            contactDetails.compactMap { $0[FilterKeys.contactId]?.normalizeJamiId() }
        )
    }

    func shouldAccept(peerId: String) -> Bool {
        return allowFromUnknown || contactURIs.contains(peerId.normalizeJamiId())
    }

    static func parseAllowFromUnknown(value: String?) -> Bool {
