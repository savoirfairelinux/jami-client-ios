...
- (NSDictionary<NSString*, NSString*>*)decrypt:(NSString*)keyPath treated:(NSString*)treatedMessagesPath value: (NSDictionary*)value {
    ...
    try {
        ...
        auto m = unpacked.get().as<dht::ImMessage>();
        if (!m.msg.empty()) {
            if (isMessageTreated(m.id, [treatedMessagesPath UTF8String])) {
                return result;
            }
            NSLog(@"******ImMessage");
            std::string datatype = utf8_make_valid(m.datatype);
            if (datatype.empty()) {
                datatype = "text/plain";
            }
            if (datatype == MIME_TYPE_GIT || datatype == MIME_TYPE_TEXT_PLAIN) {
                return [self getReturnValueOfType: message peerId: @""];
            }
            return result;
        }
        ...
        // auto message = unpacked.get().as<dht::ImMessage>();
