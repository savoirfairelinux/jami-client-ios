...
- (BOOL)allowsIncomingCallsFromUnknownFor:(NSString*)accountId {
    ...
    NSURL *docs = [Constants documentsPath];
    if (!docs) return YES;
    auto path = [[docs URLByAppendingPathComponent:accountId] URLByAppendingPathComponent:accountConfig].path.UTF8String;
                                         ...
                                         value:(NSDictionary*)value
{
    ...
    try {
        ...
        if (peerCR.connType.empty()) {
            ...
            // requests). Start the daemon so it can answer the pending ICE offer;
            // otherwise the sender's queued messages will never be delivered.
