...
class NotificationService: UNNotificationServiceExtension {
    ...
    private func shouldAcceptIncomingCall(peerId: String) -> Bool {
        let filter = IncomingCallFilter(
            allowUnknown: self.adapterService.allowsIncomingCallsFromUnknown(accountId: self.accountId),
            contactDetails: self.adapterService.getContacts(accountId: self.accountId)
        )
...
extension NotificationService {
    ...
    private func contactProfileName(accountId: String, contactId: String) -> String? {
        ...
        let overridePath = ProfilePathHelper.existingContactProfileOverridePath(accountId: accountId,
                                                                                contactId: contactId,
                                                                                documents: documents)
        return VCardUtils.parseMergedProfile(basePath: path, overridePath: overridePath)?.alias
    }
}
