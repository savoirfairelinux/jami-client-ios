...
class NotificationService: UNNotificationServiceExtension {
    ...
    var taskCompleted = false /// all values from dht parsed, conversation synchronized if needied and files downloaded
    ...
    override func didReceive(_ request: UNNotificationRequest, withContentHandler contentHandler: @escaping (UNNotificationContent) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: url) {[weak self, weak group] (data, _, _) in
                ...
                do {
                    ...
                    case .gitMessage:
                        if !self.daemonStarted {
                            ...
                            self.adapterService.startAccountsWithListener { [weak self, weak group] event, eventData in
                                guard let self = self else {
                                    return
                                }
                                switch event {
                                ...
                                case .fileTransferDone:
                                    ...
                                    if self.taskCompleted && self.numberOfFiles == 0 {
                                        group?.leave()
                                    }
                                case .syncCompleted:
                                    if !self.taskCompleted {
            ...
            if !self.daemonStarted && !self.taskCompleted {
                self.taskCompleted = true
                group?.leave()
            }
        }
        task.resume()
        _ = group.wait(timeout: .now() + notificationTimeout)
    }
