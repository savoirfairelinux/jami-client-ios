...
class NotificationService: UNNotificationServiceExtension {
    ...
    override func didReceive(_ request: UNNotificationRequest, withContentHandler contentHandler: @escaping (UNNotificationContent) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: url) {[weak self, weak group] (data, _, _) in
            ...
            if !self.daemonStarted, numberOfMessages > 0 {
                if numberOfMessages > 3 && numberOfMessages < 7 {
                    additionalWaitTime = 5
                } else if numberOfMessages >= 7 && numberOfMessages < 15 {
                    additionalWaitTime = 10
                } else if numberOfMessages >= 15 {
                    additionalWaitTime = 15
                }
                ...
                self.adapterService!.startDaemonWithListener { [weak self, weak group] event, eventData in
                    ...
                    if self.daemonStarted && !answerReceived && numberOfMessages < 1 {
                        answerReceived = true
                        group?.leave()
                    }
                }
        ...
        if additionalWaitTime > 0 {
            ...
            group1.enter()
            _ = group1.wait(timeout: .now() + DispatchTimeInterval.seconds(additionalWaitTime))
        }
    }
