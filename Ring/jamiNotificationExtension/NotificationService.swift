...
class NotificationService: UNNotificationServiceExtension {
    ...
    private static let notificationsCount = "notificationsCount"
    ...
    private func setNotificationCount(notification: UNMutableNotificationContent) {
        ...
        if let count = userDefaults.object(forKey: NotificationService.notificationsCount) as? NSNumber {
            let new: NSNumber = count.intValue + 1 as NSNumber
            notification.badge = new
            userDefaults.set(new, forKey: NotificationService.notificationsCount)
        }
    }
