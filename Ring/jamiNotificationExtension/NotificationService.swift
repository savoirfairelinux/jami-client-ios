                        ...
                        if !self.daemonStarted {
                            ...
                            self.adapterService!.startDaemonWithListener { [weak self, weak group] event, eventData in
                                ...
                                case .fileTransfer:
                                    ...
                                    downloadingFiles -= 1
                                    if taskCompleted && downloadingFiles == 0 {
                                        group?.leave()
                                    }
...
extension NotificationService: DarwinNotificationHandler {
    ...
    func createAttachment(identifier: String, image: UIImage, options: [NSObject: AnyObject]?) -> UNNotificationAttachment? {
            ...
            do {
                ...
                os_log("&&&&&&failed to create attachments")
            }
            return nil
        }

    func presentFileNotification(from: String, url: URL) {
        let content = UNMutableNotificationContent()
        ...
        UNUserNotificationCenter.current().add(notificationRequest) { (error) in
            if let error = error {
                os_log("&&&&&&&&Unable to Add Notification Request")
                print("Unable to Add Notification Request (\(error), \(error.localizedDescription))")
            }
        }
    }
