...
class DaemonServiceTests: XCTestCase {
    ...
    /**
     ...
     - Returns: the RINGDaemonService used to do the test
     */
    func testStart() -> RINGDaemonService {
        var hasStartError = false
        let daemonService = RINGDaemonService.init(dRingAdaptor: DRingAdaptator())
        do {
    ...
    /**
     ...
     - Returns: the RINGDaemonService used to do the test
     */
    func testStop() -> RINGDaemonService {
    ...
    func testAlreadyRunningException() {
        ...
        XCTAssertThrowsError(try daemonService.startDaemon()) { (error) in
            XCTAssertEqual(error as? RINGStartDaemonError, RINGStartDaemonError.DaemonAlreadyRunning)
        }
    }
    ...
    func testDaemonNotRunningException() {
        ...
        XCTAssertThrowsError(try daemonService.stopDaemon()) { (error) in
            XCTAssertEqual(error as? RINGStopDaemonError, RINGStopDaemonError.DaemonNotRunning)
        }
    }
    ...
    func testDaemonFailToInit() {
        let daemonService = RINGDaemonService.init(dRingAdaptor: RINGFixtureFailInitDRingAdaptor())
        XCTAssertThrowsError(try daemonService.startDaemon()) { (error) in
            XCTAssertEqual(error as? RINGStartDaemonError, RINGStartDaemonError.InitializationFailure)
        }
    }
    ...
    func testDaemonFailToStart() {
        let daemonService = RINGDaemonService.init(dRingAdaptor: RINGFixtureFailStartDRingAdaptor())
        XCTAssertThrowsError(try daemonService.startDaemon()) { (error) in
            XCTAssertEqual(error as? RINGStartDaemonError, RINGStartDaemonError.StartFailure)
        }
    }
}
