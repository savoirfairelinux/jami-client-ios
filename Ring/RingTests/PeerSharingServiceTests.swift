...
class PeerSharingServiceTests: XCTestCase {
    ...
    func testPeerTunnelEndpointURL_buildsLoopbackURLsForWebSchemes() {
        ...
        XCTAssertEqual(httpTunnel.endpointURL?.absoluteString,
                       TestConstants.loopbackURL(scheme: TestConstants.httpScheme,
                                                 port: TestConstants.defaultPort))
        XCTAssertEqual(httpsTunnel.endpointURL?.absoluteString,
                       TestConstants.loopbackURL(scheme: TestConstants.httpsScheme,
                                                 port: TestConstants.alternatePort))
    }

    func testPeerTunnelEndpointURL_rejectsUnsupportedSchemeAndInvalidPort() {
        XCTAssertNil(makeTunnel(scheme: TestConstants.unsupportedScheme).endpointURL)
        XCTAssertNil(makeTunnel(localPort: TestConstants.invalidPort).endpointURL)
        XCTAssertNil(makeTunnel(localPort: Int(UInt16.max) + 1).endpointURL)
    }
