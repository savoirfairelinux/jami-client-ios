...
final class ProfilesServiceTests: XCTestCase {
    ...
    private final class StubInvitations: InvitationProfileProviding {
        var profiles: [String: Profile] = [:]

        func invitationProfile(forURI uri: String, accountId: String) -> Profile? {
            profiles[uri]
        }
    }

    private var store: StubProfileStore!
    private var invitations: StubInvitations!
    ...
    func testContactProfileWinsOverInvitationProfile() async throws {
        ...
        invitations.profiles[uri] = profile(profileName2,
    ...
    func testContactProfileCompletelyReplacesInvitationProfile() async throws {
        ...
        invitations.profiles[uri] = profile(profileName1,
                                            photo: Data("request-photo".utf8).base64EncodedString(),
                                            uri: uri)
    ...
    func testPendingInvitationProfileIsUsedWhenPeerVCardIsMissing() async throws {
        let uri = try XCTUnwrap(JamiURI(schema: .ring, infoHash: jamiId1).uriString)
        let photo = Data("request-photo".utf8).base64EncodedString()
        invitations.profiles[uri] = profile(profileName1, photo: photo, uri: uri)
    ...
    func testStoredInvitationProfileIsUsedOnceTheRequestIsGone() async throws {
        let uri = try XCTUnwrap(JamiURI(schema: .ring, infoHash: jamiId1).uriString)
    ...
    func testJamsSearchProfileWinsOverInvitationProfile() async throws {
        let uri = try XCTUnwrap(JamiURI(schema: .ring, infoHash: jamiId1).uriString)
        invitations.profiles[uri] = profile(profileName2, uri: uri)
        service.cacheJamsSearchProfile(uri: uri, accountId: accountId1, alias: profileName1, photo: nil)
    ...
    func testEmptyJamsSearchProfileIsNotCached() async throws {
        let uri = try XCTUnwrap(JamiURI(schema: .ring, infoHash: jamiId1).uriString)

        service.cacheJamsSearchProfile(uri: uri, accountId: accountId1, alias: nil, photo: nil)
        let resolved = try await resolvedProfile(uri: uri)
    ...
    func testSentRequestStoresTheJamsSearchProfile() throws {
        ...
        service.invitationSent(uri: uri, accountId: accountId1)
