...
class DaemonServiceTests: XCTestCase {
    ...
    func testDaemonFailToInit() {
        let daemonService = DaemonService.init(dRingAdaptor: FixtureFailInitDRingAdaptor())
        XCTAssertThrowsError(try daemonService.startDaemon()) { (error) in
    ...
    func testDaemonFailToStart() {
        let daemonService = DaemonService.init(dRingAdaptor: FixtureFailStartDRingAdaptor())
        XCTAssertThrowsError(try daemonService.startDaemon()) { (error) in
