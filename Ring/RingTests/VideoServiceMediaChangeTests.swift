...
class VideoServiceMediaChangeTests: XCTestCase {
    ...
    // MARK: - Regression test: audio mute must not change source

    /// Muting audio must only toggle the muted flag — changing the source
    /// triggers an SDP re-invite in the daemon (sipcall.cpp:isReinviteRequired),
    /// which restarts video on both sides.
    ...
    // MARK: - Guard against over-correction: video mute must still change source
