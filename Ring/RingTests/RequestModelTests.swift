...
final class RequestModelTests: XCTestCase {
    ...
    func testUpdateFromSameParticipant_DoesNotDuplicateParticipant() {
        let metadata = [
            RequestModel.RequestKey.conversationId.rawValue: conversationId1,
            RequestModel.RequestKey.from.rawValue: jamiId1
        ]
        let request = RequestModel(withDictionary: metadata, accountId: accountId1, type: .conversation)

        request.updatefrom(dictionary: metadata)

        XCTAssertEqual(request.participants.count, 1)
