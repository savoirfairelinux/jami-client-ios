...
class PeerSharingViewModelTests: XCTestCase {
    ...
    private enum TestConstants {
        ...
        static func loopbackURL(scheme: String = webScheme, port: Int = port) -> String {
    ...
    private func unsupportedService(id: String = TestConstants.otherServiceId) -> PeerServiceInfo {
        PeerServiceInfo(id: id,
                        name: "Unsupported Service",
                        description: "Unsupported service description",
                        scheme: TestConstants.unsupportedScheme,
                        device: deviceId1)
    }
    ...
    /// Drives a web service through query → open → TunnelOpened so the viewer is presented
    /// AND its loopback URL is ready (the tunnel is established).
    private func driveToPresented(serviceId: String = TestConstants.serviceId,
        ...
        wait(until: { self.viewModel.browseURL != nil }, "browse URL ready")
    }

    // MARK: - Present on tap, load in the viewer

    func testRefresh_keepsUnsupportedServicesVisible() {
        let web = webService()
        let unsupported = unsupportedService()

        deliverServices([web, unsupported])
        wait(until: { self.viewModel.services.count == 2 }, "all services loaded")

        XCTAssertTrue(viewModel.services.contains { $0.id == web.id })
        XCTAssertTrue(viewModel.services.contains { $0.id == unsupported.id })
    }

    func testBeginBrowsing_unsupportedServiceDoesNothing() {
        let unsupported = unsupportedService()
        deliverServices([unsupported])
        wait(until: { self.viewModel.services.contains { $0.id == unsupported.id } },
             "unsupported service loaded")

        viewModel.beginBrowsing(unsupported)

        XCTAssertNil(viewModel.presentedBrowseSession)
        XCTAssertNil(viewModel.browseURL)
        XCTAssertEqual(mockAdapter.openServiceTunnelCallCount, 0)
    }

    func testBeginBrowsing_presentsImmediatelyBeforeTunnelOpens() {
        let svc = webService()
        deliverServices([svc])
        wait(until: { self.viewModel.services.contains { $0.id == svc.id } }, "services loaded")

        viewModel.beginBrowsing(svc)

        // The viewer is presented right away, in a loading state (no URL, no failure) — the
        // tunnel has not opened yet.
        XCTAssertEqual(viewModel.presentedBrowseSession?.service.id, svc.id,
                       "Viewer must present immediately on tap")
        XCTAssertNil(viewModel.browseURL, "URL must not be ready before the tunnel opens")
        XCTAssertFalse(viewModel.browseFailed, "Must not be in a failed state while still opening")
    }

    func testTunnelOpens_providesBrowseURL() {
        driveToPresented()

        XCTAssertEqual(viewModel.browseURL?.absoluteString,
                       TestConstants.loopbackURL(),
                       "Once the tunnel opens, the loopback URL is handed to the viewer")
        XCTAssertFalse(viewModel.browseFailed)
    }

    func testTunnelOpenFailure_setsBrowseFailed() {
        let svc = webService()
        deliverServices([svc])
        wait(until: { self.viewModel.services.contains { $0.id == svc.id } }, "services loaded")

        mockAdapter.openTunnelReturnValue = ""   // daemon can't open the tunnel
        viewModel.beginBrowsing(svc)

        wait(until: { self.viewModel.browseFailed }, "viewer shows the failed state")
        XCTAssertNotNil(viewModel.presentedBrowseSession, "Viewer stays up to show the error")
        XCTAssertNil(viewModel.browseURL, "No URL on failure")
    }

    func testActiveTunnelWithInvalidEndpoint_setsBrowseFailed() {
        let svc = webService()
        deliverServices([svc])
        wait(until: { self.viewModel.services.contains { $0.id == svc.id } }, "services loaded")
        ...
        wait(until: { self.viewModel.browseFailed },
             "viewer fails when tunnel is active but URL cannot be built")
        XCTAssertNil(viewModel.browseURL)
        XCTAssertNotNil(viewModel.presentedBrowseSession)
    }
