...
final class CallHeaderModelTests: XCTestCase {
    ...
    private let jamiId = "8f4a1c9e02b7d5638a1f0c4d7e2b95af3c68d10e"

    private func call(peerUri: String, displayName: String = "",
                      isRecording: Bool = false) -> CallState {
        var call = CallState(id: CallId(raw: "call1"), accountId: "acc",
                             direction: .outgoing, peerUri: peerUri,
                             status: .current)
        call.displayName = displayName
        call.peerIsRecording = isRecording
        return call
    }

    private func participant(_ uri: String) -> ConferenceParticipantInfo {
        ConferenceParticipantInfo([ConfInfoKey.uri.rawValue: uri,
                                   ConfInfoKey.device.rawValue: "d1"])!
    }

    private func conference(_ uris: [String]) -> ConferenceState {
        ConferenceState(id: ConfId(raw: "conf1"), accountId: "acc",
                        participants: uris.map(participant))
    }
    ...
    private func header(call: CallState?, conference: ConferenceState? = nil,
        ...
        if let conference = conference {
            rows = ConferenceParticipants.rows(from: conference, localJamiId: localId)
    ...
    private func pendingRow(_ uri: String) -> PendingParticipantRow {
        PendingParticipantRow(id: uri, callId: CallId(raw: "invite"), uri: uri,
                              status: .connecting)
    }
    ...
    func testAnUnansweredInviteIsAnnouncedOnTheStatusLine() {
        ...
        ongoing.pendingInvites = [PendingConferenceInvite(callId: CallId(raw: "invite"),
                                                          peerUri: "alice",
                                                          status: .ringing)]
    ...
    func testRecordingIsCarriedFromTheCall() {
        let model = header(call: call(peerUri: "bob", isRecording: true))
    ...
    func testAPeerOnTwoDevicesGetsOneAvatar() {
        let conference = ConferenceState(
            id: ConfId(raw: "conf1"), accountId: "acc",
            participants: [participant(localId),
                           ConferenceParticipantInfo([ConfInfoKey.uri.rawValue: "alice",
                                                      ConfInfoKey.device.rawValue: "d1"])!,
                           ConferenceParticipantInfo([ConfInfoKey.uri.rawValue: "alice",
                                                      ConfInfoKey.device.rawValue: "d2"])!,
                           participant("bob")])
