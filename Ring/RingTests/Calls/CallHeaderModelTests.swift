...
final class CallHeaderModelTests: XCTestCase {
    ...
    func testAnUnansweredInviteStopsTheHeaderNamingOnePerson() {
        ...
        XCTAssertEqual(model.avatars.map(\.uri), ["bob", "alice"],
    ...
    func testAnInviteeWhoAnswersStopsBeingPending() {
        let joined = header(call: call(peerUri: "alice"),
                            conference: conference([localId, "alice", "bob"]))
        XCTAssertEqual(joined.avatars.map(\.isPending), [false, false],
                       "answering settles the face it was already drawn with")
        XCTAssertFalse(joined.isInviting)
    }
    ...
    func testAnUnansweredInviteLeavesTheDurationRunning() {
        ...
        ongoing.pendingInvites = [PendingConferenceInvite(callId: CallTestFixtures.secondaryCallId,
                                                          peerUri: "alice",
                                                          status: .ringing)]
        XCTAssertNotNil(CallHeaderModel.statusLine(for: ongoing)
    ...
    func testAnInviteIsStillAnnouncedWhenItsFaceDoesNotFit() {
        let model = header(call: call(peerUri: "alice"),
                           conference: conference([localId, "alice", "bob", "carol", "dave"]),
                           pending: [pendingRow("erin")])
    ...
    func testAPeerOnTwoDevicesGetsOneAvatar() {
        ...
        XCTAssertEqual(model.avatars.map(\.uri), ["alice", "bob"],
    ...
    func testConferenceShowsAtMostThreeAvatarsAndNeverTheLocalOne() {
        ...
        XCTAssertEqual(model.avatars.map(\.uri), ["alice", "bob", "carol"])
