...
final class CallStoreTests: XCTestCase {
    ...
    func testHostedSwarmPlacementsAreScopedByAccount() async throws {
        let conferenceId1 = ConfId(raw: "conf-account-1")
        let conferenceId2 = ConfId(raw: "conf-account-2")
        callAPI.placeCallReturn = nil
        let placementsStarted = expectation(description: "swarm calls placed")
        placementsStarted.expectedFulfillmentCount = 2
        callAPI.onPlaceCall = { placementsStarted.fulfill() }

        async let placed1 = store.placeSwarmCall(accountId: accountId1,
                                                 conversationId: conversationId1,
                                                 audioOnly: true,
                                                 videoSource: String(),
                                                 timeout: 2)
        async let placed2 = store.placeSwarmCall(accountId: accountId2,
                                                 conversationId: conversationId1,
                                                 audioOnly: true,
                                                 videoSource: String(),
                                                 timeout: 2)
        await fulfillment(of: [placementsStarted], timeout: 1)

        sendEvent(.conferenceCreated(conferenceId: conferenceId2.raw,
                                     conversationId: conversationId1,
                                     accountId: accountId2))
        sendEvent(.conferenceCreated(conferenceId: conferenceId1.raw,
                                     conversationId: conversationId1,
                                     accountId: accountId1))

        let resolved1 = try await placed1
        let resolved2 = try await placed2
        XCTAssertEqual(resolved1, conferenceId1)
        XCTAssertEqual(resolved2, conferenceId2)

        let state = await store.snapshot()
        XCTAssertEqual(state.call(CallId(raw: conferenceId1.raw))?.accountId, accountId1)
        XCTAssertEqual(state.call(CallId(raw: conferenceId2.raw))?.accountId, accountId2)
    }
