...
extension ParticipantCanvasTests {
    ...
    func testNameStaysInsideItsPlateAfterAZeroWidthLayoutPass() throws {
        ...
        for mode in [CanvasLayoutMode.grid, .fullscreen("p0")] {
            ...
            XCTAssertTrue(CGRect(origin: .zero, size: chip.size).contains(text),
                          "in \(mode) the name escaped its plate: text \(text) in plate "
                            + "\(CGRect(origin: .zero, size: chip.size))")
        }
    }
    ...
    func testPlateResizesWhenTheNameResolvesAfterLayout() throws {
        ...
        XCTAssertTrue(CGRect(origin: .zero, size: resolved.size).contains(text),
                      "the name escaped its plate: \(text) in \(resolved.size)")
    }
