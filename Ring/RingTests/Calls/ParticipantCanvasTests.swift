...
final class ParticipantCanvasTests: XCTestCase {
    ...
    func testPreviewCornerRoundingFollowsMode() {
        ...
        canvas.apply(models: [localModel], mode: .grid)
        canvas.layoutIfNeeded()
        XCTAssertEqual(tile(CanvasParticipant.localId, in: canvas)?.cornerStyle, .square,
                       "fullscreen dialing preview is edge to edge")

        canvas.apply(models: [remoteModel, localModel], mode: .grid)

        XCTAssertEqual(tile(CanvasParticipant.localId, in: canvas)?.cornerStyle,
                       .clipped(CanvasLayout.tileCornerRadius),
                       "preview floats over other tiles, so it clips its own corners")
        XCTAssertEqual(tile("call1", in: canvas)?.cornerStyle, .square,
    ...
    func testCardStyleKeepsConferenceTilesClearOfTheScreenEdge() {
        ...
        for index in 0..<4 {
            ...
            XCTAssertEqual(tile.cornerStyle, .backdrop(CanvasLayout.tileCornerRadius),
                           "conference tiles are rounded cards")
            XCTAssertEqual(tile.layer.cornerRadius, 0,
                           "cards must not mask the video layer to round their corners")
        }
    }

    func testSpotlightFocusRoundsWithoutMaskingItsVideo() {
        let canvas = makeCanvas()
        canvas.apply(models: remoteModels(4), mode: .spotlight("p0"), style: .cards)
        canvas.layoutIfNeeded()

        let focus = try? XCTUnwrap(tile("p0", in: canvas))
        XCTAssertEqual(focus?.cornerStyle, .backdrop(CanvasLayout.tileCornerRadius))
        XCTAssertEqual(focus?.layer.cornerRadius, 0,
                       "the largest video in the call must not be clipped every frame")
    }
    ...
    func testPlainStyleKeepsDirectCallTilesEdgeToEdge() {
        ...
        XCTAssertEqual(tile?.cornerStyle, .square)
    }
