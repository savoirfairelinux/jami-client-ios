...
final class CallViewModelTests: XCTestCase { // swiftlint:disable:this type_body_length
    ...
    func testCallDetachedBeforeItsEndStillFollowsTheConferenceItLeft() async {
        ...
        await Harness.wait {
            model.currentCallId == survivorId && model.call?.id == survivorId
        }
        XCTAssertFalse(model.shouldDismiss,
                       "the session lives on in the member that stayed in the conference")
    }

    func testMissingConferenceMemberDismissesEndedCall() async {
        let harness = await Harness(callHasVideo: false)
        let model = harness.makeModel(localJamiId: jamiId1)
        let missingCallId = CallTestFixtures.hostCallId
        harness.createConference(with: [harness.callId, missingCallId])
        await Harness.wait { model.conference?.memberCallIds.count == 2 }

        harness.changeConferenceMembers(to: [missingCallId])
        await Harness.wait { model.conference == nil }
        harness.send(.callStateChanged(callId: harness.callId.raw, state: .over,
                                       rawState: LibJamiCallState.over.rawValue,
                                       accountId: Harness.accountId,
        ...
        XCTAssertEqual(model.call?.status, .terminated(.over))
        XCTAssertEqual(model.currentCallId, harness.callId,
                       "a nonexistent member must not become a permanent observation target")
    }

    func testMissingConferenceMemberDoesNotHideLaterLiveCall() async {
        let harness = await Harness(callHasVideo: false)
        let model = harness.makeModel(localJamiId: jamiId1)
        let missingCallId = CallId(raw: "000-missing-call")
        let liveCallId = CallTestFixtures.hostCallId
        await harness.addOngoingCall(liveCallId, peerUri: CallTestFixtures.tertiaryPeerUri)
        harness.createConference(with: [harness.callId, missingCallId, liveCallId])
        await Harness.wait { model.conference?.memberCallIds.count == 3 }

        harness.changeConferenceMembers(to: [missingCallId, liveCallId])
        await Harness.wait { model.conference == nil }
        harness.send(.callStateChanged(callId: harness.callId.raw, state: .over,
                                       rawState: LibJamiCallState.over.rawValue,
                                       accountId: Harness.accountId,
        ...
        await Harness.wait {
            model.currentCallId == liveCallId && model.call?.id == liveCallId
        }
        XCTAssertFalse(model.shouldDismiss)
    }
