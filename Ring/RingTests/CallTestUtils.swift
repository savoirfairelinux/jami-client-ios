...
// MARK: - Rx Test Helpers

extension XCTestCase {
    /// Expects and waits for an event from a subject within the specified timeout
    func expectEvent<T>(
        from subject: Observable<T>,
        ...
        subject
            .take(1)
            .timeout(.seconds(Int(timeout)), scheduler: MainScheduler.instance)
            .subscribe(
                onNext: { event in
                    capturedEvent = event
                    expectation.fulfill()
                ...
                onError: { error in
                    XCTFail("No event was published within timeout: \(error)")
                }
            )
    ...
    /// Expects that no events are emitted from a subject within the specified timeout
    func expectNoEvents<T>(
        from subject: Observable<T>,
        timeout: TimeInterval = 0.5,
        disposeBag: DisposeBag
    ) {
        let expectation = XCTestExpectation(description: "No events published")
        expectation.isInverted = true
        
        subject
            .take(1)
            .timeout(.milliseconds(Int(timeout * 1000)), scheduler: MainScheduler.instance)
            .subscribe(
                onNext: { _ in
                    XCTFail("An event was unexpectedly published")
                }, 
                onError: { _ in
