...
final class GroupAvatarRendererTests: XCTestCase {
    ...
    private func makeCandidate(name: String, role: ParticipantRole?, image: UIImage? = nil) -> GroupAvatarCandidate {
    ...
    func testSelectForDisplay_AllBannedReturnsEmpty() {
        let candidates = [
            makeCandidate(name: profileName1, role: ParticipantRole.banned),
            makeCandidate(name: profileName2, role: ParticipantRole.left)
        ]
    ...
    func testSelectForDisplay_FiltersBannedAndLeft() {
        let candidates = [
            makeCandidate(name: profileName1, role: ParticipantRole.admin),
            makeCandidate(name: profileName2, role: ParticipantRole.member),
            makeCandidate(name: profileName3, role: ParticipantRole.banned),
            makeCandidate(name: profileName4, role: ParticipantRole.left)
        ]
        let selection = GroupAvatarRenderer.selectForDisplay(from: candidates)
    ...
    func testSelectForDisplay_IncludesInvited() {
        let candidates = [
            makeCandidate(name: profileName1, role: ParticipantRole.admin),
            makeCandidate(name: profileName2, role: ParticipantRole.invited)
        ]
        let selection = GroupAvatarRenderer.selectForDisplay(from: candidates)
        XCTAssertEqual(selection.members.count, 2)
    ...
    func testSelectForDisplay_AdminAlwaysFirst() {
        let candidates = [
            makeCandidate(name: profileName2, role: ParticipantRole.member),
            makeCandidate(name: profileName3, role: ParticipantRole.member),
            makeCandidate(name: profileName1, role: ParticipantRole.admin)
        ]
        let selection = GroupAvatarRenderer.selectForDisplay(from: candidates)
    ...
    func testSelectForDisplay_PrioritizesImageOverName() {
        ...
        let candidates = [
            makeCandidate(name: profileName1, role: ParticipantRole.admin),
            makeCandidate(name: profileName2, role: ParticipantRole.member),
            makeCandidate(name: profileName3, role: ParticipantRole.member, image: image),
            makeCandidate(name: jamiId3, role: ParticipantRole.member)
        ]
        let selection = GroupAvatarRenderer.selectForDisplay(from: candidates)
        XCTAssertEqual(selection.members.count, 2)
        XCTAssertEqual(selection.members[0].name, profileName1)
    ...
    func testSelectForDisplay_HashNameGetsLowestPriority() {
        let candidates = [
            makeCandidate(name: profileName1, role: ParticipantRole.admin),
            makeCandidate(name: jamiId2, role: ParticipantRole.member),
            makeCandidate(name: registeredName1, role: ParticipantRole.member),
            makeCandidate(name: jamiId3, role: ParticipantRole.member)
        ]
        let selection = GroupAvatarRenderer.selectForDisplay(from: candidates)
        XCTAssertEqual(selection.members.count, 2)
    ...
    func testSelectForDisplay_OverflowWith4Members() {
        let candidates = [
            ...
            makeCandidate(name: profileName2, role: ParticipantRole.member),
            makeCandidate(name: profileName3, role: ParticipantRole.member),
            makeCandidate(name: profileName4, role: ParticipantRole.member)
        ]
        let selection = GroupAvatarRenderer.selectForDisplay(from: candidates)
        XCTAssertEqual(selection.members.count, 2)
        XCTAssertEqual(selection.overflowCount, 2)
    ...
    func testSelectForDisplay_OverflowExcludesInactiveFromCount() {
        let candidates = [
            ...
            makeCandidate(name: profileName2, role: ParticipantRole.member),
            makeCandidate(name: profileName3, role: ParticipantRole.member),
            makeCandidate(name: profileName4, role: ParticipantRole.banned),
            makeCandidate(name: registeredName1, role: ParticipantRole.left)
        ]
        let selection = GroupAvatarRenderer.selectForDisplay(from: candidates)
        XCTAssertEqual(selection.members.count, 3)
        XCTAssertEqual(selection.overflowCount, 0)
    ...
    func testSelectForDisplay_IndicesMapToOriginalArray() {
        ...
        XCTAssertEqual(candidates[selection.selectedIndices[0]].role, ParticipantRole.admin)
    }
    ...
    func testMemberEquality_SameImageInstance() {
        let image = createTestImage()
        let firstMember = GroupAvatarMember(image: image, name: profileName1)
        let secondMember = GroupAvatarMember(image: image, name: profileName1)
        XCTAssertEqual(firstMember, secondMember)
    }

    func testMemberEquality_DifferentImageInstances() {
        let firstMember = GroupAvatarMember(image: createTestImage(), name: profileName1)
        let secondMember = GroupAvatarMember(image: createTestImage(), name: profileName1)
        XCTAssertNotEqual(firstMember, secondMember)
    }

    func testMemberEquality_BothNilImages() {
        let firstMember = GroupAvatarMember(image: nil, name: profileName1)
        let secondMember = GroupAvatarMember(image: nil, name: profileName1)
        XCTAssertEqual(firstMember, secondMember)
    }
    ...
    func testMemberEquality_DifferentNames() {
        let image = createTestImage()
        let firstMember = GroupAvatarMember(image: image, name: profileName1)
        let secondMember = GroupAvatarMember(image: image, name: profileName2)
        XCTAssertNotEqual(firstMember, secondMember)
    }
