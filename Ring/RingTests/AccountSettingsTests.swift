...
class AccountSettingsTests: XCTestCase {
    ...
    private let testHost = "stun.invalid"
    ...
    func testIsValidStunServerAcceptsHostAndHostPort() {
        XCTAssertTrue(AccountSettings.isValidStunServer(testHost))
        XCTAssertTrue(AccountSettings.isValidStunServer("\(testHost):3478"))
        XCTAssertTrue(AccountSettings.isValidStunServer("\(testHost):0"))
    }

    func testIsValidStunServerRejectsInvalidPortAndEmpty() {
        XCTAssertFalse(AccountSettings.isValidStunServer(""))
        XCTAssertFalse(AccountSettings.isValidStunServer("\(testHost):abc"))
        XCTAssertFalse(AccountSettings.isValidStunServer("\(testHost):"))
        XCTAssertFalse(AccountSettings.isValidStunServer("\(testHost):99999"))
        XCTAssertFalse(AccountSettings.isValidStunServer(":3478"))
        XCTAssertFalse(AccountSettings.isValidStunServer("\(testHost):0:0"))
    }
}
