...
class ConferenceManagementServiceTests: XCTestCase {
    ...
    func testEndCall() {
        ...
        let endCompletable = conferenceManagementService.endCallOrConference(callId: CallTestConstants.callId, isSwarm: false)
        ...
        verifyAdapter(
            property: mockCallsAdapter.endConferenceCallCount,
            expectedValue: 0,
            message: "End conference should not be called"
        )
        verifyAdapter(
    ...
    func testEndConference() {
        let expectation = XCTestExpectation(description: "End conference completed")
        mockCallsAdapter.endConferenceReturnValue = true
        setupCallWithMultipleParticipants(
            TestConstants.conferenceId,
            participants: [CallTestConstants.callId, TestConstants.secondCallId]
        )

        let endCompletable = conferenceManagementService.endCallOrConference(callId: TestConstants.conferenceId, isSwarm: false)

        endCompletable
            .subscribe(onCompleted: {
                expectation.fulfill()
            }, onError: { error in
                XCTFail("End conference should complete without error: \(error)")
            })
            .disposed(by: disposeBag)
        ...
        verifyAdapter(
            property: mockCallsAdapter.endConferenceCallCount,
            expectedValue: 1,
            message: "End conference should be called once"
        )
        verifyAdapter(
            property: mockCallsAdapter.endConferenceCallId,
            expectedValue: TestConstants.conferenceId,
            message: "Conference ID should match"
        )
    }
    ...
    func testEndParticipant() {
        let expectation = XCTestExpectation(description: "End participant completed")
        setupCallWithSingleParticipant(TestConstants.conferenceId)

        conferenceManagementService.endParticipant(
            confId: TestConstants.conferenceId,
            participantId: TestConstants.participantURI,
        ...
        verifyAdapter(
            property: mockCallsAdapter.endConferenceParticipantCallCount,
            expectedValue: 1,
            message: "End conference participant should be called"
        )
        verifyAdapter(
            property: mockCallsAdapter.endConferenceParticipantParticipantId,
            expectedValue: TestConstants.participantURI,
            message: "Participant ID should match"
        )
        verifyAdapter(
            property: mockCallsAdapter.endConferenceParticipantConferenceId,
            expectedValue: TestConstants.conferenceId,
            message: "Conference ID should match"
        )
        verifyAdapter(
            property: mockCallsAdapter.endConferenceParticipantDeviceId,
