...
final class ConversationEventOrderingTests: XCTestCase {
    ...
    func testSourceReturnsWhileDeliveryQueueIsBusy() {
        let queue = DispatchQueue(label: "conversation-events-test")
        var delivered = false
        let source = ConversationEventSource(queue: queue) { event in
        ...
        queue.resume()
        queue.sync { XCTAssertTrue(delivered) }
    }
    ...
    private func checkReadyBeforeUpdates(existingConversation: Bool) {
        ...
        let source = service.makeEventSource { event in
            switch event {
