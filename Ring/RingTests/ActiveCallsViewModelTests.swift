...
final class ActiveCallsViewModelTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        let conversation = ConversationModel(withId: "conv1", accountId: "account1", info: [:])
        conversation.type = .publicChat
        conversationVM.conversation = conversation

        let participant = ParticipantInfo(jamiId: "peer", role: .admin, profileService: injectionBag.profileService)
        ...
        swarmInfo.finalTitle.accept("Swarm")
    ...
    func testActiveCalls_WhenSameCallReportedForMultipleAccounts_DedupesToSingleRow() {
        ...
        let account1 = AccountModel(withAccountId: "account1")
        let account2 = AccountModel(withAccountId: "account2")
        ...
        injectionBag.callService.activeCallsChanged(conversationId: conversationId, calls: calls, account: account1)
        injectionBag.callService.activeCallsChanged(conversationId: conversationId, calls: calls, account: account2)

        // Wait for Rx -> MainScheduler delivery.
        let exp = expectation(description: "callsByAccount updated")
