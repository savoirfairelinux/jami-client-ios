...
final class ActiveCallsViewModelTests: XCTestCase {
    ...
    func testActiveCalls_WhenSameCallReportedForMultipleAccounts_DedupesToSingleRow() {
        ...
        let emissions = expectation(description: "view model processes both tracker emissions")
        emissions.expectedFulfillmentCount = 2
        viewModel.$callsByAccount
            .filter { !$0.isEmpty }
            .sink { _ in emissions.fulfill() }
        ...
        XCTAssertEqual(allRows.first?.call.accountId, account1.id, "remaining row must belong to the currently-selected account")
    }
}
