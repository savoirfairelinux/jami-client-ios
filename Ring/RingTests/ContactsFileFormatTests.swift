...
final class ContactsFileFormatTests: XCTestCase {
    ...
    private func createJamiAccount() throws -> String {
        ...
        details[ConfigKey.accountDTMFType.rawValue] = "oversip"
        details[ConfigKey.ringtoneEnabled.rawValue] = false.toString()
        details[ConfigKey.proxyEnabled.rawValue] = true.toString()
    ...
    private func waitForAccountReady(accountId: String) throws {
        ...
        XCTFail("account \(accountId) did not leave \(initializing) within 15s")
    }
    ...
    private func waitForDecodedContacts(accountId: String, expectedCount: Int) throws -> [[String: Any]] {
        ...
        XCTFail("contacts file did not contain \(expectedCount) entries within 10s")
        return []
    }
}
