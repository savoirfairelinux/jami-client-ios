...
final class MessageMarkdownTests: XCTestCase {
    ...
    @available(iOS 15.0, *)
    func test_attributedString_markdownLink_disallowedSchemeNotLinked() {
        for input in [Sample.javascriptLink, Sample.telLink] {
            guard let attributed = MessageMarkdown.attributedString(from: input, linkColor: .blue) else {
                return XCTFail("Expected attributed string for \(input)")
            }
            XCTAssertFalse(attributed.runs.contains { $0.link != nil }, input)
        }
    }
