...
final class DBManagerConversationDeletionTests: XCTestCase {
    ...
    func testPlaceholderProfileRoundTripsBinaryData() throws {
        let uri = "jami:\(jamiId1)"
        let photo = Data([0, 1, 2, 255]).base64EncodedString()
        let placeholder = Profile(uri: uri,
                                  alias: profileName1,
                                  photo: photo,
                                  type: ProfileType.ring.rawValue)

        XCTAssertTrue(manager.savePlaceholderProfile(placeholder, accountId: accountId))

        let loaded = try XCTUnwrap(manager.placeholderProfile(for: uri, accountId: accountId))
        XCTAssertEqual(loaded.alias, profileName1)
        XCTAssertEqual(loaded.photo, photo)
    }

    func testRemovingConversationProfileKeepsPlaceholderProfile() throws {
        let uri = "jami:\(jamiId1)"
        let placeholder = Profile(uri: uri,
                                  alias: profileName1,
                                  photo: nil,
        ...
        XCTAssertEqual(manager.placeholderProfile(for: uri, accountId: accountId)?.alias, profileName1)
    }

    func testUnreadablePlaceholderProfileIsIgnored() throws {
        let uri = "jami:\(jamiId1)"
        let path = try XCTUnwrap(ProfilePathHelper.placeholderProfilePath(accountId: accountId,
                                                                          profileURI: uri,
                                                                          documents: try XCTUnwrap(Constants.documentsPath),
                                                                          createIfNotExists: true))
        try Data("not a vcard".utf8).write(to: URL(fileURLWithPath: path))

        XCTAssertNil(manager.placeholderProfile(for: uri, accountId: accountId))
    }
