...
final class DBManagerConversationDeletionTests: XCTestCase {
    ...
    func testRemovingProfileKeepsContactProfileAndDeletesClientFiles() throws {
        let uri = "jami:\(jamiId1)"
        ...
        let contactProfile = try XCTUnwrap(ProfilePathHelper.contactProfilePath(accountId: accountId,
                                                                                contactId: uri,
                                                                                documents: documents))
        ...
        let localOverride = try XCTUnwrap(container.contactProfileOverridePath(accountId: accountId,
                                                                               profileURI: uri,
                                                                               createIfNotExists: true))
        let data = Data("profile".utf8)
        try data.write(to: URL(fileURLWithPath: contactProfile))
        for path in legacyProfiles + [localOverride] {
            try data.write(to: URL(fileURLWithPath: path))
        }

        container.removeProfile(accountId: accountId, profileURI: uri)

        XCTAssertTrue(FileManager.default.fileExists(atPath: contactProfile))
        for path in legacyProfiles + [localOverride] {
            XCTAssertFalse(FileManager.default.fileExists(atPath: path))
        }
    }

    func testProvisionalProfileRoundTripsBinaryData() throws {
        let uri = "jami:\(jamiId1)"
        let photo = Data([0, 1, 2, 255]).base64EncodedString()
        let provisional = Profile(uri: uri,
                                  alias: profileName1,
                                  photo: photo,
                                  type: ProfileType.ring.rawValue)

        XCTAssertTrue(manager.saveProvisionalProfile(provisional, accountId: accountId))

        let loaded = try XCTUnwrap(manager.provisionalProfile(for: uri, accountId: accountId))
        XCTAssertEqual(loaded.alias, profileName1)
        XCTAssertEqual(loaded.photo, photo)
    }

    func testRemovingConversationProfileKeepsProvisionalProfile() throws {
        let uri = "jami:\(jamiId1)"
        let provisional = Profile(uri: uri,
                                  alias: profileName1,
                                  photo: nil,
        ...
        XCTAssertEqual(manager.provisionalProfile(for: uri, accountId: accountId)?.alias, profileName1)
    }

    func testUnreadableProvisionalProfileIsIgnored() throws {
        let uri = "jami:\(jamiId1)"
        let path = try XCTUnwrap(ProfilePathHelper.provisionalProfilePath(accountId: accountId,
                                                                          profileURI: uri,
                                                                          documents: try XCTUnwrap(Constants.documentsPath),
                                                                          createIfNotExists: true))
        try Data("not a vcard".utf8).write(to: URL(fileURLWithPath: path))

        XCTAssertNil(manager.provisionalProfile(for: uri, accountId: accountId))
    }
