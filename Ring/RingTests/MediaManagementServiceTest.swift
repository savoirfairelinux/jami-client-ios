...
class MediaManagementServiceTests: XCTestCase {
    ...
    private func expectCallUpdate(timeout: TimeInterval = 1.0) -> CallModel? {
        return expectEvent(from: callUpdates.asObservable(), timeout: timeout, disposeBag: disposeBag)
    }
    
    private func expectNoCallUpdates(timeout: TimeInterval = 0.5) {
        expectNoEvents(from: callUpdates.asObservable(), timeout: timeout, disposeBag: disposeBag)
    }
    ...
    func testAudioMuted() {
        ...
        if let updatedCall = expectCallUpdate() {
            XCTAssertTrue(updatedCall.audioMuted, "Call audio should be muted")
    ...
    func testAudioUnmuted() {
        ...
        if let updatedCall = expectCallUpdate() {
            XCTAssertFalse(updatedCall.audioMuted, "Call audio should be unmuted")
        }
    }
    ...
    func testVideoMuted() {
        ...
        if let updatedCall = expectCallUpdate() {
            XCTAssertTrue(updatedCall.videoMuted, "Call video should be muted")
            XCTAssertEqual(updatedCall.callId, CallTestConstants.callId, "Event should refer to the correct call")
        }
    }
    ...
    func testVideoUnmuted() {
        ...
        if let updatedCall = expectCallUpdate() {
            XCTAssertFalse(updatedCall.videoMuted, "Call video should be unmuted")
            XCTAssertEqual(updatedCall.callId, CallTestConstants.callId, "Event should refer to the correct call")
        }
    }
    ...
    func testHandleRemoteRecordingChanged() {
        ...
        if let updatedCall = expectCallUpdate() {
            XCTAssertTrue(updatedCall.callRecorded, "Call should be marked as recorded")
    ...
    func testHandleRemoteRecordingDisabled() {
        ...
        if let updatedCall = expectCallUpdate() {
            XCTAssertFalse(updatedCall.callRecorded, "Call should not be marked as recorded")
            XCTAssertEqual(updatedCall.callId, CallTestConstants.callId, "Updated call should have the correct ID")
        }
    }
    ...
    func testHandleCallPlacedOnHold() {
        ...
        if let updatedCall = expectCallUpdate() {
            XCTAssertTrue(updatedCall.peerHolding, "Call should be marked as on hold")
            XCTAssertEqual(updatedCall.callId, CallTestConstants.callId, "Updated call should have the correct ID")
        }
    }
    ...
    func testHandleMediaNegotiationStatus() {
        ...
        if let updatedCall = expectCallUpdate() {
            XCTAssertEqual(mockCallsAdapter.callDetailsCallCount, 1, "Call details should be requested once")
            XCTAssertEqual(updatedCall.mediaList.count, 1, "Updated call should have one media item")
        }
    }
