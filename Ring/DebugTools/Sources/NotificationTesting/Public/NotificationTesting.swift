...
public enum NotificationTesting {
    ...
    ///
    /// The collector URL is resolved from shared App Group defaults:
    ///
    ///   1. **DebugTools UI** — the main app's NotificationTestingConfigView
    ///      writes the collector URL into the App Group's shared `UserDefaults`
    ///      under key `DebugToolsServerURL`.
    ///
    ///   2. **Notification extension handoff** — the extension reads from the
    ///      same shared defaults on its next wake-up, so it uses the URL last
    ///      saved by the main app.
    ///
    ///   3. **Simulator fallback** — when no shared URL exists and the code is
    ///      running on the iOS Simulator, the logger defaults to
