...
public struct NotificationTestingConfigView: View {
    ...
    @SwiftUI.State private var endpointType: NotificationTesting.EndpointType
    ...
    public init(
        ...
        let savedType = defaults?.string(forKey: "DebugToolsEndpointType")
            .flatMap(NotificationTesting.EndpointType.init(rawValue:))
        _endpointType = SwiftUI.State(initialValue: savedType ?? .collector)
    ...
    private var collectorURLRow: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("Collector URL")
            TextField("http://192.168.x.y:8080", text: $collectorURL)
    ...
    private var endpointTypeRow: some View {
        HStack {
            Text("Endpoint Type")
            Spacer()
            Picker("Type", selection: $endpointType) {
                Text("OTel").tag(NotificationTesting.EndpointType.otel)
                Text("Collector").tag(NotificationTesting.EndpointType.collector)
            }
            .pickerStyle(.segmented)
            .frame(width: 200)
            .onChange(of: endpointType) { newValue in
                NotificationTesting.setEndpointType(newValue)
            }
        }
        .padding()
    }
