...
final class SmartListViewTests: JamiBaseOneAccountUITest {
    ...
    func testSearchDoesNotReappearAfterPromotingTemporaryConversation() throws {
        ...
        let tempRow = app.cells.firstMatch
        waitForElementToAppear(tempRow, timeout: 10)
        tempRow.tap()

        let messageField = app.textViews.firstMatch
        waitForElementToAppear(messageField, timeout: 10)
        messageField.tap()
        messageField.typeText("hi")
        let sendButton = app.buttons.matching(NSPredicate(format: "isHittable == true")).allElementsBoundByIndex.last
        sendButton?.tap()
