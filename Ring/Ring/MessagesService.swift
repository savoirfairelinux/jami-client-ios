...
class MessagesService: MessagesAdapterDelegate {
    ...
    func didReceiveMessage(_ message: Dictionary<String, String>, from senderAccount: String, to receiverAccountId: String) {
        print("didReceiveMessage: \(message) from: \(senderAccount) to: \(receiverAccountId)")
    }

    func messageStatusChanged(_ status: MessageStatus, for messageId: UInt64, from senderAccountId: String, to receiverAccount: String) {
