...
class CallViewController: UIViewController, StoryboardBased, ViewModelBased, ContactPickerDelegate {
    ...
    func configureIncomingVideView() {
        ...
        videoContainerViewModel = ContainerViewModel(localId: localId, delegate: self)
        ...
        self.viewModel.layoutUpdated
            ...
            .subscribe(onNext: { [weak self] updated in
                ...
                if let participants = self.viewModel.getConferenceParticipants() {
                    ...
                    self.videoContainerViewModel.setCallLayout(layout: self.viewModel.getLayout())
                }
                return
    ...
    func setupBindings() {
        ...
        self.viewModel.renderStarted
            ...
            .subscribe(onNext: { [weak self] renderId in
                guard let self = self, !renderId.isEmpty else { return }
                self.incomingVideoStarted()
                if let input = self.viewModel.getVideoInput(renderId: renderId) {
                    self.videoContainerViewModel.addVideoInput(videoInput: input, renderId: renderId)
                    self.videoContainerViewModel.setCallLayout(layout: self.viewModel.getLayout())
                }
            })
            .disposed(by: self.disposeBag)

        self.viewModel.renderStopped
            .observe(on: MainScheduler.instance)
            .subscribe(onNext: { [weak self] renderId in
                guard let self = self, !renderId.isEmpty, let videoContainerViewModel = self.videoContainerViewModel else { return }
                videoContainerViewModel.removeVideoInput(renderId: renderId)
                videoContainerViewModel.setCallLayout(layout: self.viewModel.getLayout())
            })
            .disposed(by: self.disposeBag)
    }
