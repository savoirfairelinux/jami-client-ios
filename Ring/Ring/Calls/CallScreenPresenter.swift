...
final class CallScreenPresenter {
    ...
    private func presentCallScreen(for call: CallState) {
        ...
        let presented = isMinimized ? nil : callController
        callController = nil
        isMinimized = false
    ...
    private func deferUntilChildTransitionEnds(_ controller: UIViewController) -> Bool {
        ...
        return coordinator.animate(alongsideTransition: nil) { [weak self, weak controller] _ in
            self?.dismissCallScreen(controller)
        }
    }
