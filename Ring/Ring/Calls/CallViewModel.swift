...
class CallViewModel: Stateable, ViewModel {
    ...
    var call: CallModel? {
        didSet {
            self.saveVCard.subscribe(onNext: { (call, vCard) in
                self.contactsService.saveVCard(vCard: vCard, forContactWithRingId: call.fromRingId)
                    .subscribe()
    ...
    lazy var dismisVC: Observable<Bool> = {
        return callService.currentCall.map({call in
            return call.state == .over && call.callId == self.call?.callId
        }).map({ hide in
            return hide
        })
    }()
    ...
    lazy var info: Observable<String> = {
        return callService.currentCall.map({ call in
    ...
    lazy var saveVCard: Observable<(CallModel, CNContact)> = {
        return Observable<(CallModel, CNContact)>.zip(self.callService.currentCall.asObserver(),
    ...
    func ignoreCall() {
        self.callService.refuse(callId: self.call!.callId).subscribe(onCompleted: {
    ...
    func placeCall(with uri: String) {
        let name = self.contactsService.contact(withRingId: uri)?.userName
        self.callService.placeCall(withAccount: self.accountService.currentAccount!, toRingId: uri, name: name)
