...
class CallViewModel: Stateable, ViewModel {
    ...
    lazy var callObservable: Observable<CallModel> = {
        return self.callService.currentCallWithID(callID: (self.call?.callId)!)
    }()
    ...
    lazy var callButtonState: Observable<UIImage> = {
        let onImage = UIImage(asset: Asset.unpauseCall)
    ...
    lazy var callPaused: Observable<Bool> = {
        return self.callService.currentCall.filter({ call in
            ...
            if  call.state == .hold || (call.state == .current && call.peerHolding) {
                return true
            }
            return false
        })
