...
class CallViewModel: Stateable, ViewModel {
    ...
    let menuItemsManager = ConferenceMenuItemsManager()
    ...
    func getConferenceParticipants() -> [ConferenceParticipant]? {
        ...
        for var participant in participants where participant.uri != nil {
            if let call = self.callService.call(participantHash: participant.uri!, accountID: account.id) {
                participant.displayName = call.getDisplayName()
            }
        }
        return self.callService.getConferenceParticipants(for: self.rendererId)
    }
}
