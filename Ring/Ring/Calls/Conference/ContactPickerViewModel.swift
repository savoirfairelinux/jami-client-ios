...
final class ContactPickerViewModel: ObservableObject, ViewModel {
    ...
    /// Unfiltered sections from the Rx data source (forCall mode only).
    private var unfilteredContactSections: [ContactPickerSection] = []
    /// Pre-built avatar providers for conversation items, keyed by SwarmInfo.id.
    ...
    /// Call after setting `type` and `currentCallId` to start observing data.
    func bind() {
        switch type {
        case .forCall:
            bindContactSections()
    ...
    private func bindConversationSections() {
        ...
        // Pre-build avatar providers for each conversation item.
...
private extension ContactPickerViewModel {
    ...
    func appendCallSection(from calls: [String: CallModel], to sections: inout [ContactPickerSection]) -> [String] {
        ...
        for (conferenceID, contacts) in conferences {
            // conferences always have at least one contact per entry.
            let avatar = AvatarProvider(
                profileService: profileService,
                size: .medium45,
                avatar: contacts[0].imageData.asObservable(),
                displayName: contacts[0].firstLine.asObservable(),
                isGroup: true
            )
