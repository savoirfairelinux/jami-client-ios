...
class ContainerViewModel: ObservableObject {
    ...
    var pendingInputs = [String: VideoInput]()
    ...
    func addVideoInput(videoInput: VideoInput) {
        if let participant = participants.first(where: { $0.id == videoInput.renderId }) {
            ...
            if participants.isEmpty {
                ...
                pendingInputs[videoInput.renderId] = videoInput
            }
        }
    ...
    func removeVideoInput(renderId: String) {
        if let participant = participants.filter({ $0.id == renderId }).first {
            participant.setVideoInput(videoInput: nil, mode: .resizeAspect)
        }
    ...
    func updateParticipantInfo(info: ConferenceParticipant) -> Bool {
        if let participant = participants.first(where: { $0.id == info.sinkId }) {
            ...
            if participant.videoInput == nil {
                if let videoInput = self.pendingInputs.removeValue(forKey: info.sinkId) {
                    participant.setVideoInput(videoInput: videoInput, mode: .resizeAspectFill)
                }
            }
            return true
        }
    ...
    func addParticipantInfo(info: ConferenceParticipant) {
        let participant = ParticipantViewModel(id: info.sinkId)
        participant.info = info
        if let videoInput = self.pendingInputs.removeValue(forKey: info.sinkId) {
            participant.setVideoInput(videoInput: videoInput, mode: .resizeAspectFill)
        }
    ...
    private func updateParticipants(with participantsInfo: [ConferenceParticipant]) {
        ...
        //        let filtered = participantsInfo.filter { participant in
        //            !participant.sinkId.isEmpty && participant.sinkId != "host_video_0"
        //        }
