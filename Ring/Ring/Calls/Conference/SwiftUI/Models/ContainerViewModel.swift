    ...
    init(localId: String, delegate: PictureInPictureManagerDelegate, injectionBag: InjectionBag, currentCall: Observable<CallModel>, hasVideo: Bool, incoming: Bool, callId: String) {
        ...
        self.callService.currentConferenceEvent
            ...
            .filter({ [weak self] conference -> Bool in
                ...
                return conference.calls.contains(self.callId) ||
                    conference.conferenceID == self.callId
            })
            .subscribe(onNext: { [weak self] conf in
                ...
                if conferenceDestroyed {
                    ...
                    for callId in calls {
                        if let call = self.callService.call(callID: callId), call.state == .current {
                            ...
                            self.updateWith(participantsInfo: [participant], mode: .resizeAspect)
                ...
                self.callId = conf.conferenceID

                if conf.state == ConferenceState.conferenceCreated.rawValue {
                    self.updateWith(participantsInfo: [], mode: .resizeAspectFill)
                }
                if let participants = self.getConferenceParticipants() {
                    self.updateWith(participantsInfo: participants, mode: .resizeAspectFill)
                }
            })
            .disposed(by: self.disposeBag)
        self.callService
            ...
            .subscribe(onNext: { [weak self] call in
                ...
                let pending = PendingConferenceCall(info: confInfo, injectionBag: self.injectionBag)
        ...
        self.conferenceState
            .subscribe(onNext: { [weak self] (state) in
                ...
                case .muteAudio(let info):
                    guard let uri = info.uri else { return }
                    self.conferenceActionsModel.muteParticipant(participantId: uri, active: !info.isAudioMuted, conferenceId: callId, device: info.device, streamId: info.sinkId)
