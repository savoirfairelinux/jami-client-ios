...
class ContainerViewModel: ObservableObject {
    ...
    var mainGridViewModel: MainGridViewModel {
        MainGridViewModel(participantsCount: participants.count, firstParticipant: self.participants.first?.id ?? "")
    }
    ...
    func addVideoInput(videoInput: VideoInput) {
        if let participant = participants.first(where: { $0.id == videoInput.renderId }) {
            if participant.videoInput == nil {
                let mode = participants.count > 1 ? AVLayerVideoGravity.resizeAspectFill : AVLayerVideoGravity.resizeAspect
                participant.setVideoInput(videoInput: videoInput, mode: mode)
            }
        } else {
            let mode = participants.isEmpty ? AVLayerVideoGravity.resizeAspect : AVLayerVideoGravity.resizeAspectFill
            let model = ParticipantViewModel(videoInput: videoInput, mode: mode)
            participants.append(model)
        }
        self.setCallLayout(layout: self.getNewLayout())
    }

    func removeVideoInput(renderId: String) {
        if let participant = participants.filter({ $0.id == renderId }).first {
            participant.videoInput = nil
        }
        self.setCallLayout(layout: self.getNewLayout())
    }
    ...
    func addParticipantInfo(info: ConferenceParticipant) {
        let model = ParticipantViewModel(id: info.sinkId)
    ...
    func conferenceUpdated(participantsInfo: [ConferenceParticipant]) {
        ...
        self.mainGridViewModel.updatedLayout(participantsCount: self.participants.count, firstParticipant: self.participants.first?.id ?? "")
        self.setCallLayout(layout: self.getNewLayout())
    }
    ...
    private func updateParticipants(with participantsInfo: [ConferenceParticipant]) {
        let ids = Set(participantsInfo.map { $0.sinkId })
        participants.removeAll { !ids.contains($0.id) }

        // Update participant info or add new participants
        for info in participantsInfo where !updateParticipantInfo(info: info) {
