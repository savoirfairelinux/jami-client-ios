...
class ConferenceActionsModel {
    ...
    func isLocalHost(info: ConferenceParticipant) -> Bool {
        guard let account = self.accountService.currentAccount else { return false }
        return info.uri == account.jamiId
    }
    ...
    func getItemsForConferenceFor(participant: ConferenceParticipant,
        ...
        let isLocalModerator = self.isLocalModerator(info: participant)
        let isLocalHost = self.isLocalHost(info: host)
        ...
        return menuItemsManager.getMenuItemsFor(call: participantCall, isHost: isParticipantHost, layout: layout, active: active, role: role, isHandRised: isHandRised)
    }
