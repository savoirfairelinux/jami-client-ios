...
class ParticipantViewModel: Identifiable, ObservableObject, Equatable, Hashable {
    ...
    init(info: ConferenceParticipant, injectionBag: InjectionBag, conferenceState: PublishSubject<State>) {
        self.id = info.sinkId
    ...
    func addItem(item: MenuItem) {
        ...
        case .maximize:
            ...
            conferenceActions.append(ButtonInfoWrapper(info: button))
        case .setModerator:
            var button = self.isModerator ? ButtonInfo(background: .clear, stroke: .clear, name: "moderator", action: ParticipantAction.setModerator(info: info)) :
                ButtonInfo(background: .clear, stroke: .clear, name: "moderator", action: ParticipantAction.setModerator(info: info))
            button.isSystem = false
            button.imageColor = self.isModerator ? .white : Color(UIColor.lightGray)
            conferenceActions.append(ButtonInfoWrapper(info: button))
        case .muteAudio:
            ...
            conferenceActions.append(ButtonInfoWrapper(info: button))
        case .hangup:
            var button = ButtonInfo(background: .clear, stroke: .clear, name: "block_symbol", action: ParticipantAction.hangup(info: info))
            button.isSystem = false
            conferenceActions.append(ButtonInfoWrapper(info: button))
        case .lowerHand:
            let button =
    ...
    func stopPendingCall() {
        self.conferenceState.onNext(ParticipantAction.hangupPending(callId: self.id))
    }
}
