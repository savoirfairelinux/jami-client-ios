...
class ParticipantViewModel: Identifiable, ObservableObject, Equatable, Hashable {
    ...
    func addItem(item: MenuItem) {
        ...
        case .maximize:
            ...
            conferenceActions.append(ButtonInfoWrapper(info: button))
        case .setModerator:
            let button = ButtonInfo(background: .clear, stroke: .clear, name: "crown", action: ParticipantAction.setModerator(info: info))
            conferenceActions.append(ButtonInfoWrapper(info: button))
        case .muteAudio:
            let button = ButtonInfo(background: .clear, stroke: .clear, name: "mic", action: ParticipantAction.muteAudio(info: info))
            conferenceActions.append(ButtonInfoWrapper(info: button))
        case .hangup:
            let button = ButtonInfo(background: .clear, stroke: .clear, name: "slash.circle", action: ParticipantAction.hangup(info: info))
            conferenceActions.append(ButtonInfoWrapper(info: button))
        case .lowerHand:
            let button = ButtonInfo(background: .clear, stroke: .clear, name: "hand.raised", action: ParticipantAction.raseHand(info: info))
            conferenceActions.append(ButtonInfoWrapper(info: button))
        }
    }
