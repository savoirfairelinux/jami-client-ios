...
struct CallHeaderAvatar: Equatable, Identifiable {
    let uri: String
    let isPending: Bool
...
struct CallHeaderModel: Equatable {
    ...
    init(call: CallState?,
        ...
        self.isInviting = !pending.isEmpty
        ...
        if self.showsRoster {
            ...
            self.avatars = Self.avatars(rows: rows, pending: pending)
        } else {
    ...
    private static func avatars(rows: [ConferenceParticipantRow],
                                pending: [PendingParticipantRow]) -> [CallHeaderAvatar] {
        let candidates = rows.filter { !$0.isLocal }
            .map { CallHeaderAvatar(uri: $0.uri, isPending: false) }
            + pending.map { CallHeaderAvatar(uri: $0.uri, isPending: true) }
        var seen = Set<String>()
        return Array(candidates.filter { seen.insert($0.uri).inserted }
                        .prefix(maximumAvatars))
    }
