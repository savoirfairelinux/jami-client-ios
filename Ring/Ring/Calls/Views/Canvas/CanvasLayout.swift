...
enum CanvasLayout {
    ...
    struct Input {
        ...
        var tileSpacing: CGFloat { style == .cards ? CanvasLayout.tileSpacing : 0 }
    }
    ...
    private static func gridLayout(remote: [CanvasParticipant], input: Input) -> Plan {
        ...
        default:
            ...
            layout.contentSize = CGSize(width: input.canvasSize.width,
                                        height: max(stackedHeight, input.canvasSize.height))
            layout.scrollEnabled = stackedHeight > input.canvasSize.height
        }
        return layout
    }
