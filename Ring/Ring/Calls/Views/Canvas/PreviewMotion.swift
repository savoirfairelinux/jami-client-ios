...
enum PreviewMotion {
    ...
    struct WallTravel: Equatable {
        ...
        init(from: UIEdgeInsets, to: UIEdgeInsets, duration: TimeInterval) {
            self.from = from
            self.to = to
        ...
        static func settled(_ insets: UIEdgeInsets) -> WallTravel {
            return WallTravel(from: insets, to: insets, duration: 0)
        }
        ...
        var chromePresence: CGFloat {
            ...
            let end: CGFloat = to.top + to.bottom > 0 ? 1 : 0
        ...
        var current: UIEdgeInsets {
            ...
            return UIEdgeInsets(
                ...
                right: from.right + (to.right - from.right) * progress)
        }
