...
class ConferenceParticipantView: UIView {
    ...
    @objc func showMenu() {
        ...
        let stackViewFrame = self.superview?.frame.origin
        let scrollViewframe = self.superview?.superview?.frame.origin
        var origin = menu.frame.origin
        origin.x += viewFrame.x + (stackViewFrame?.x ?? 0) + (scrollViewframe?.x ?? 0)
        origin.y += viewFrame.y + (stackViewFrame?.y ?? 0) + (scrollViewframe?.y ?? 0)
