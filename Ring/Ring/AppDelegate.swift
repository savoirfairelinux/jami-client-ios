...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func cleanTestDataIfNeed() {
        if TestEnvironment.shared.isRunningTest {
            let dispatchGroup = DispatchGroup()
            if TestEnvironment.shared.createFirstAccount,
               let firstAccount = TestEnvironment.shared.firstAccountId {
                dispatchGroup.enter()
                self.accountService.removeAccountAndWaitForCompletion(id: firstAccount)
                    .subscribe(onNext: { _ in
                        dispatchGroup.leave()
                    })
                    .disposed(by: self.disposeBag)
            ...
            if TestEnvironment.shared.createSecondAccount,
               let secondAccount = TestEnvironment.shared.secondAccountId {
                dispatchGroup.enter()
                self.accountService.removeAccountAndWaitForCompletion(id: secondAccount)
                    .subscribe(onNext: { _ in
                        dispatchGroup.leave()
                    })
                    .disposed(by: self.disposeBag)
