...
class MigrateAccountViewModel: Stateable, ViewModel {
    ...
    lazy var cancelable: Observable<Bool> = {
        return Observable.just(self.accountService.hasValidAccount())
    }()

    lazy var showMigrateAnotherAccountButton: Observable<Bool> = {
    ...
    func removeAccount() {
        ...
        // if all accounts need to be migrated present next account
        if !self.accountService.hasValidAccount() {
            migrateAnotherAccount()
            return
        }
        // choose next available account 
        for account in self.accountService.accounts where (account.id != accountToMigrate &&
    ...
    func migrateAnotherAccount() {
        for account in self.accountService.accounts where (account.id != accountToMigrate &&
            account.status == .errorNeedMigration) {
                self.migrationState.accept(AccountMigrationState.finished)
                self.stateSubject.onNext(AppState.needMigrateAccount(accountId: account.id))
                return
        }
    }
}
