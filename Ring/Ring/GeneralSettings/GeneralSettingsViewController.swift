...
class GeneralSettingsViewController: UIViewController, StoryboardBased, ViewModelBased {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        viewModel.togleLimitLocationSharingDuration(enable: self.viewModel.limitLocationSharingDuration.value)
    }
    ...
    func makeLocationSharingDurationCell() -> DisposableCell {
        ...
        //        self.viewModel.locationSharingDuration.value
        textField.keyboardType = .numberPad
        textField.textAlignment = .left
        textField.setContentHuggingPriority(.fittingSizeLevel, for: .horizontal)
        textField.heightAnchor.constraint(equalToConstant: 45).isActive = true
        ...
        viewModel.locationSharingDuration
            .subscribe(onNext: {[weak titleLabel, weak textField, weak self] (_) in
        ...
        cell.selectionStyle = .none
        cell.isUserInteractionEnabled = viewModel.limitLocationSharingDuration.value
        cell.alpha = viewModel.limitLocationSharingDuration.value ? 1 : 0.3
