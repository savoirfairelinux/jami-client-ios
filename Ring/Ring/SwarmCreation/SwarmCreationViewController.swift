...
class SwarmCreationViewController: UIViewController, ViewModelBased, StoryboardBased, UISearchResultsUpdating {
    ...
    override func viewDidLoad() {
        self.navigationItem.title = "Select Contacts"// L10n.Swarmcreation.createTheSwarm
        let backButton = UIBarButtonItem(title: "Cancel", style: .plain, target: self, action: #selector(backButtonTapped))

        self.navigationItem.leftBarButtonItem = backButton

        let continueButton = UIBarButtonItem(title: "Continue", style: .plain, target: self, action: #selector(backButtonTapped))
