...
class ConversationModel: Equatable {
    ...
    func indexOfMessage(_ messageId: String) -> Int? {
    ...
    func insertByParent(_ message: MessageModel) -> Int {
        ...
        if messages.isEmpty || messages.last?.id == message.parentId {
            ...
            relocateOrphanedChild(of: message.id)
            return idx
        }

        // Case 2: this message is a parent of an existing child → insert before earliest child
        let childIndices = messageIndex.values.filter { messages[$0].parentId == message.id }
        if let minIdx = childIndices.min() {
            messages.insert(message, at: minIdx)
            rebuildIndex(from: minIdx)
            return minIdx
        }
    ...
    private func relocateOrphanedChild(of parentId: String) {
        ...
        if let newParentPos = messageIndex[parentId] {
            ...
            relocateOrphanedChild(of: childId)
        }
    }
