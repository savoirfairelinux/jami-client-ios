...
struct ConversationParticipant: Equatable {
    ...
    init (info: [String: String], isLocal: Bool) {
        ...
        if let uri = info["uri"], !uri.isEmpty {
            self.uri = uri
        }
...
class ConversationModel: Equatable {
    ...
    convenience init(withParticipantUri participantUri: JamiURI, accountId: String) {
        self.init()
        self.participants = [ConversationParticipant(uri: participantUri.uriString ?? "")]
    ...
    func updateRequestFromInfo(info: [String: String]) {
        self.isRequest = true
        if let conversationId = info[ConversationAttributes.conversationId.rawValue] {
            self.conversationId = conversationId
        }
        if let from = info[ConversationAttributes.from.rawValue] {
    ...
    static func == (lhs: ConversationModel, rhs: ConversationModel) -> Bool {
        if lhs.conversationType == .nonSwarm && rhs.conversationType == .nonSwarm {
            return (lhs.participants[0] == rhs.participants[0] && lhs.accountId == rhs.accountId)
        }
    ...
    func containsParticipant(participant: String) -> Bool {
        return self.getParticipants().map { participant in
            return participant.uri.replacingOccurrences(of: "ring:", with: "")
        }.contains(participant.replacingOccurrences(of: "ring:", with: ""))
    }
}
