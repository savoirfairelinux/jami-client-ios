...
struct CollaborativeDocument: Equatable {
    ...
    /**
     ...
     `documentsForAccount`. The document's own identifier is stored under `id`;
     the announcing commit has a separate `announcement` identifier.
     */
    init?(fromNative map: [String: String]) {
        guard let id = map["id"], !id.isEmpty else { return nil }
        self.id = id
