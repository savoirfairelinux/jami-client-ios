...
struct TextFieldAlertView: UIViewControllerRepresentable {
    ...
    func updateUIViewController(_ uiViewController: UIViewController, context: UIViewControllerRepresentableContext<TextFieldAlertView>) {
        ...
        if isShowing && uiViewController.presentedViewController == nil {
            alertController.addTextField { textField in
                textField.placeholder = placeholder
                textField.text = text
            }
            
            alertController.addAction(UIAlertAction(title: L10n.Global.cancel, style: .cancel) { _ in
                isShowing = false
                onCancel()
            })
            
            alertController.addAction(UIAlertAction(title: L10n.Global.save, style: .default) { _ in
