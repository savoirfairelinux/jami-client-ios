...
enum ExpandedAvatarMetrics {
    ...
    static func decodePixels(width: CGFloat, height: CGFloat) -> CGFloat {
        let expanded = size(width: width, height: height)
        return max(expanded.width, expanded.height) * UIScreen.main.scale
    }
}
...
struct ExpandableAvatar<BottomOverlay: View>: View {
    ...
    private var expandedPixels: CGFloat {
    ...
    private var expandableImage: some View {
        ...
        .onChange(of: provider.avatar) { _ in
            if isExpanded {
                provider.loadExpandedAvatar(maxPixels: expandedPixels)
            }
        }
    }
