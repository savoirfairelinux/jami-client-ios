...
enum ExpandedAvatarMetrics {
    static let heightFraction: CGFloat = 0.5
    static let infoBackdropStrength: CGFloat = 0.9
    ...
    static func size(width: CGFloat, height: CGFloat) -> CGSize {
        CGSize(width: width, height: min(width, height * heightFraction))
    }

    static var decodePixels: CGFloat {
        let screen = ScreenDimensionsManager.shared
        let expanded = size(width: screen.adaptiveWidth, height: screen.adaptiveHeight)
        return min(max(expanded.width, expanded.height) * UIScreen.main.scale, maxDecodePixels)
    }
}
...
struct ExpandableAvatar<BottomOverlay: View>: View {
    ...
    private var expandedSize: CGSize {
        ExpandedAvatarMetrics.size(width: screen.adaptiveWidth, height: screen.adaptiveHeight)
    }
    ...
    var body: some View {
        if provider.canExpand {
            ...
            AvatarSwiftUIView(source: provider)
        }
    }

    private var expandableImage: some View {
        GeometryReader { proxy in
            image(stretchedBy: overscroll(in: proxy), width: proxy.size.width)
        }
        ...
        .onChange(of: provider.avatar) { _ in
            if isExpanded {
                provider.loadExpandedAvatar(maxPixels: ExpandedAvatarMetrics.decodePixels)
            }
        }
    }

    private func accessiblePicture<Content: View>(_ content: Content) -> some View {
        content
    ...
    private func blurredCopy(width: CGFloat, height: CGFloat) -> some View {
        styledPicture(width: width, height: height)
            .blur(radius: ExpandedAvatarMetrics.blurRadius, opaque: true)
            .mask(
                LinearGradient(
                    gradient: Gradient(stops: [
                        .init(color: .clear, location: 0),
                        .init(color: .clear, location: ExpandedAvatarMetrics.blurStart),
    ...
    private func image(stretchedBy stretch: CGFloat, width: CGFloat) -> some View {
        accessiblePicture(styledPicture(width: width, height: size.height + stretch))
            .overlay {
                if isExpanded {
                    blurredCopy(width: width, height: size.height + stretch)
                }
            }
