...
class Table<T: ManagedObject> {
    ...
    static func create(_ setup: (T) -> Void) {
        ...
        do {
            ...
            let newElem = T(entity: entityDescription!, insertInto: moc)
            setup(newElem)
            try moc.save()
        } catch {
    ...
    static func createIfNotExist(_ predicate: NSPredicate, setup: (T) -> Void) -> Bool {
        ...
        do {
            let fetchedContact = try managedObjectContext.fetch(fetchRequest)
    ...
    // MARK: - Read functions
    static func fetchedResultsController(_ sortDescriptors: [NSSortDescriptor] = []) -> NSFetchedResultsController<NSFetchRequestResult> {

        let moc = CDManager.sharedInstance.managedObjectContext
        let request = NSFetchRequest<NSFetchRequestResult>(entityName: Contact.entityName)
