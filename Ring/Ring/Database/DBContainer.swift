...
final class DBContainer {
    ...
    func isDBExistsInAccountFolder(account: String) -> Bool {
        let path = accountFolderPath(account: account)
    ...
    func needMigrateToDB2(for accountID: String) -> Bool {
        guard let dbase = self.forAccount(account: accountID, setDBVersion: true) else {
            return true
        }
...
//    }
    ...
    func moveDbToAccountFolder(for accountID: String) -> Bool {
        ...
        let fileManager = FileManager.default
        do {
            try fileManager.moveItem(atPath: oldPath.path, toPath: newPath)
