...
class DBManager {
    ...
    func migrateToDbVersion2(accountId: String, accountHash: String, accountType: AccountType) -> Bool {
        ...
        do {
            try newDB.transaction { [weak self] in
                ...
                if try !self.migrateAccountToVCard(for: accountId, accountHash: accountHash, dataBase: newDB, accountType: accountType) {
    ...
    func migrateAccountToVCard(for accountId: String, accountHash: String, dataBase: Connection, accountType: AccountType) throws -> Bool {
        ...
        let uri = JamiURI(schema: type, infoHach: accountHash)
        let accountUri = uri.uriString != nil ? uri.uriString! : accountHash
        ...
        return Completable.create { [unowned self] completable in
            do {
                ...
                try dataBase.transaction {
                    ...
                    self.dbConnections.removeAllContects(accountId: accountId)
                    completable(.completed)
                }
            } catch {
                completable(.error(DBBridgingError.deleteConversationFailed))
    ...
    func getProfile(for profileUri: String, createIfNotExists: Bool, accounId: String) throws -> Profile? {
        guard let profilePath = self.dbConnections
            .contactProfilePath(accountId: accounId,
                                profileURI: profileUri,
                                createifNotExists: createIfNotExists) else {return nil}
        if self.dbConnections
            .contactProfileExists(accountId: accounId,
