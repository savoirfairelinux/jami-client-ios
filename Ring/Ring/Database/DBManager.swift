        ...
        for conversationID in conversations.map({$0.id}) {
            ...
            for interaction in interactions {
                ...
                if let message = self.convertToMessage(interaction: interaction, author: author) {
                    ...
                    let isLater = conversationModel.lastDisplayedMessage.id == -1 || conversationModel.lastDisplayedMessage.timestamp < message.receivedDate
                    if displayedMessage && isLater {
                        conversationModel.lastDisplayedMessage = (message.messageId, message.receivedDate)
                    }
                }
            }
