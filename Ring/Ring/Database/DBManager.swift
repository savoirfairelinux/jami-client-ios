...
class DBManager {
    ...
    func accountVCard(for accountId: String) -> Profile? {
        ...
        return CNContactVCardSerialization.parseToProfile(data: data)
    }
    ...
    private func getProfileFromPath(path: String) -> Profile? {
        guard let data = FileManager.default.contents(atPath: path),
              let profile = CNContactVCardSerialization.parseToProfile(data: data) else {
            return nil
        }
        return profile
    }
    ...
    private func saveProfile(profile: Profile, path: String) throws {
        ...
        let data = try CNContactVCardSerialization.dataWithImageAndUUID(from: profile, andImageCompression: 40000)
