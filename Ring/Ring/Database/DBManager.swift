...
class DBManager {
    ...
    func conversationParticipants(accountId: String) -> [String] {
        guard let dataBase = self.dbConnections.forAccount(account: accountId),
              let conversations = try? self.conversationHelper.selectAll(dataBase: dataBase) else {
            return []
        }
    ...
    func contactProfile(for profileUri: String, accountId: String) -> Profile? {
        guard let path = dbConnections.contactProfilePath(accountId: accountId,
                                                          profileURI: profileUri) else { return nil }
        return getProfileFromPath(path: path)
    }

    func legacyContactProfile(for profileUri: String, accountId: String) -> Profile? {
        guard let path = dbConnections.legacyContactProfilePath(accountId: accountId,
                                                                profileURI: profileUri) else { return nil }
        return getProfileFromPath(path: path)
    }

    func placeholderProfile(for profileUri: String, accountId: String) -> Profile? {
        dbConnections.placeholderProfile(accountId: accountId, profileURI: profileUri)
    }

    @discardableResult
    func savePlaceholderProfile(_ profile: Profile, accountId: String) -> Bool {
        dbConnections.savePlaceholderProfile(profile, accountId: accountId)
    }

    func removePlaceholderProfile(for profileUri: String, accountId: String) {
        dbConnections.removePlaceholderProfile(accountId: accountId, profileURI: profileUri)
    }
