...
static id <ConversationsAdapterDelegate> _conversationsDelegate;
...
- (void)registerConfigurationHandler {
    ...
    confHandlers.insert(exportable_callback<ConversationSignal::ConversationMemberEvent>([&](const std::string& accountId, const std::string& conversationId, const std::string& memberUri, int event) {
        if (ConversationsAdapter.conversationsDelegate) {
...
- (uint32_t)countInteractions:(NSString*)accountId conversationId:(NSString*)conversationId from:(NSString*)messageFrom to:(NSString*)messsageTo {
    return countInteractions(std::string([accountId UTF8String]), std::string([conversationId UTF8String]), std::string([messageFrom UTF8String]), std::string([messsageTo UTF8String]));
}
...
#pragma mark ConversationsAdapterDelegate
+ (id <ConversationsAdapterDelegate>)conversationsDelegate {
    return _conversationsDelegate;
}

+ (void) setConversationsDelegate:(id<ConversationsAdapterDelegate>)delegate {
    _conversationsDelegate = delegate;
}
