    ...
    void bindSinkFunctions() {
        ...
        target.push = [this](DRing::SinkTarget::FrameBufferPtr buf) {
            ...
            if(VideoAdapter.delegate) {
                ...
                // assuming R8G8B8A8
                NSInteger depth = sizeof(daemonFramePtr_->ptr) << 2;
                NSInteger bufferLength = (width * height * depth);
                NSLog(@"push: w:%ld, h:%ld, depth:%ld", (long)width, (long)height, (long)depth);
                NSData* data = [NSData dataWithBytesNoCopy:daemonFramePtr_->ptr length:bufferLength freeWhenDone:false];
                CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
                CGContextRef bitmapContext = CGBitmapContextCreate((void *)data, width, height, 8, 4*width, colorSpace,  kCGImageAlphaPremultipliedLast | kCGBitmapByteOrderDefault);
                ...
                @autoreleasepool {
                    ...
                    isRendering = true;
                    [VideoAdapter.delegate writeFrameWithWidth:width withHeight:height withBuffer:data withImage: newimage];
                    isRendering = false;
                }
            }
