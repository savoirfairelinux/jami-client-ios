...
extension ConversationNavigation where Self: Coordinator, Self: StateableResponsive {
    ...
    func contactList() {
        let contactListViewController = ContactListViewController.instantiate(with: self.injectionBag)
        self.present(viewController: contactListViewController,
                     withStyle: .show,
                     withAnimation: true,
                     withStateable: contactListViewController.viewModel)
    }
