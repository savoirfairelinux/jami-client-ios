...
class LoadingView: UIViewController {
    ...
    var successImage: UIImageView = {
        ...
        imageView.frame = CGRect(origin: CGPoint.zero, size: CGSize(width: 50, height: 50))
        return imageView
    }()
    ...
    func updateLoadingView() {
        self.view.removeSubviews(recursive: true)
        self.addToContainerMessageAndView(viewToAdd: loadingActivityIndicator)
    }

    func showSuccessView() {
        self.view.removeSubviews(recursive: true)
        self.addToContainerMessageAndView(viewToAdd: successImage)
    }

    func addToContainerMessageAndView(viewToAdd: UIView) {
        if message.isEmpty {
            containerView.frame = CGRect(origin: view.center, size: CGSize(width: defaultSize, height: defaultSize))
            containerView.center = view.center
            view.addSubview(containerView)
            containerView.contentView.addSubview(viewToAdd)
            viewToAdd.center = containerView.center
            return
        }
        ...
        containerView.frame = CGRect(origin: view.center, size: CGSize(width: width, height: height))
        containerView.center = view.center
        ...
        let centerY = containerView.contentView.bounds.midY - (viewHeight * 0.5) - margin
