...
struct GroupAvatarMember: Equatable {
    ...
    // Identity comparison (===) avoids costly pixel equality and correctly triggers re-renders on image-object change.
    static func == (lhs: GroupAvatarMember, rhs: GroupAvatarMember) -> Bool {
...
struct GroupAvatarCandidate {
    let member: GroupAvatarMember
    let role: String?
}
...
enum GroupAvatarRenderer {
    ...
    struct Selection {
        let members: [GroupAvatarMember]
        let selectedIndices: [Int]
        let overflowCount: Int
    }

    private static let activeRoles: Set<String> = MemberRoles.active

    static func selectForDisplay(
        from candidates: [GroupAvatarCandidate],
        maxVisible: Int = 3
    ) -> Selection {
        let indexed = candidates.enumerated().filter { _, candidate in
            guard let role = candidate.role else { return true }
            return activeRoles.contains(role)
        }
        guard !indexed.isEmpty else { return Selection(members: [], selectedIndices: [], overflowCount: 0) }

        let maxSlots = indexed.count <= maxVisible ? indexed.count : maxVisible - 1

        let adminEntry = indexed.first { $0.element.role == MemberRoles.admin }
        let others = indexed.filter { $0.offset != adminEntry?.offset }
            .sorted { candidatePriority($0.element) > candidatePriority($1.element) }

        var selected = [(offset: Int, element: GroupAvatarCandidate)]()
        if let adminEntry = adminEntry {
            selected.append(adminEntry)
        }
        selected.append(contentsOf: others.prefix(maxSlots - selected.count))

        let overflowCount = max(indexed.count - selected.count, 0)
        return Selection(
            members: selected.map(\.element.member),
            selectedIndices: selected.map(\.offset),
            overflowCount: overflowCount
        )
    }

    private static func candidatePriority(_ candidate: GroupAvatarCandidate) -> Int {
        if candidate.member.image != nil { return 2 }
        let name = candidate.member.name
        if !name.isEmpty && !isJamiHashId(name) { return 1 }
        return 0
    }
