...
class PreferenceManager {
    ...
    static func registerDonationsDefaults() {
        let defaultStartDate = dateFormatter.date(from: DefaultValues.donationStartDate)!
    ...
    static func setStartDonationDate(_ date: Date) {
    ...
    static func temporarilyDisableDonationCampaign() {
        ...
        if let date7DaysAhead = Calendar.current.date(byAdding: .day, value: 7, to: currentDate) {
            setStartDonationDate(date7DaysAhead)
        }
    }

    static func setStartDateTo1MinuteFromNow() {
        let currentDate = Date()
        if let date1MinuteAhead = Calendar.current.date(byAdding: .minute, value: 1, to: currentDate) {
            setStartDonationDate(date1MinuteAhead)
        }
    }
