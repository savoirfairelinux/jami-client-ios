...
private enum AvatarMetrics {
    ...
    struct LayoutPreset {
        let adminDiameterRatio: CGFloat
        let secondaryDiameterRatio: CGFloat
        // Change sizes and offsets together — they're tuned to overlap.
        let offsets: [(x: CGFloat, y: CGFloat)]
    }

    static let twoCircle = LayoutPreset(
        adminDiameterRatio: 0.52,
        secondaryDiameterRatio: 0.34,
        offsets: [(-0.14, -0.14),
                  ( 0.18,  0.18)]
    )
    static let threeCircle = LayoutPreset(
        adminDiameterRatio: 0.46,
        secondaryDiameterRatio: 0.30,
        offsets: [(-0.14, -0.16),
                  ( 0.22,  0.00),
                  (-0.04,  0.24)]
    )
        ...
        if let imageData = profileImageData, !imageData.isEmpty,
           let image = UIImage(data: imageData) {
            ...
            if !username.isSHA1() && !username.isEmpty && !isGroup {
                ...
                label.font = UIFont.systemFont(ofSize: labelFontSize ?? (size * AvatarMetrics.monogramFontRatio), weight: .semibold)
                label.textColor = .white
                label.textAlignment = .center
                addSubview(label)
            } else {
                let symbolSize = max((size * AvatarMetrics.iconSizeRatio).rounded(), AvatarMetrics.minIconSize)
...
class AvatarProvider: ObservableObject {
    ...
    private func subscribeGroupParticipants(swarmInfo: SwarmInfoProtocol) {
        swarmInfo.avatarData.asObservable()
            ...
            .subscribe(onNext: { [weak self] data in
                self?.hasCustomAvatar = data != nil && !data!.isEmpty
            })
            .disposed(by: disposeBag)
    ...
    func renderGroupSnapshot() {
        ...
        groupAvatarSnapshot = renderer.image { ctx in
            ...
            if count == 1 && overflow == 0 {
                ...
                let adminSize = totalSize * preset.adminDiameterRatio
                let otherSize = totalSize * preset.secondaryDiameterRatio
                let shadowRadius = totalSize * AvatarMetrics.shadowBlurRatio
                let shadowY = totalSize * AvatarMetrics.shadowOffsetYRatio

                let offsets = preset.offsets.map { (x: totalSize * $0.x, y: totalSize * $0.y) }
    ...
    private func drawParticipantCircle(in context: CGContext, participant: ParticipantInfo,
        ...
        if let avatarImage = participant.provider.avatar {
            ...
            if !name.isSHA1() && !name.isEmpty {
                let fontSize = min(max((diameter * AvatarMetrics.monogramFontRatio).rounded(),
                                       AvatarMetrics.minMonogramFontSize),
                                   AvatarMetrics.maxMonogramFontSize)
                ...
                let iconSize = max((diameter * AvatarMetrics.iconSizeRatio).rounded(), AvatarMetrics.minIconSize)
    ...
    private func drawOverflowBadge(in context: CGContext, center: CGPoint, size: CGFloat,
        ...
        let text = "+\(count)"
        let fontSize = size * AvatarMetrics.monogramFontRatio
        let font = UIFont.systemFont(ofSize: fontSize, weight: .semibold)
        let attrs: [NSAttributedString.Key: Any] = [.font: font, .foregroundColor: UIColor.white]
    ...
    private func renderEmptyGroupIcon(totalSize: CGFloat) -> UIImage {
        ...
        return renderer.image { _ in
            ...
            let iconSize = max((totalSize * AvatarMetrics.iconSizeRatio).rounded(), AvatarMetrics.minIconSize)
            let config = UIImage.SymbolConfiguration(pointSize: iconSize, weight: .semibold)
...
struct AvatarSwiftUIView: View {
    ...
    @ViewBuilder private var monogramView: some View {
        ...
        ZStack {
            ...
            if !displayText.isSHA1() && !displayText.isEmpty && !source.isGroup {
                ...
                let iconSize = max((effectiveSize * AvatarMetrics.iconSizeRatio).rounded(), AvatarMetrics.minIconSize)
    ...
    private var monogramFontSize: CGFloat {
        min(max((effectiveSize * AvatarMetrics.monogramFontRatio).rounded(),
                AvatarMetrics.minMonogramFontSize),
            AvatarMetrics.maxMonogramFontSize)
    }
}
