...
final class ConferenceAvatarRelayProvider: AvatarRelayProviding {
    private var idToAvatarRelay: [String: BehaviorRelay<Data?>] = [:]
    private var idToNameRelay: [String: BehaviorRelay<String>] = [:]

    func register(id: String, avatar: BehaviorRelay<Data?>, name: BehaviorRelay<String>) {
        idToAvatarRelay[id] = avatar
        idToNameRelay[id] = name
    }

    func avatarRelay(for jamiId: String) -> BehaviorRelay<Data?> {
        if let relay = idToAvatarRelay[jamiId] {
            return relay
        }
        // Fallback empty relay to satisfy interface; ideally all ids are registered
        let empty = BehaviorRelay<Data?>(value: nil)
        idToAvatarRelay[jamiId] = empty
        return empty
    }

    func nameRelay(for jamiId: String) -> BehaviorRelay<String> {
        if let relay = idToNameRelay[jamiId] {
            return relay
        }
        let empty = BehaviorRelay<String>(value: "")
        idToNameRelay[jamiId] = empty
        return empty
    }
}
