...
- (void)registerConfigurationHandler {
    ...
    confHandlers.insert(exportable_callback<ConfigurationSignal::IncomingAccountMessage>([&](const std::string& account_id, const std::string& from, const std::map<std::string, std::string>& payloads) {
...
- (NSUInteger)sendMessage:(NSDictionary*)message withAccountId:(NSString*)accountId
    ...
    return (NSUInteger) sendAccountTextMessage(std::string([accountId UTF8String]),
                           ...
                           [Utils dictionnaryToMap:message]);
}

- (MessageStatus)statusForMessage:(uint64_t)messageId {
