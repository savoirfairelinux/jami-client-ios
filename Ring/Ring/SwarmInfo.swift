...
class SwarmInfo {
    ...
    var participants = BehaviorRelay(value: [ParticipantData]()) // particiapnts added to swarm
    var contacts = BehaviorRelay(value: [ParticipantData]()) // contacts that could be added to swarm
    ...
    func addContactToParticipantsList(jamiId: String) {
        ...
        if let index = contactsValue.firstIndex(where: { data in
            data.jamiId == jamiId
        }) {
    ...
    func addContactToParticipantsList(participantData: ParticipantData) {
        var contactsValue = contacts.value
        if let index = contactsValue.firstIndex(where: { data in
            data.jamiId == participantData.jamiId
        }) {
            contactsValue.remove(at: index)
            contacts.accept(contactsValue)
        }
        var participantsValue = participants.value
        participantsValue.append(participantData)
        participants.accept(participantsValue)
    }
