...
class CallsService: CallsAdapterDelegate, VCardSender {
    ...
    var createdConferences =
        Set<String>() /// set of created conferences, waiting to calls to be attached
    ...
    let newCall = BehaviorRelay<CallModel>(value: CallModel(
        withCallId: "",
        callDetails: [:],
        withMedia: [[:]]
    ))
    ...
    let currentConferenceEvent: BehaviorRelay<ConferenceUpdates> = .init(value: ConferenceUpdates(
        "",
        "",
        Set<String>()
    ))
    ...
    init(withCallsAdapter callsAdapter: CallsAdapter, dbManager: DBManager) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(refuseUnansweredCall(_:)),
                                               name: NSNotification
                                                .Name(rawValue: NotificationName
                                                        .refuseCallFromNotifications.rawValue),
                                               object: nil)
        calls.asObservable()
            .subscribe(onNext: { calls in
                if calls.isEmpty {
                    NotificationCenter.default.post(
                        name: NSNotification
                            .Name(NotificationName.restoreDefaultVideoDevice.rawValue),
                        object: nil,
                        userInfo: nil
                    )
                }
    ...
    func refuseUnansweredCall(_ notification: NSNotification) {
        guard let callId = notification
                .userInfo?[Constants.NotificationUserInfoKeys.callID.rawValue] as? String
        else {
            return
        }
        guard let call = call(callID: callId) else {
            return
        }

        if call.state == .incoming {
            refuse(callId: callId)
                .subscribe { _ in
                    print("Call ignored")
                }
                .disposed(by: disposeBag)
    ...
    func getVideoCodec(call: CallModel) -> String? {
        let callDetails = callsAdapter.callDetails(
    ...
    func accept(call: CallModel?) -> Completable {
        return Completable.create(subscribe: { completable in
            ...
            let success = self.callsAdapter.acceptCall(
                withId: callId,
                accountId: call?.accountId,
    ...
    func joinConference(confID: String, callID: String) {
        guard let secondConf = call(callID: callID) else { return }
        guard let firstConf = call(callID: confID) else { return }
        if let pending = pendingConferences[confID], !pending.isEmpty {
            pendingConferences[confID]!.insert(callID)
        } else {
            pendingConferences[confID] = [callID]
        }
        if secondConf.participantsCallId.count == 1 {
            callsAdapter.joinConference(
                confID,
                call: callID,
                accountId: firstConf.accountId,
                account2Id: secondConf.accountId
            )
        } else {
            callsAdapter.joinConferences(
                confID,
                secondConference: callID,
                accountId: firstConf.accountId,
                account2Id: secondConf.accountId
            )
        }
    ...
    func joinCall(firstCallId: String, secondCallId: String) {
        guard let firstCall = call(callID: firstCallId) else { return }
        guard let secondCall = call(callID: secondCallId) else { return }
        if let pending = pendingConferences[firstCallId], !pending.isEmpty {
            pendingConferences[firstCallId]!.insert(secondCallId)
        } else {
            pendingConferences[firstCallId] = [secondCallId]
        }
        callsAdapter.joinCall(
            firstCallId,
            second: secondCallId,
            accountId: firstCall.accountId,
            account2Id: secondCall.accountId
        )
    }

    func isParticipant(participantURI: String?, activeIn conferenceId: String,
                       accountId: String) -> Bool? {
        guard let uri = participantURI,
              let participantsArray = callsAdapter.getConferenceInfo(
                conferenceId,
                accountId: accountId
              ) as? [[String: String]] else { return nil }
        let participants = arrayToConferenceParticipants(
            participants: participantsArray,
            onlyURIAndActive: true
        )
        for participant in participants
        where participant.uri?.filterOutHost() == uri.filterOutHost() {
            return participant.isActive
        }
        return nil
    }

    private func arrayToConferenceParticipants(
        participants: [[String: String]],
        onlyURIAndActive: Bool
    ) -> [ConferenceParticipant] {
        var conferenceParticipants = [ConferenceParticipant]()
        for participant in participants {
            conferenceParticipants.append(ConferenceParticipant(
                info: participant,
                onlyURIAndActive: onlyURIAndActive
            ))
        }
    ...
    func conferenceInfoUpdated(conference conferenceID: String, info: [[String: String]]) {
        let participants = arrayToConferenceParticipants(
            participants: info,
            onlyURIAndActive: false
        )
        conferenceInfos[conferenceID] = participants
        currentConferenceEvent.accept(ConferenceUpdates(
            conferenceID,
            ConferenceState.infoUpdated.rawValue,
            [""]
        ))
    }

    func isModerator(participantId: String, inConference confId: String) -> Bool {
        let participants = conferenceInfos[confId]
        let participant = participants?.filter { confParticipant in
            confParticipant.uri?.filterOutHost() == participantId.filterOutHost()
        }.first
    ...
    func setActiveParticipant(conferenceId: String, maximixe: Bool, jamiId: String) {
        guard let conference = call(callID: conferenceId),
              let isActive = isParticipant(
                participantURI: jamiId,
                activeIn: conferenceId,
                accountId: conference.accountId
              ) else { return }
        let newLayout = isActive ? getNewLayoutForActiveParticipant(
            currentLayout: conference.layout,
            maximixe: maximixe
        ) : .oneWithSmal
        conference.layout = newLayout
        callsAdapter.setActiveParticipant(
            jamiId,
            forConference: conferenceId,
            accountId: conference.accountId
        )
        callsAdapter.setConferenceLayout(
            newLayout.rawValue,
            forConference: conferenceId,
            accountId: conference.accountId
        )
    ...
    private func getNewLayoutForActiveParticipant(currentLayout: CallLayout,
                                                  maximixe: Bool) -> CallLayout {
    ...
    func callAndAddParticipant(participant contactId: String,
        ...
        placeCall
            ...
            .disposed(by: disposeBag)
        placeCall.connect().disposed(by: disposeBag)
        return placeCall
    }
    ...
    func stopCall(call: CallModel) {
        callsAdapter.hangUpCall(call.callId, accountId: call.accountId)
    }

    func stopPendingCall(callId: String) {
        guard let call = call(callID: callId) else { return }
        stopCall(call: call)
    }

    func answerCall(call: CallModel) -> Bool {
        NSLog("call service answerCall %@", call.callId)
        return callsAdapter.acceptCall(
            withId: call.callId,
            accountId: call.accountId,
            withMedia: call.mediaList
        )
    ...
    func placeCall(withAccount account: AccountModel,
        ...
        if mediaList.isEmpty {
            ...
            mediaAttribute[MediaAttributeKey.mediaType.rawValue] = MediaAttributeValue.audio
                .rawValue
            ...
            if !isAudioOnly {
                mediaAttribute[MediaAttributeKey.mediaType.rawValue] = MediaAttributeValue.video
                    .rawValue
        ...
        return Single<CallModel>.create(subscribe: { [weak self] single in
            if let self = self, let callId = self.callsAdapter.placeCall(withAccountId: account.id,
                                                                         ...
                                                                         withMedia: mediaList),
               !callId.isEmpty,
               let callDictionary = self.callsAdapter.callDetails(
                withCallId: callId,
    ...
    func sendVCard(callID: String, accountID: String) {
        ...
        guard let profile = dbManager.accountVCard(for: accountID) else { return }
    ...
    func updateCallUUID(callId: String, callUUID: String) {
        if let call = call(callID: callId), let uuid = UUID(uuidString: callUUID) {
            call.callUUID = uuid
        }
    }
    ...
    func didChangeCallState(
        withCallId callId: String,
        state: String,
        accountId: String,
        stateCode _: NSInteger
    ) {
        if let callDictionary = callsAdapter.callDetails(withCallId: callId, accountId: accountId) {
            // Add or update new call
            var call = calls.value[callId]
            ...
            if call?.state == .over || call?.state == .failure {
                ...
                currentCallsEvents.onNext(finishedCall)
                var values = calls.value
                values[callId] = nil
                calls.accept(values)
                // clear pending conferences if need
                if pendingConferences.keys.contains(callId) {
                    pendingConferences[callId] = nil
                }
                if let confId = shouldCallBeAddedToConference(callId: callId),
                   var pendingCalls = pendingConferences[confId],
                   let index = pendingCalls.firstIndex(of: callId) {
                    pendingCalls.remove(at: index)
                    if pendingCalls.isEmpty {
                        pendingConferences[confId] = nil
                    } else {
                        pendingConferences[confId] = pendingCalls
                    }
                }
                updateConferences(callId: callId)
                return
            }
            ...
            if (newCall.state == .ringing && newCall.callType == .outgoing) ||
                ...
                sendVCard(callID: callId, accountID: newCall.accountId)
            }
                    ...
                    if let pendingCall = self.call(callID: confId) {
                        DispatchQueue.main.asyncAfter(deadline: .now() + seconds) {
                            if pendingCall.participantsCallId.count == 1 {
                                self.callsAdapter.joinCall(
                                    confId,
                                    second: callId,
                                    accountId: pendingCall.accountId,
                                    account2Id: accountId
                                )
                            } else {
                                self.callsAdapter.joinConference(
                                    confId,
                                    call: callId,
                                    accountId: pendingCall.accountId,
                                    account2Id: accountId
            ...
            currentCallsEvents.onNext(newCall)
        }
    }

    func didChangeMediaNegotiationStatus(
        withCallId callId: String,
        event _: String,
        withMedia: [[String: String]]
    ) {
        guard let call = calls.value[callId],
              let callDictionary = callsAdapter.callDetails(
                withCallId: callId,
                accountId: call.accountId
              ) else { return }
    ...
    func didReceiveMediaChangeRequest(
        ...
        for media in withMedia {
            ...
            if index >= 0 {
                ...
                answerMedia[MediaAttributeKey.muted.rawValue] = call
                    .mediaList[index][MediaAttributeKey.muted.rawValue]
                answerMedia[MediaAttributeKey.enabled.rawValue] = call
                    .mediaList[index][MediaAttributeKey.enabled.rawValue]
        ...
        callsAdapter.answerMediaChangeResquest(
            callId,
            accountId: accountId,
    ...
    func shouldCallBeAddedToConference(callId: String) -> String? {
        var confId: String?
        pendingConferences.keys.forEach { [weak self] initialCall in
            guard let self = self, let pendigs = self.pendingConferences[initialCall],
                  !pendigs.isEmpty
            else { return }
    ...
    func isCurrentCall() -> Bool {
        for call in calls.value.values {
    ...
    func callPlacedOnHold(withCallId callId: String, holding: Bool) {
        guard let call = calls.value[callId] else {
            return
        }
    ...
    func callMediaUpdated(call: CallModel) {
        ...
        if mediaList.isEmpty {
            guard let attributes = callsAdapter.currentMediaList(
                withCallId: call.callId,
                accountId: call.accountId
            ) else { return }
        ...
        if let callDictionary = callsAdapter.callDetails(
            withCallId: call.callId,
            accountId: call.accountId
        ) {
            call.update(withDictionary: callDictionary, withMedia: mediaList)
    ...
    func conferenceCreated(conference conferenceID: String, accountId: String) {
        let conferenceCalls = Set(callsAdapter
        ...
        pendingConferences.forEach { pending in
            ...
            // replace callID by new Conference ID, and remove calls that was already added to
            // onference
            ...
            guard var callDetails = self.callsAdapter.getConferenceDetails(
                conferenceID,
                accountId: accountId
            ) else { return }
            callDetails[CallDetailKey.accountIdKey.rawValue] = self.call(callID: callId)?.accountId
            callDetails[CallDetailKey.audioOnlyKey.rawValue] = self.call(callID: callId)?
                .isAudioOnly.toString()
            let mediaList = [[String: String]]()
            let conf = CallModel(
                withCallId: conferenceID,
                callDetails: callDetails,
                withMedia: mediaList
            )
            ...
            currentConferenceEvent.accept(ConferenceUpdates(
                conferenceID,
                ConferenceState.conferenceCreated.rawValue,
                conferenceCalls
            ))
        }
    }

    func conferenceChanged(conference conferenceID: String, accountId: String, state _: String) {
        if createdConferences.contains(conferenceID) {
            // a conference was created but calls was not attached to a conference. In this case a
            // conference should be added first.
            conferenceCreated(conference: conferenceID, accountId: accountId)
            return
        }
    ...
    func conferenceRemoved(conference conferenceID: String) {
        ...
        currentConferenceEvent.accept(ConferenceUpdates(
            conferenceID,
            ConferenceState.conferenceDestroyed.rawValue,
            conference.participantsCallId
        ))
        var values = calls.value
    ...
    func updateConferences(callId: String) {
        let conferences = calls.value.keys.filter { callID -> Bool in
            guard let callModel = self.calls.value[callID] else { return false }
            return callModel.participantsCallId.count > 1 && callModel.participantsCallId
                .contains(callId)
        }

        guard let conferenceID = conferences.first,
              let conference = call(callID: conferenceID) else { return }
        let conferenceCalls = Set(callsAdapter
                                    .getConferenceCalls(
                                        conferenceID,
                                        accountId: conference.accountId
        ...
        for callID in conferenceCalls {
            call(callID: callID)?.participantsCallId = conferenceCalls
        }
    }
    ...
    func setModeratorParticipant(confId: String, participantId: String, active: Bool) {
        ...
        callsAdapter.setConferenceModerator(
            participantId,
            forConference: confId,
            accountId: conference.accountId,
            active: active
        )
    }
    ...
    func hangupParticipant(confId: String, participantId: String, device: String) {
        guard let conference = call(callID: confId) else { return }
        callsAdapter.hangupConferenceParticipant(
            participantId,
            forConference: confId,
            accountId: conference.accountId,
            deviceId: device
