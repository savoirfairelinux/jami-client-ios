...
class CallsService: CallsAdapterDelegate {
    ...
    func requestMediaChange(call callId: String, mediaLabel: String) {
        ...
        for index in 0..<medias where mediaList[index][MediaAttributeKey.LABEL.rawValue] == mediaLabel {
            mediaList[index][MediaAttributeKey.ENABLED.rawValue] = MediaAttributeValue.TRUE.rawValue
            let muted = mediaList[index][MediaAttributeKey.MUTED.rawValue]
            mediaList[index][MediaAttributeKey.MUTED.rawValue] = muted == MediaAttributeValue.TRUE.rawValue ? MediaAttributeValue.FALSE.rawValue : MediaAttributeValue.TRUE.rawValue
            found = true
            break
        }
        if !found && mediaLabel == "video_0" {
            ...
            media[MediaAttributeKey.ENABLED.rawValue] = MediaAttributeValue.TRUE.rawValue
            media[MediaAttributeKey.MUTED.rawValue] = MediaAttributeValue.FALSE.rawValue
        ...
        if call != nil {
            ...
            for media in withMedia {
                ...
                if index >= 0 {
                    ...
                    var answerMedia = media
                    answerMedia[MediaAttributeKey.MUTED.rawValue] = MediaAttributeValue.TRUE.rawValue
                    answerMedia[MediaAttributeKey.ENABLED.rawValue] = MediaAttributeValue.TRUE.rawValue
                    answerMedias.append(answerMedia)
                }
            }
