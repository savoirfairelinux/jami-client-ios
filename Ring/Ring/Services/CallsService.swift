...
class CallsService: CallsAdapterDelegate, VCardSender {
    ...
    func requestMediaChange(call callId: String, mediaLabel: String, source: String) {
        ...
        for index in 0..<medias where mediaList[index][MediaAttributeKey.label.rawValue] == mediaLabel {
            ...
            let device = source.contains("camera://") ? source : "camera://" + source
            mediaList[index][MediaAttributeKey.source.rawValue] = muted == "true" ? device : source.replacingOccurrences(of: "camera://", with: "")
