...
class CallsService: CallsAdapterDelegate {
    ...
    fileprivate let results: Results<CallModel>

    let history: Observable<Results<CallModel>>
    ...
    init(withCallsAdapter callsAdapter: CallsAdapter) {
        ...
        self.realm = realm
        self.results = realm.objects(CallModel.self)

        history = Observable.collection(from: results, synchronousStart: true)
    ...
    func placeCall(withAccount account: AccountModel, toRingId ringId: String) -> Single<CallModel> {
        ...
        let call = CallModel(withCallId: ringId, valuesFromDictionary: [String: String]())
        ...
        if let callDictionary = self.callsAdapter.callDetails(withCallId: callId) {
            ...
            if call?.state == .over {
                ...
                do {
                    ...
                    self.log.error("Unable to save the call into the history: \(error)")
                }
