        ...
        return Observable.create { [weak self] observable in
            ...
            if success {
                if let request = self.requests.value.filter({ $0.participants.first?.jamiId == jamiId && $0.accountId == accountId
                    ...
                    self.createProfile(with: "ring:" + jamiId, alias: request.name, photo: request.avatar.base64EncodedString(), accountId: request.accountId)
                }
...
extension RequestsService: RequestsAdapterDelegate {
    /**
       incomingTrustRequestReceived signal emmited in 2 cases:
     - for a newly received contact request
     - at the beggining of application for all unhandeled contact requests
     */
    ...
    /**
       conversationRequestReceived signal emmited in 3 cases:
     - for a newly  received conversation request
     - at the beggining of application for all unhandeled conversation requests
