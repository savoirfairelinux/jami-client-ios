...
class RequestsService {
    ...
    func loadRequests(withAccount accountId: String, accountURI: String) {
        ...
        profilesService.clearCachedPlaceholderProfiles(accountId: accountId)
        ...
        if let contactRequests = trustRequestsDictionaries?.map({ dictionary in
            ...
            contactRequests.forEach(cachePlaceholderProfile)
            ...
            for contactRequest in contactRequests where contactRequest.conversationId.isEmpty {
                ...
                if !currentRequests.filter({ $0.participants == contactRequest.participants && $0.accountId == accountId && $0.isCoredialog() }).isEmpty {
                    continue
                }
    ...
    private func conversationReady(_ notification: NSNotification) {
        ...
        if let index = self.requests.value.firstIndex(where: { request in
            ...
            persistPlaceholderProfile(for: request)
    ...
    private func getRequest(withJamiId participantId: String, accountId: String) -> RequestModel? {
        guard let request = self.requests.value.filter({ $0.participants.first?.jamiId == participantId && $0.accountId == accountId && $0.isCoredialog() }).first else {
            return nil
        }
        return request
    }
    ...
    private func cachePlaceholderProfile(_ request: RequestModel) {
        guard let uri = profileURI(for: request) else { return }
        profilesService.cachePlaceholderProfile(uri: uri,
                                                accountId: request.accountId,
                                                alias: request.name,
                                                photo: request.avatar?.base64EncodedString())
    }

    private func persistPlaceholderProfile(for request: RequestModel?) {
        guard let request, let uri = profileURI(for: request) else { return }
        profilesService.persistPlaceholderProfile(uri: uri, accountId: request.accountId)
    }

    private func removePlaceholderProfile(for request: RequestModel?) {
        guard let request, let uri = profileURI(for: request) else { return }
        profilesService.removePlaceholderProfile(uri: uri, accountId: request.accountId)
    }
    ...
    func acceptContactRequest(jamiId: String, withAccount accountId: String) -> Observable<Void> {
        return Observable.create { [weak self] observable in
            ...
            if success {
                if let request {
                    self.persistPlaceholderProfile(for: request)
    ...
    func discardContactRequest(jamiId: String, withAccount accountId: String) -> Observable<Void> {
        return Observable.create { [weak self] observable in
            ...
            let success = self.requestsAdapter.discardTrustRequest(fromContact: jamiId,
                                                                   withAccountId: accountId)
            if success {
                self.removePlaceholderProfile(for: request)
                self.removeRequest(withJamiId: jamiId, accountId: accountId)
        ...
        return Completable.create { [weak self] completable in
            ...
            do {
                ...
                if let uri = JamiURI(schema: .ring, infoHash: jamiId).uriString {
                    self.profilesService.persistPlaceholderProfile(uri: uri, accountId: accountId)
                }
    ...
    func conversationRemoved(conversationId: String, accountId: String) {
        if let index = self.requests.value.firstIndex(where: { request in
            ...
            var values = self.requests.value
            let request = values.remove(at: index)
            removePlaceholderProfile(for: request)
            self.requests.accept(values)
        }
    }
}
...
extension RequestsService: RequestsAdapterDelegate {
    ...
    func incomingTrustRequestReceived(from jamiId: String, to accountId: String, conversationId: String, withPayload payload: Data, receivedDate: Date) {
        ...
        cachePlaceholderProfile(request)
