...
/**
 ...
 Live editing events fire while a document is open, at the rate the peers type,
 and they arrive on the daemon's own thread. A document update may also arrive
 while the document is closed; its empty payload then only reports that stored
 content changed and should be fetched when needed.
...
@objc protocol CollaborationAdapterDelegate {
    /// `update` is empty when the document changed while it was not open here;
    /// in that case it is a notification to fetch the document, not CRDT bytes.
