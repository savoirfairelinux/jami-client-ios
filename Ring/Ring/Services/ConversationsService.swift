...
extension ConversationsService {
    ...
    // Location saved doesn't actually contain the location
    ...
    func deleteLocationUpdate(incoming: Bool, peerUri: String, to accountId: String, shouldRefreshConversations: Bool) -> Completable {
        return Completable.create(subscribe: { [unowned self] completable in
            self.dbManager.deleteLocationUpdates(incoming: incoming, peerUri: peerUri, to: accountId)
                .subscribeOn(ConcurrentDispatchQueueScheduler(qos: .background))
