...
class ConversationsService {
    ...
    func clearHistory(conversation: ConversationModel, keepConversation: Bool) {
        self.dbManager.clearHistoryBetween(accountUri: conversation.accountUri, accountId:conversation.accountId, and: conversation.recipientRingId, keepConversation: keepConversation)
            .subscribeOn(ConcurrentDispatchQueueScheduler(qos: .background))
            .subscribe(onCompleted: { [weak self] in
