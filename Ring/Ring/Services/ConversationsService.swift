...
class ConversationsService {
        ...
        if let conv = self.getConversationForId(conversationId: conversationId, accountId: accountId) {
            ...
            if let info = messageAdapter.getConversationInfo(forAccount: accountId, conversationId: conversationId) as? [String: String],
               let participantsInfo = messageAdapter.getConversationMembers(accountId, conversationId: conversationId) as? [[String: String]] {
                ...
                self.messageAdapter.loadConversationMessages(accountId, conversationId: conversationId, from: "", size: 2)
            }
        }
        ...
        dbManager.getConversationsObservable(for: accountId)
            ...
            .subscribe(onNext: { [weak self] conversationsModels in
                ...
                if let swarmIds = self?.messageAdapter.getSwarmConversations(forAccount: accountId) as? [String] {
                    for swarmId in swarmIds {
                        self?.messageAdapter.loadConversationMessages(accountId, conversationId: swarmId, from: "", size: 1)
                    }
                }
    ...
    func loadMoreMessages(conversationId: String, accountId: String, from: String) {
        self.messageAdapter.loadConversationMessages(accountId, conversationId: conversationId, from: from, size: 20)
    }
    ...
    func sendSwarmMessage(conversationId: String, accountId: String, message: String, parentId: String) {
        self.messageAdapter.sendSwarmMessage(accountId, conversationId: conversationId, message: message, parentId: parentId)
    }
...
//                .disposed(by: self.disposeBag)
    ...
    func setMessageAsRead(conversation: ConversationModel, messageID: String) {
