...
class ConversationsService {
    ...
    func generateDataTransferMessage(transferId: UInt64,
        ...
        self.log.debug("transfer file size is: \(fileSizeWithUnit)")
    ...
    func transferStatusChanged(_ transferStatus: DataTransferStatus,
        ...
        if let messages: [MessageModel] = conversation?.messages.filter({ (message) -> Bool in
            ...
            if let message = messages.first {
                self.dbManager
                    ...
                    .subscribe(onCompleted: { [unowned self] in
                        self.messagesSemaphore.signal()
                        self.log.info("transferStatusChanged: transfer status updated")
                        // event ?
                    }, onError: { _ in
                        self.messagesSemaphore.signal()
                    })
                ...
                self.log.warning("transferStatusChanged: transfer not found")
                self.messagesSemaphore.signal()
            }
        } else {
            self.messagesSemaphore.signal()
        ...
        log.debug("transferStatusChanged: \(transferStatus.description) for: \(messageId) from: \(account.id) to: \(uri)")
    }
}
