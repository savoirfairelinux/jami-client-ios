...
final class ConversationEventSource: NSObject {
    ...
    private let independentQueue = DispatchQueue(label: "com.jami.ConversationEventSource.independentQueue")
    private let onEvent: (ConversationEvent) -> Void

    init(queue modelQueue: DispatchQueue, onEvent: @escaping (ConversationEvent) -> Void) {
    ...
    private func send(_ event: ConversationEvent) {
        modelQueue.async { [onEvent] in
            onEvent(event)
        }
    }

    private func sendIndependently(_ event: ConversationEvent) {
        independentQueue.async { [onEvent] in
            onEvent(event)
        }
    }
...
extension ConversationEventSource: MessagesAdapterDelegate {
    ...
    func didReceiveMessage(_ message: [String: String], from senderAccount: String,
        ...
        sendIndependently(.incomingAccountMessage(accountId: receiverAccountId, from: senderAccount,
    ...
    func activeCallsChanged(conversationId: String, accountId: String, calls: [[String: String]]) {
        sendIndependently(.activeCallsChanged(accountId: accountId, conversationId: conversationId,
                                              calls: calls))
    }
