...
class AccountsService: AccountAdapterDelegate {
    ...
    func enableProxy(accountID: String, enable: Bool) {
        ...
        self.setProxyServer(accountID: accountID, enable: enable)
    ...
    func proxyEnabled(accountID: String) -> Observable<Bool> {
        let currentProxyState = Observable.just(getCurrentProxyState(accountID: accountID))
        let proxyChanges: Observable<Bool?> = self.sharedResponseStream
            .filter({ event in
        ...
        return Observable.combineLatest(currentProxyState, proxyChanges) { (current, changed) in
