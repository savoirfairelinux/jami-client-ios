...
class AccountsService: AccountAdapterDelegate {
    ...
    fileprivate func loadDatabases() -> Bool {
        for account in accountList {
            if dbManager.needMigrateToDbVersion2(accountId: account.id) {
    ...
    func addSipAccount(userName: String,
        ...
        do {
            ...
            if let sipAccount = newAccount,
                let accountURI = AccountModelHelper.init(withAccount: sipAccount).uri {
                _ = self.dbManager.saveAccountProfile(alias: nil, photo: nil, accountId: account, accountURI: accountURI)
            } else {
                _ = self.dbManager.saveAccountProfile(alias: nil, photo: nil, accountId: account, accountURI: "")
            }
            return true
        } catch {
            return false
