...
class AccountsService: AccountAdapterDelegate {
    ...
    fileprivate func loadDatabases() -> Bool {
        for account in accountList {
            if dbManager.isNeedMigrationToDb2(accountId: account.id) {
                if let jamiId = AccountModelHelper
                    ...
                    if !dbManager.migrateToDb2(accountId: account.id,
                                               jamiId: jamiId, accountType: type) {
                        return dbManager.migrateToDb2(accountId: account.id,
                                                      jamiId: jamiId, accountType: type)
                    }
                } else {
                    return false
