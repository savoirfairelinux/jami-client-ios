...
private enum PeerTunnelEndpoint {
    ...
    static func normalizedWebScheme(_ scheme: String) -> String? {
        let normalized = scheme.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        return webSchemes.contains(normalized) ? normalized : nil
    }

    static func url(scheme: String, localPort: Int) -> URL? {
        guard let normalizedScheme = normalizedWebScheme(scheme),
...
extension PeerServiceInfo {
    var isWebService: Bool {
        PeerTunnelEndpoint.normalizedWebScheme(scheme) != nil
    }
}
