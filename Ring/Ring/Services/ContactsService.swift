...
class ContactsService {
    ...
    fileprivate func loadContactRequest() {
        if let currentAccount = self.currentAccount {
    ...
    func accept(contactRequest: ContactRequestModel) -> Completable {
    ...
    func discard(contactRequest: ContactRequestModel) -> Completable {
        return Completable.create { completable in
            if let currentAccount = self.currentAccount {
                let result = self.contactsAdapter.discardTrustRequest(fromContact: contactRequest.ringId,
                                                                      withAccountId: currentAccount.id)
    ...
    func sendContactRequest(toContactRingId ringId: String, vCard: CNContact) -> Completable {
        return Completable.create { completable in
            if let currentAccount = self.currentAccount {
                do {
    ...
    func removeContact(withRingId ringId: String, ban: Bool) -> Completable {
        return Completable.create(subscribe: { completable in
            if let currentAccount = self.currentAccount {
    ...
    func saveVCard(vCard: CNContact, withName name: String) -> Completable {
        return Completable.create(subscribe: { completable in
            if let directoryURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
                do {
        ...
        if let destinationAccount = self.accounts?.filter({ account in
            ...
            do {
                ...
                if let vCard = vCards.first {
                    ...
                    let contactRequest = ContactRequestModel(withRingId: senderAccount, vCard: vCard, receivedDate: receivedDate)
