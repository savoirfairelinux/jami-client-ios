...
class ContactsService {
    ...
    func isContact(hash: String, accountId: String) -> Bool {
        guard let details = self.contactsAdapter.contactDetails(withURI: hash, accountId: accountId),
              let added = Int64(details["added"] ?? ""),
              let removed = Int64(details["removed"] ?? "") else { return false }
    ...
    func removeAllContacts(for accountId: String) {
        DispatchQueue.global(qos: .background).async {
            ...
            var serviceEvent = ServiceEvent(withEventType: .allContactsRemoved)
