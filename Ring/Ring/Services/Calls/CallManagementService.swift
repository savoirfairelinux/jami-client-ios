...
protocol CallManaging {
    ...
    func updateCall(callId: String, callState: CallState, callDictionary: [String: String], mediaList: [[String: String]]) -> CallModel?
...
class CallManagementService: CallManaging {
        ...
        return Single<CallModel>.create(subscribe: { [weak self] single in
            if let self = self, let callId = self.callsAdapter.placeCall(withAccountId: account.id,
               ...
               let callDictionary = self.callsAdapter.callDetails(withCallId: callId, accountId: account.id) {
                ...
                self.trackCallStarted(call: call)
    ...
    func updateCall(callId: String, callState: CallState, callDictionary: [String: String], mediaList: [[String: String]] = [[String: String]]()) -> CallModel? {
        ...
        if let updatedCall = call {
            if isNewCall && callState.isActive() {
                trackCallStarted(call: updatedCall)
            } else if let prevState = previousState, prevState != callState {
                trackCallStateChanged(call: updatedCall, newState: callState)
            }
    ...
    func removeCall(callId: String, callState: CallState) {
        ...
        trackCallEnded(call: finishedCall, duration: callDuration)
    ...
    /// Tracks and emits a call ended event
    private func trackCallEnded(call: CallModel, duration: Int) {
    ...
    /// Tracks and emits a call started event
    private func trackCallStarted(call: CallModel) {
    ...
    /// Tracks and emits a call state changed event
    private func trackCallStateChanged(call: CallModel, newState: CallState) {
