...
/*
 * CallsService Thread Safety Contract:
 *
 * This service uses ThreadSafeQueueHelper to ensure consistent thread safety across all call services.
 ...
 *
 * These patterns are enforced across all call services to prevent deadlocks
 * and ensure consistent thread safety.
 */
...
class CallsService: CallsAdapterDelegate {
    ...
    func didChangeCallState(withCallId callId: String, state: String, accountId: String, stateCode: NSInteger) {
        ...
        let mediaList = callsAdapter.currentMediaList(withCallId: callId, accountId: accountId) ?? []
        if let call = self.callManagementService.addOrUpdateCall(callId: callId, callState: callState, callDictionary: callDictionary, mediaList: mediaList) {
    ...
    func receivingCall(withAccountId accountId: String, callId: String, fromURI uri: String, withMedia mediaList: [[String: String]]) {
        ...
        guard let call = self.callManagementService.addOrUpdateCall(callId: callId, callState: .incoming, callDictionary: callDictionary, mediaList: mediaList) else { return }
