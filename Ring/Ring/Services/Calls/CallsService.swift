...
class CallsService: CallsAdapterDelegate {
    ...
    func conferenceCreated(conferenceId: String, conversationId: String, accountId: String) {
        ...
        if waitForConference.contains(conversationId) {
            relay.accept((confId: conferenceId, conversationId: conversationId))
        }
        ...
        return Single.create { [weak self] single in
            ...
            let subscription = self.relay
                .subscribe(onNext: { conferences in
                    if uri.contains(conferences.conversationId) {
                        let call = CallModel()
            ...
            // We don't care about the Single<CallModel> result here since it will fail
            // but we want to trigger the call placement process
            let callDisposable = self.callManagementService.placeCall(
                ...
                onSuccess: { _ in 
                    // We don't expect this to succeed with a call model
                    // The actual call will be obtained via conference events
                },
                onFailure: { error in
                    // If the call initiation fails completely, report the error
//                    timeoutWorkItem.cancel()
//                    single(.failure(error))
                }
            )
            
            // Return a disposable that will cancel the timeout, subscription, and call disposable
            return Disposables.create {
                timeoutWorkItem.cancel()
