...
class CallsService: CallsAdapterDelegate, CallPlacingCapable {
    // Service instances
    ...
    func joinConferenceIfNeeded(callId: String, accountId: String) {
        guard let confId = conferenceManagementService.shouldCallBeAddedToConference(callId: callId),
              let pendingCall = callManagementService.call(callId: confId) else {
            return
        }
