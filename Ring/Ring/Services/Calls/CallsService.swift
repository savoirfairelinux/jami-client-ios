...
class CallsService: CallsAdapterDelegate {
    ...
    var waitingSwarmCall: String = ""
    var swarmCallCreated = BehaviorRelay<(confId: String, conversationId: String, accountId: String)>(value: (confId: "", conversationId: "", accountId: ""))
    ...
    func receivingCall(withAccountId accountId: String, callId: String, fromURI uri: String, withMedia: [[String: String]]) {
        ...
        _ = self.callManagementService.addOrUpdateCall(callId: callId, callState: .incoming, callDictionary: callDictionary, mediaList: withMedia)
    }
    ...
    func conferenceCreated(conferenceId: String, conversationId: String, accountId: String) {
        if !conversationId.isEmpty && self.waitingSwarmCall.contains(conversationId) {
            ...
            swarmCallCreated.accept((confId: conferenceId, conversationId: conversationId, accountId: accountId))
        }
        ...
        return Single.create { [weak self] single in
            ...
            let subscription = self.swarmCallCreated
                .subscribe(onNext: { conference in
                    if conversationId == conference.conversationId {
                        let call = self.callManagementService.createSwarmCallModel(conference: conference)
