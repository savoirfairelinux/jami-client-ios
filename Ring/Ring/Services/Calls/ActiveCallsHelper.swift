...
struct ConversationCallState {
    ...
    mutating func setCalls(for conversationId: String, to calls: [ActiveCall]) {
...
class ActiveCallsHelper {
    var activeCalls = BehaviorRelay<[String: ConversationCallState]>(value: [:])

    private func updateCalls(for accountId: String, update: (inout ConversationCallState) -> Void) {
        var all = activeCalls.value
        var accountCalls = all[accountId] ?? ConversationCallState()
        update(&accountCalls)
        all[accountId] = accountCalls
        activeCalls.accept(all)
    }

    func updateActiveCalls(conversationId: String, accountId: String, calls: [[String: String]], account: AccountModel) {
        let parsedCalls: [ActiveCall] = calls.compactMap { callDict in
            guard let id = callDict["id"],
                  let uri = callDict["uri"],
                  let device = callDict["device"] else { return nil }

            let currentDeviceId = account.devices.first(where: \.isCurrent)?.deviceId ?? ""
            let isLocal = uri == account.jamiId && currentDeviceId == device
        ...
        updateCalls(for: accountId) { callsState in
            callsState.setCalls(for: conversationId, to: parsedCalls)
        }
    }

    func ignoreCall(_ call: ActiveCall) {
        updateCalls(for: call.accountId) { callsState in
            callsState.ignoreCall(call)
        }
    }

    func answerCall(_ callURI: String) {
        guard let parsed = CallURI(callURI),
              let (accountId, call) = findActiveCall(conversationId: parsed.conversationId, callId: parsed.id)
        else {
            return
        }

        updateCalls(for: accountId) { callsState in
            callsState.answerCall(call)
        }
    }

    private func findActiveCall(conversationId: String, callId: String) -> (accountId: String, call: ActiveCall)? {
        for (accountId, callState) in activeCalls.value {
            if let call = callState.calls(for: conversationId).first(where: { $0.id == callId }) {
