...
class ConferenceManagementService: ConferenceManaging {
    ...
    func handleConferenceCreated(conferenceId: String, conversationId: String, accountId: String) async {
        return await withCheckedContinuation { continuation in
            queueHelper.barrierAsync {
                // Step 1: Check if any calls are attached to the conference
                let conferenceCalls = Set(self.callsAdapter.getConferenceCalls(conferenceId, accountId: accountId))
                
                // Step 2: If no calls yet, register the conference and wait
                ...
                // The conference has calls, so process it fully
                
                // Step 3: Clean up tracking as we're now processing this conference
                var createdConferences = self.createdConferencesRelay.value
                createdConferences.remove(conferenceId)
                self.createdConferencesRelay.accept(createdConferences)

                // Step 4: Get current pending participants and prepare updates
                let pendingConferences = self.pendingConferencesRelay.value
                var updatedPendingConferences = pendingConferences
                
                // Find the call that initiated this conference
                var sourceCallId: String?
                for (callId, pendingSet) in pendingConferences {
                    // Find a matching pending conference where some calls have joined
                    if conferenceCalls.contains(callId) && !conferenceCalls.isDisjoint(with: pendingSet) {
                        sourceCallId = callId
                        
                        // Step 5: Identify which invited participants haven't joined yet
                ...
                // Step 6: Update all participants to show they're in the same conference
                for callId in conferenceCalls {
                ...
                // Step 7: Create a unified conference object that represents the call
                if let sourceId = sourceCallId, let sourceCall = self.calls.value[sourceId] {
                    guard var callDetails = self.callsAdapter.getConferenceDetails(conferenceId, accountId: accountId) else { 
                        // Even without details, still update pending conferences
                ...
                // Step 8: Notify that the conference is ready for all participants
                self.currentConferenceEvent.accept((conferenceId, ConferenceState.conferenceCreated.rawValue, conferenceCalls))
                
                // Update pending participants tracking if needed
