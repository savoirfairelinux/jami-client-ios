...
class ConferenceManagementService {
    ...
    private func endCall(callId: String, call: CallModel, isSwarm: Bool) -> Bool {
        if call.participantsCallId.count >= 2 || isSwarm {
            return callsAdapter.endConference(callId, accountId: call.accountId) ||
    ...
    func endParticipant(confId: String, participantId: String, device: String) {
        guard let conference = calls.get()[confId] else { return }
        callsAdapter.endConferenceParticipant(participantId, forConference: confId, accountId: conference.accountId, deviceId: device)
    }
