...
final class MessageHandlingService: MessageHandling {
    ...
    ///   - callsQueue: The shared dispatch queue for thread-safe call operations (not used in this service)
    init(
        ...
        // We don't need to store the callsQueue as we're only reading from the calls relay
    }
