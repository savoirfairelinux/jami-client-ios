...
public final class DataTransferService: DataTransferAdapterDelegate {
    ...
    func dataTransferEvent(withTransferId transferId: UInt64, withEventCode eventCode: Int) {
        ...
        switch event {
        case .created:
            self.log.debug("dataTransferEvent: transfer created")
            createTransfer(withId: transferId)
        case .closed_by_host, .closed_by_peer:
            self.log.warning("dataTransferEvent: transfer cancelled")
        case .wait_peer_acceptance:
            self.log.debug("dataTransferEvent: transfer waiting peer acceptance")
        case .wait_host_acceptance:
            self.log.debug("dataTransferEvent: transfer waiting host acceptance")
            // accept file test
            acceptTransfer(withId: transferId)
        case .ongoing:
            self.log.debug("dataTransferEvent: transfer ongoing")
        case .finished:
            self.log.debug("dataTransferEvent: transfer finished")
        case .invalid, .invalid_pathname, .unjoinable_peer, .unsupported:
            self.log.error("dataTransferEvent: transfer error")
        }
        // emit service event
        let serviceEventType: ServiceEventType
            = event == .created ? .dataTransferCreated : .dataTransferChanged
