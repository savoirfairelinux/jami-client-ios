...
public final class DataTransferService: DataTransferAdapterDelegate {
    ...
    func dataTransferInfo(
        withId fileId: String,
        accountId: String,
        ...
        err = dataTransferAdapter.dataTransferInfo(withId: fileId, accountId: accountId, with: info)
        if err != .success {
            log.error("DataTransferService: error getting transfer info for id: \(fileId)")
            return nil
        }
        return info
    }

    func getTransferProgress(
        withId transferId: String,
        accountId: String,
        conversationId: String,
        isSwarm _: Bool
    ) -> Int {
        let info = NSDataTransferInfo()
        info.conversationId = conversationId
        _ = dataTransferAdapter.dataTransferInfo(
            withId: transferId,
            accountId: accountId,
            with: info
        )
    ...
    func downloadFile(withId transferId: String,
                      ...
                      fileName _: inout String,
                      accountID: String,
                      conversationID: String) {
        let path = ""
        dataTransferAdapter.downloadSwarmTransfer(
            withFileId: transferId,
            accountId: accountID,
    ...
    func cancelTransfer(withId transferId: String, accountId: String,
                        conversationId: String) -> NSDataTransferError {
        let err = dataTransferAdapter.cancelDataTransfer(
            withId: transferId,
            accountId: accountId,
            conversationId: conversationId
        )
        if err != .success {
            log.error("couldn't cancel transfer with id: \(transferId)")
        }
        return err
    }
    ...
    func sendFile(
        ...
        dataTransferAdapter.sendSwarmFile(withName: displayName,
    ...
    func sendAndSaveFile(displayName: String,
        ...
        if !conversation.isSwarm() {
            fileUrl = getFilePathForTransfer(
                forFile: displayName,
                accountID: conversation.accountId,
                conversationID: conversation.id
            )
        } else {
            fileUrl = createFileUrlForSwarm(
        ...
        do {
            ...
            log.error("couldn't copy image to cache")
        }
        sendFile(
    ...
    private func createFileUrlForDirectory(
        directory: String,
        fileName: String,
        accountID: String,
        conversationID: String
        ...
        if directoryExists && isDirectory.boolValue {
            ...
            var fileExists = FileManager.default.fileExists(
                atPath: filePathCheck.path,
                isDirectory: &isDirectory
            )
            var duplicates = 2
            while fileExists {
                // check
                // /.../AppData/Documents/directory/<fileNameOnly>_<duplicates>.<fileExtensionOnly>
        ...
        do {
            ...
            log.error("DataTransferService: error creating dir")
            return nil
        }
    }

    private func getFilePathForTransfer(
        forFile fileName: String,
        accountID: String,
        conversationID: String
    ) -> URL? {
        return createFileUrlForDirectory(directory: Directories.downloads.rawValue,
                                         fileName: fileName,
                                         accountID: accountID,
                                         conversationID: conversationID)
    }

    func getFilePathForRecordings(
        forFile fileName: String,
        accountID: String,
        conversationID: String,
        isSwarm: Bool
        ...
        if isSwarm {
            return createFileUrlForSwarm(
                fileName: fileName,
                accountID: accountID,
                conversationID: conversationID
            )
        }
        return createFileUrlForDirectory(directory: Directories.recorded.rawValue,
                                         fileName: fileName,
                                         accountID: accountID,
                                         conversationID: conversationID)
    }
    ...
    func createFileUrlForSwarm(fileName: String, accountID: String,
    ...
    func dataTransferEvent(
        ...
        guard let event = NSDataTransferEventCode(rawValue: UInt32(eventCode)) else {
            log.error("DataTransferService: can't get transfer code")
            return
        }

        log.info("DataTransferService: event: \(stringFromEventCode(with: event))")
        ...
        /// do not emit an created event for outgoing transfer, since it already saved in db for non
        /// swarm conversation. For swarm conversation message created when interaction received
        if event == .created && info?.flags != 1 {
            return
        }
        /// we aggregate all non-create type transfer events into the update category
        /// emit service event
        let serviceEventType: ServiceEventType = event == .created ? .dataTransferCreated :
            .dataTransferChanged
