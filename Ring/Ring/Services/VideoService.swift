...
class VideoService {
    ...
    let incomingVideoFrame = PublishSubject<FrameInfo>()
...
extension VideoService: VideoAdapterDelegate {
    ...
    func writeFrame(withWidth width: Int, withHeight height: Int, withImage image: UIImage?) {
        let frameInfo = FrameInfo(width: CGFloat(width), height: CGFloat(height), image: image)
        self.incomingVideoFrame.onNext(frameInfo)
    }
}
