...
class ProfilesService {
    ...
    var profiles = ConcurentDictionary(
        name: "com.contactProfiles",
        dictionary: [String: ReplaySubject<Profile>]()
    )
    var accountProfiles = ConcurentDictionary(
        name: "com.accountProfiles",
        dictionary: [String: ReplaySubject<Profile>]()
    )
    ...
    init(withProfilesAdapter adapter: ProfilesAdapter, dbManager: DBManager) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(messageReceived(_:)),
                                               name: NSNotification
                                                .Name(rawValue: ProfileNotifications
                                                        .messageReceived
                                                        .rawValue),
                                               object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(contactAdded(_:)),
                                               name: NSNotification
                                                .Name(rawValue: ProfileNotifications.contactAdded
                                                        .rawValue),
                                               object: nil)
    }
    ...
    func profileReceived(contact uri: String, withAccountId accountId: String, path: String) {
        ...
        _ = dbManager
            .createOrUpdateRingProfile(profileUri: uriString,
                                       ...
                                       image: profile.photo,
                                       accountId: accountId)
        triggerProfileSignal(uri: uriString, createIfNotexists: false, accountId: accountId)
    }

    @objc
    private func contactAdded(_ notification: NSNotification) {
        guard let ringId = notification
                .userInfo?[ProfileNotificationsKeys.ringID.rawValue] as? String
        else {
            return
        }
        guard let accountId = notification
                .userInfo?[ProfileNotificationsKeys.accountId.rawValue] as? String
        else {
            return
        }
    ...
    private func messageReceived(_ notification: NSNotification) {
        ...
        guard let message = notification
                .userInfo?[ProfileNotificationsKeys.message.rawValue] as? [String: String]
        else {
            return
        }
           ...
           let decoded = vCardKey.removingPercentEncoding {
            guard let regex =
                    try? NSRegularExpression(
                        pattern: "x-ring/ring.profile.vcard;id=([A-z0-9]+),part=([0-9]+),of=([0-9]+)"
                    )
            else {
                return
            }
            ...
            if of == numberOfReceivedChunk {
                buildVCardFromChunks(cardID: id, ringID: ringId, accountId: accountId)
            }
        }
    }

    private func buildVCardFromChunks(cardID: Int, ringID: String, accountId: String) {
        guard let vcard = base64VCards[cardID] else {
            return
        }
    ...
    private func triggerProfileSignal(uri: String, createIfNotexists: Bool, accountId: String) {
        guard let profileObservable = profiles.get(key: uri) as? ReplaySubject<Profile> else {
            return
        }
        dbManager
    ...
    func getProfile(uri: String, createIfNotexists: Bool,
                    accountId: String) -> Observable<Profile> {
        if let profile = profiles.get(key: uri) as? ReplaySubject<Profile> {
            return profile.asObservable().share()
        }
        let profileObservable = ReplaySubject<Profile>.create(bufferSize: 1)
        profiles.set(value: profileObservable, for: uri)
...
extension ProfilesService {
    func getAccountProfile(accountId: String) -> Observable<Profile> {
        if let profile = accountProfiles.get(key: accountId) as? ReplaySubject<Profile> {
            return profile.asObservable().share()
        }
        let profileObservable = ReplaySubject<Profile>.create(bufferSize: 1)
        accountProfiles.set(value: profileObservable, for: accountId)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
    ...
    private func triggerAccountProfileSignal(accountId: String) {
        guard let profileObservable = accountProfiles
                .get(key: accountId) as? ReplaySubject<Profile>
        else {
            return
        }
        dbManager
    ...
    func accountProfileUpdated(accountId: String) {
        triggerAccountProfileSignal(accountId: accountId)
    }
