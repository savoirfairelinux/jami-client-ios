        ...
        self.dataTransferService
            ...
            .subscribe(onNext: { [weak self] event in
                ...
                guard let transferId: String = event.getEventInput(ServiceEventInput.transferId),
                      ...
                      let messageId: String = event.getEventInput(ServiceEventInput.messageId) else {
                    self.log.error("ConversationsManager: can't find entry")
                    return
                }
                     guard let transferInfo = self.dataTransferService.dataTransferInfoNew(withId: transferId, accountId: accountId, conversationId: conversationId),
                    ...
                    self.conversationService
                        ...
                        .subscribe(onCompleted: {
                            guard let transferInfo = self.dataTransferService
                                    .dataTransferInfoNew(withId: transferId, accountId: accountId, conversationId: conversationId) else { return }
                ...
                case .dataTransferChanged:
                    ...
                    let status: DataTransferStatus = transferInfo.bytesProgress == 0 ? .awaiting : transferInfo.bytesProgress == transferInfo.totalSize ? .success : .ongoing
                    switch status {
                    case .canceled:
...
//                       // status = DataTransferStatus.error
                    ...
                    self.conversationService.transferStatusChanged(status, for: transferId, conversationId: conversationId, interactionId: messageId, accountId: accountId, to: "")
                    } else {
                        ...
                        self.conversationService.transferStatusChanged(status, for: transferId, conversationId: conversationId, interactionId: messageId, accountId: accountId, to: transferInfo.peer)
                    }
                default:
                    break
                }
    ...
    func autoAcceptTransfer(transferInfo: NSDataTransferInfo, transferId: String, accountId: String) {
        ...
        if self.dataTransferService.acceptTransfer(withId: transferId, interactionID: messageData.messageID,
                                                   ...
                                                   conversationID: String(messageData.conversationID), name: "") != .success {
...
extension  ConversationsManager: MessagesAdapterDelegate {
    ...
    func conversationLoaded(conversationId: String, accountId: String, messages: [[String: String]]) {
        guard let account = self.accountsService.getAccount(fromAccountId: accountId) else { return }
        let messagesNew = messages.map { dictionary -> MessageModel in
            let newMessage = MessageModel(withInfo: dictionary, accountURI: account.jamiId)
            if newMessage.type == .fileTransfer {
            if let transferInfo = self.dataTransferService.dataTransferInfoNew(withId: newMessage.daemonId, accountId: accountId, conversationId: conversationId) {
            ...
            } else { newMessage.transferStatus = .success }
            }
            return newMessage
        }.filter { messageModel in
            messageModel.type != .merge
        }
        self.conversationService.insertMessages(messages: messagesNew, accountId: accountId, displayed: true, conversationId: conversationId)
    ...
    func newInteraction(conversationId: String, accountId: String, message: [String: String]) {
        guard let account = self.accountsService.getAccount(fromAccountId: accountId) else { return }
        if let fileId = message[MessageAttributes.fileId.rawValue], !fileId.isEmpty {
           // if let transferInfo = self.dataTransferService.dataTransferInfoNew(withId: fileId, accountId: accountId, conversationId: conversationId) {
                let newMessage = MessageModel(withInfo: message, accountURI: account.jamiId)
                newMessage.content = message[MessageAttributes.displayName.rawValue]!
                newMessage.transferStatus = .awaiting
                self.conversationService.insertMessage(message: newMessage, accountId: accountId, displayed: true, conversationId: conversationId, fileId: fileId)
            var filename = ""
            self.dataTransferService.downloadFile(withId: fileId, interactionID: newMessage.messageId, fileName: &filename, accountID: accountId, conversationID: conversationId, name: newMessage.content)
//                if transferInfo.bytesProgress == 0 && newMessage.incoming {
//                    self.autoAcceptTransfer(transferInfo: transferInfo, transferId: fileId, accountId: accountId)
//                }
                return
           // }
        }
