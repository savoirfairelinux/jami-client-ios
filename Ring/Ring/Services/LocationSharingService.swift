...
class LocationSharingService: NSObject {
    ...
    // For notification
    private let stopSharingStream = PublishSubject<ServiceEvent>()
    let stopSharingShared: Observable<ServiceEvent>

    init(dbManager: DBManager) {
        self.dbManager = dbManager

        self.sendLocationStream.disposed(by: self.disposeBag)
        self.sendLocationShared = self.sendLocationStream.share()
        self.deleteLocationStream.disposed(by: self.disposeBag)
        self.deleteLocationShared = self.deleteLocationStream.share()
        self.stopSharingStream.disposed(by: self.disposeBag)
        self.stopSharingShared = self.stopSharingStream.share()
        super.init()
    ...
    private func triggerStopSharing(accountId: String, peerUri: String, content: String) {
        ...
        event.addEventInput(.peerUri, value: peerUri)
        event.addEventInput(.content, value: content)
        self.stopSharingStream.onNext(event)
    }
}
