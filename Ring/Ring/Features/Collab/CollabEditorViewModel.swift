...
extension CollabEditorViewModel {
    ...
    ///
    /// In a directory of its own, so that a second export cannot write over the
    /// copy an application is still reading from the first: the name is the
    /// document's, and two exports of one document would otherwise be one file.
