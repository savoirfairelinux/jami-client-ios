...
extension CollabEditorViewModel {
    ...
    func embed(_ bytes: [String: Data], in text: String, under scheme: String) -> String {
        ...
        for (attachmentId, data) in bytes where !data.isEmpty {
            ...
            written = written.replacingOccurrences(
                of: CollabEditorViewModel.reference(to: attachmentId, under: scheme),
                with: dataURL)
        }
        return written
    }
    ...
    private static func reference(to attachmentId: String, under scheme: String) -> String {
        ...
        return scheme + (escaped ?? attachmentId)
    }
