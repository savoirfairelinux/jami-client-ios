...
class MeCoordinator: Coordinator, StateableResponsive {
    ...
    func start () {
        let meViewModel = MeViewModel(withInjectionBag: self.injectionBag)
        let meViewController = MeViewController.instantiate(with: meViewModel)
        self.present(viewController: meViewController, withStyle: .show, withAnimation: true, withStateable: meViewModel)
    }

    private func showMeDetail () {
        let meDetailViewController = MeDetailViewController.instantiate()
