...
class MeCoordinator: Coordinator, StateableResponsive {
    ...
    required init (with injectionBag: InjectionBag) {
        ...
        self.stateSubject
            ...
            case .needAccountMigration(let accountId):
                self.needMigrateAccount(accountId: accountId)
            }
    ...
    func needMigrateAccount(accountId: String) {
        if let parent = self.parentCoordinator as? AppCoordinator {
            parent.stateSubject.onNext(AppState.needMigrateAccount(accountId: accountId))
        }
    }
