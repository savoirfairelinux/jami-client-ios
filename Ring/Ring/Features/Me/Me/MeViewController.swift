...
class MeViewController: EditProfileViewController, StoryboardBased, ViewModelBased {
        ...
        let configureCell: (TableViewSectionedDataSource, UITableView, IndexPath, SettingsSection.Item)
            -> UITableViewCell = {
                ...
                case .proxy:
                    ...
                    self.viewModel.proxyAddress.asDriver()
                ...
                case .notifications:
                    ...
                    cell.selectionStyle = .none
                    cell.proxyDisabledLabel.text = L10n.Accountpage.noProxyExplanationLabel
                    cell.enableNotificationsLabel.text = L10n.Accountpage.enableNotifications
                    self.viewModel.notificationWarningLabel.asDriver(onErrorJustReturn: true)
                        .drive(cell.proxyDisabledLabel.rx.isHidden)
                        .disposed(by: cell.disposeBag)
                    ...
                    cell.enableNotificationsSwitch.rx.value.skip(1)
                        .observeOn(MainScheduler.instance)
                        .subscribe(onNext: { [weak self] (enable) in
                            self?.viewModel.changeNotificationsAvailibility(enable: enable)
                        }).disposed(by: cell.disposeBag)
                    return cell
                }
        }
    ...
    func askProxyAddressAlert() {
        ...
        let actionConfirm = UIAlertAction(title: L10n.Accountpage.saveProxyAddress, style: .default) { _ in
