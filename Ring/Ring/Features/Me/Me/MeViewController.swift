            ...
            -> UITableViewCell = {
                ...
                case .turnEnabled:
                    ...
                    switchView.rx
                        ...
                        .disposed(by: cell.disposeBag)
                    return cell
                case .turnAddress(let value):
                    let cell = self
                        .configureTurnCell(cellType: .turnAddress(value: value),
                                                     value: value)
                    return cell
    ...
    func  configureSipCredentialsCell(cellType: SettingsSection.SectionRow,
        ...
        case .turnAddress:
            text.rx.text.orEmpty.distinctUntilChanged()
                .bind(to: self.viewModel.turnAddress)
                .disposed(by: cell.disposeBag)
            cell.textLabel?.text = L10n.Account.turnAddress
        case .turnUsername:
            text.rx.text.orEmpty.distinctUntilChanged()
                .bind(to: self.viewModel.turnUsername)
                .disposed(by: cell.disposeBag)
            cell.textLabel?.text = L10n.Account.turnUsername
        case .turnPassword:
            text.rx.text.orEmpty.distinctUntilChanged()
                .bind(to: self.viewModel.turnPassword)
                .disposed(by: cell.disposeBag)
            cell.textLabel?.text = L10n.Account.turnPassword
        case .turnRealm:
            text.rx.text.orEmpty.distinctUntilChanged()
                .bind(to: self.viewModel.turnRealm)
                .disposed(by: cell.disposeBag)
            cell.textLabel?.text = L10n.Account.turnRealm
        default:
            break
        }
        cell.textLabel?.sizeToFit()
