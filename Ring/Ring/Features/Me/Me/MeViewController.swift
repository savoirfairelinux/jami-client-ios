...
class MeViewController: EditProfileViewController, StoryboardBased, ViewModelBased {
    ...
    func confirmBoothModeAlert() {
        let alert = UIAlertController(title: L10n.AccountPage.enableBoothMode,
                                      message: L10n.AccountPage.boothModeAlertMessage,
                                      preferredStyle: .alert)
        let actionCancel = UIAlertAction(title: L10n.Actions.cancelAction,
                                          ...
                                          style: .default) { [weak self] _ in
                                            ...
                                            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                                                if let textFields = alert.textFields,
                                                    ...
                                                    !text.isEmpty {
                                                    let result = self?.viewModel.enableBoothMode(enable: true, password: text)
                                                    if result ?? true {
                                                        self?.stopLoadingView()
                                                        return
                                                    }
