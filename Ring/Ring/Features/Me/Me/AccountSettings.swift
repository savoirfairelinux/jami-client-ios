...
class AccountSettings: ObservableObject {
    ...
    // tls
    @Published var tlsVerifyServer: Bool = false
    @Published var tlsVerifyClient: Bool = false
    @Published var tlsRequireClientCertificate: Bool = false
    @Published var disableSecureDlgCheck: Bool = false
    // turn
...
extension AccountSettings {
    private func setUPSIPParameters() {
        ...
        self.disableSecureDlgCheck = self.getBoolState(for: ConfigKey.disableSecureDlgCheck)
    }
    ...
    func enableVerifyTlsServer(enable: Bool) {
        if self.tlsVerifyServer == enable { return }
        let property = ConfigKeyModel(withKey: ConfigKey.tlsVerifyServer)
        self.accountService.switchAccountPropertyTo(state: enable, accountId: account.id, property: property)
        self.tlsVerifyServer = enable
    }

    func enableVerifyTlsClient(enable: Bool) {
        if self.tlsVerifyClient == enable { return }
        let property = ConfigKeyModel(withKey: ConfigKey.tlsVerifyClient)
        self.accountService.switchAccountPropertyTo(state: enable, accountId: account.id, property: property)
        self.tlsVerifyClient = enable
    }

    func enableTlsClientCertificate(enable: Bool) {
        if self.tlsRequireClientCertificate == enable { return }
        let property = ConfigKeyModel(withKey: ConfigKey.tlsRequireClientCertificate)
        self.accountService.switchAccountPropertyTo(state: enable, accountId: account.id, property: property)
        self.tlsRequireClientCertificate = enable
    }

    func disableSecureDlgCheck(disable: Bool) {
        if self.disableSecureDlgCheck == disable { return }
        let property = ConfigKeyModel(withKey: ConfigKey.disableSecureDlgCheck)
        self.accountService.switchAccountPropertyTo(state: disable, accountId: account.id, property: property)
        self.disableSecureDlgCheck = disable
    }
