...
class MeViewModel: ViewModel, Stateable {
    ...
    func enableProxy(enable: Bool) {
        ...
        let proxyState = self.accountService.getCurrentProxyState(accountID: account.id)

        if proxyState == enable {
            return
        }

        self.accountService.enableProxy(accountID: account.id, enable: enable)

        if !enable {
            if let delegate = UIApplication.shared.delegate as? AppDelegate {
                self.accountService.setCurrentDeviceToken(accountID: account.id)
