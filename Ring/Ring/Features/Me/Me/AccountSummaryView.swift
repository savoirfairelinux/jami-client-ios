...
struct AccountSummaryView: View {
    init(injectionBag: InjectionBag, account: AccountModel, stateSubject: PublishSubject<State>) {
        _model = StateObject(wrappedValue: AccountSummaryVM(injectionBag: injectionBag, account: account, stateSubject: stateSubject))
    }
            ...
            Form {
                Section(header: Text(L10n.AccountPage.profileHeader)) {
                    ...
                    .sheet(isPresented: $showEditPrpofile) {
                        EditProfileView(injectionBag: model.injectionBag, account: model.account, profileImage: model.profileImage, profileName: model.profileName, username: model.username, isPresented: $showEditPrpofile)
                    }
                }
                ...
                Section {
                    ...
                    NavigationLink(destination: LinkedDevicesView(account: model.account, accountService: model.accountService)) {
                        HStack {
                            ...
                            Text(L10n.AccountPage.linkedDevices)
                        }
                    }
