        ...
        List {
            Section(header: Text("ACCOUNT SETTINGS")) {
                NavigationLink(destination: ChatSettingsView()) {
                    SettingsRow(iconName: "message", title: "Chat")
                }
                NavigationLink(destination: CallSettingsView()) {
                    SettingsRow(iconName: "phone", title: "Call")
                }
                NavigationLink(destination: NotificationsSettingsView(model: AccountSettings(account: model.account, injectionBag: model.injectionBag))) {
                    SettingsRow(iconName: "bell", title: "Notifications")
                }
                NavigationLink(destination: ConnectivitySettingsView()) {
