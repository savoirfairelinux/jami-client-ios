...
class ContactRequestsViewModel: Stateable, ViewModel {
    ...
    func accept(withItem item: RequestItem) -> Observable<Void> {
        if item.request.type == .contact || (item.request.isDialog() && self.contactsService.contact(withUri: item.request.participants.first!.jamiId) == nil) {
    ...
    func ban(withItem item: RequestItem) -> Observable<Void> {
        if item.request.type == .contact || (item.request.isDialog() && self.contactsService.contact(withUri: item.request.participants.first!.jamiId) == nil) {
            ...
            let removeCompleted = self.contactsService.removeContact(withId: item.request.participants.first!.jamiId,
                                                                     ban: true,
