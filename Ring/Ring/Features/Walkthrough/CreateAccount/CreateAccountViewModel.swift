...
enum UsernameValidationState {
    ...
    var message: String {
        ...
        case let .available(message):
            return message
        case let .lookingForAvailibility(message):
            return message
...
class CreateAccountViewModel: Stateable, ViewModel {
    ...
    lazy var canAskForAccountCreation: Observable<Bool> = Observable.combineLatest(
        self.usernameValidationState.asObservable(),
        self.username.asObservable(),
        self.createState,
        resultSelector: { (usernameValidationState: UsernameValidationState,
                           username: String,
            ...
            if !username.isEmpty {
                canAsk = canAsk && usernameValidationState
                    .isAvailable && !username.isEmpty
            }
    ...
    func createAccount() {
        ...
        accountService
            .addJamiAccount(username: username,
                            password: password,
                            enable: notificationSwitch.value)
            ...
            .disposed(by: disposeBag)
        enablePushNotifications(enable: notificationSwitch.value)
    }
