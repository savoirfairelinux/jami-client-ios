...
class WalkthroughCoordinator: Coordinator, StateableResponsive {
    ...
    private func showFinalStep (with walkthroughType: WalkthroughType) {
        if walkthroughType == .createAccount {
            ...
            let linkDeviceViewModel = LinkDeviceViewModel(with: self.injectionBag)
            let linkDeviceViewController = LinkDeviceViewController.instantiate(with: linkDeviceViewModel)
            self.present(viewController: linkDeviceViewController, withStyle: .show, withAnimation: true, withStateable: linkDeviceViewModel)
        }
    }
}
