...
struct WelcomeView: View {
            ...
            switch item {
                case .jamiAccount:
                    CreateAccountView(injectionBag: model.injectionBag, dismissAction: {
                        activeView = nil
                    }, createAction: { [weak model] name in
                        activeView = nil
                ...
                case .linkDevice:
                    LinkToAccountView(dismissAction: {
                        ...
                        activeView = nil
                        guard let model = model else { return }
                        self.model.linkDevice(pin: pin, password: password)
                ...
                case .jamsAccount:
                    JamsConnectView(dismissAction: {
                        ...
                        activeView = nil
                        guard let model = model else { return }
                        self.model.connectToAccountManager(userName: username, password: password, server: server)
                    })
                case .sipAccount:
                    SIPConfigurationView(dismissAction: {
                        ...
                        activeView = nil
                        guard let model = model else { return }
                        self.model.createSipAccount(userName: username, password: password, server: server)
                    })
                case .aboutJami:
                    AboutSwiftUIView()
            }
    ...
    func accountCreationErrorAlert(error: AccountCreationError) -> some View {
        CustomAlert(content: { AlertFactory.createAlertViewWithOkButton(title: error.title, message: error.message, action: { [weak model] in
            guard let model = model else { return }
    ...
    func registrationErrorAlert() -> some View {
        CustomAlert(content: { AlertFactory.createAlertViewWithOkButton(title: L10n.CreateAccount.usernameNotRegisteredTitle, message: L10n.CreateAccount.usernameNotRegisteredMessage, action: {[weak model] in
            guard let model = model else { return }
            model.finish() }) })
    }
    ...
    func timeOutAlert() -> some View {
        CustomAlert(content: { AlertFactory.createAlertViewWithOkButton(title: L10n.CreateAccount.timeoutTitle, message: L10n.CreateAccount.timeoutMessage, action: {[weak model] in
            guard let model = model else { return }
            model.finish() }) })
    }
