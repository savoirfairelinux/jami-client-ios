...
class ComposeNewMessageCoordinator: Coordinator, StateableResponsive, ConversationNavigation {
    ...
    func openConversation(conversationId: String, accountId: String, shouldOpenSmarList: Bool) {
        if let parent = self.parentCoordinator as? ConversationsCoordinator {
            parent.stateSubject.onNext(ConversationState.openConversationForConversationId(conversationId: conversationId, accountId: accountId, shouldOpenSmarList: shouldOpenSmarList))
        }
        self.finish()
    }
}
