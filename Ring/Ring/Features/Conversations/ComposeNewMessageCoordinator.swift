...
class ComposeNewMessageCoordinator: Coordinator, StateableResponsive, ConversationNavigation {
            ...
            .subscribe(onNext: { [weak self] (state) in
                ...
                switch state {
                    case .closeComposingMessage:
                        self.finish()
                    case .openConversationForConversationId(let conversationId, let accountId, let shouldOpenSmarList, let withAnimation):
                        self.openConversation(conversationId: conversationId, accountId: accountId, shouldOpenSmarList: shouldOpenSmarList, withAnimation: withAnimation)
                    default:
                        break
                }
            })
    ...
    func openConversation(conversationId: String, accountId: String, shouldOpenSmarList: Bool, withAnimation: Bool) {
        if let parent = self.parentCoordinator as? ConversationsCoordinator {
            parent.stateSubject.onNext(ConversationState.openConversationForConversationId(conversationId: conversationId, accountId: accountId, shouldOpenSmarList: shouldOpenSmarList, withAnimation: withAnimation))
        }
