...
final class ConversationProfileEditorVM: ObservableObject {
    ...
    private var useOriginalContactAvatar = false
    ...
    var canResetContactProfile: Bool {
        ...
        return normalized(name) != normalized(baseContactName ?? "") ||
    ...
    var canRemoveAvatar: Bool {
        ...
        return !useOriginalContactAvatar && (initialLocalPhoto != nil || avatarWasChanged)
    }
    ...
    private func saveSwarm(_ swarmInfo: SwarmInfoProtocol) {
        guard let conversation = swarmInfo.conversation else { return }
        ...
        if avatarWasChanged {
            if useOriginalContactAvatar {
                ...
                guard let image = avatar?.fixOrientation(),
                      let data = image.convertToData(ofMaxSize: 40_000) else {
                    saveFailed = true
                    return
                }
