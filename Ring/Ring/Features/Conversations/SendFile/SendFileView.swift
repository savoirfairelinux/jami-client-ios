...
final class SendFileObservableModel: ObservableObject {
    ...
    private func bindViewModel() {
        ...
        // Drive all published state directly from recordingState
        viewModel.recordingState
            ...
            .subscribe(onNext: { [weak self] state in
                ...
                self.isReadyToSend = (state == .recorded)
                self.isDismissed = (state == .sent)
                self.hideInfo = (state != .initial || !self.viewModel.audioOnly)
                self.showPlayerControls = false  // reset; updated when player becomes ready
    ...
    /// Called when recording is done and viewModel.createPlayer() has been triggered.
    /// We poll briefly since createPlayer() fires after a 1-second delay.
    private func subscribePlayer() {
        playerDisposeBag = DisposeBag()
        // Wait for the player to be created (it is created after a 1-second delay in the VM)
        guard let player = viewModel.player else {
            // Player not yet created — observe playerReady via a short retry
    ...
    private func bindPlayer(_ player: PlayerViewModel) {
