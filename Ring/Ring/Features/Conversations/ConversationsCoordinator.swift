...
public enum ConversationsState: State {
...
class ConversationsCoordinator: Coordinator, StateableResponsive {
    ...
    func start () {
        let smartlistViewModel = SmartlistViewModel(withInjectionBag: self.injectionBag)
        let smartListViewController = SmartlistViewController.instantiate(with: smartlistViewModel)
        self.present(viewController: smartListViewController, withStyle: .show, withAnimation: true, withStateable: smartlistViewModel)
    }

    private func showConversation (withConversationViewModel conversationViewModel: ConversationViewModel) {
        let conversationViewController = ConversationViewController.instantiate(with: conversationViewModel)
