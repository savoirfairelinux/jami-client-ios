...
class ConversationsCoordinator: Coordinator, StateableResponsive, ConversationNavigation {
    ...
    func showIncomingCall(call: CallModel) {
        ...
        if let unhandeledCall = callsProvider.getUnhandeledCall(for: call.participantUri) {
        ...
        callsProvider.sharedResponseStream
            ...
            .subscribe(onNext: { [weak self, weak callViewController] _ in
                guard let self = self,
                      let callViewController = callViewController else { return }
                self.presentCall(controller: callViewController)
