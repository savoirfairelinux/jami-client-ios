...
enum ConversationsState: State {
    ...
    case startCall(contactRingId: String)
}
   ...
   // let accountService: AccountsService
    ...
    required init (with injectionBag: InjectionBag) {
       ...
       // self.conversationsService = injectionBag.conversationsService
        ...
        self.callService.newIncomingCall.asObservable()
//        .filter({ call in
...
//        })
        ...
        self.stateSubject.subscribe(onNext: { [unowned self] (state) in
            ...
            case .startCall(let contactRingId):
                self.startOutgoingCall(contactRingId: contactRingId)
            }
        }).disposed(by: self.disposeBag)
    ...
    private func startOutgoingCall(contactRingId: String) {
    ...
    private func showCallAlert(call: CallModel) {
        ...
        if let controller = self.navigationViewController.presentedViewController?.presentedViewController {
            controller.navigationController?.present(alert, animated: false, completion:nil)
        }
        else {
