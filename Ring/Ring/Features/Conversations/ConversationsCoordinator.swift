...
class ConversationsCoordinator: Coordinator, StateableResponsive, ConversationNavigation {
    ...
    */
    func showIncomingCall(call: CallModel) {
        ...
        let uri = JamiURI.init(schema: type, infoHach: call.participantUri, account: account).hash ?? ""
        let name = call.displayName.isEmpty ? call.registeredName : call.displayName
        if #available(iOS 10.0, *) {
            call.callUUID = UUID()
            callsProvider
                .reportIncomingCall(uuid: call.callUUID,
                                    name: name,
                                    hasVideo: !call.isAudioOnly, uri: uri) { _ in
