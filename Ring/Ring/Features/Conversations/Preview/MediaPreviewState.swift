...
class MediaPreviewPresenter: ObservableObject {
    ...
    /// Dismiss immediately without the collapse-to-thumbnail animation.
    /// Used when the underlying content is being removed (e.g. delete)
    /// so there is no target frame to animate back to.
