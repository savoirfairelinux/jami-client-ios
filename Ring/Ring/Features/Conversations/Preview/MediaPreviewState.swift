...
class MediaPreviewState: ObservableObject {
    ...
    deinit {
        if overlayWindow != nil {
            print("[MediaPreviewState] deallocated without tearDown — cleaning up")
        }
        // Capture references so cleanup can be dispatched to the main
        // thread if deinit fires on a background queue.
