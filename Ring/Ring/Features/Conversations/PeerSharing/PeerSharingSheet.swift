...
struct PeerSharingSheet: View {
    ...
    private func serviceRow(_ service: PeerServiceInfo) -> some View {
        let isPending = viewModel.pendingServices.contains(service.id)

        Button {
            handleTap(service: service, isPending: isPending)
        } label: {
            HStack(spacing: 12) {
                if isPending {
                    ProgressView()
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(rowSubtitle(service: service, isPending: isPending))
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .lineLimit(1)
                }
        ...
        .disabled(isPending)
        .accessibilityLabel(accessibilityText(service: service, isPending: isPending))
    }

    // MARK: - Helpers

    private func rowSubtitle(service: PeerServiceInfo, isPending: Bool) -> String {
        if isPending {
    ...
    private func handleTap(service: PeerServiceInfo, isPending: Bool) {
    ...
    private func copyEndpoint(tunnel: PeerTunnelInfo) {
        guard let endpoint = tunnel.endpointURL?.absoluteString else { return }
    ...
    private func accessibilityText(service: PeerServiceInfo, isPending: Bool) -> String {
        var text = service.name
        if isPending {
            text += ", \(L10n.PeerServices.connecting)"
...
//   AppDelegate.sceneDidEnterBackground closes all tunnels, so the page breaks.
