...
struct PeerSharingSheet: View {
    ...
    // MARK: - Service Row
    //
    // Every row is a web (http/https) service that opens in the in-app viewer. Non-web
    // (raw TCP, e.g. ssh) services are not supported yet and are filtered out in
    // PeerSharingViewModel: the only generic way to use one is to copy its address into
    // another app, but leaving Jami suspends the daemon and the tunnel stops serving, so
    // the address is dead on arrival.
    //
    ...
    private func serviceRow(_ service: PeerServiceInfo) -> some View {
        ...
        .accessibilityHint(L10n.PeerServices.open)
    }

    // MARK: - Helpers

    private func accessibilityText(for service: PeerServiceInfo) -> String {
        service.description.isEmpty ? service.name : "\(service.name), \(service.description)"
    }
}
