...
class PeerSharingViewModel: ObservableObject {
        ...
        queryDisposable = peerSharingService.queryPeerServices(accountId: accountId, peerId: peerId)
            ...
            .subscribe(onNext: { [weak self] result in
                ...
                if result.status == .success {
                    // iOS only consumes web (http/https) services, which open in the in-app
                    // viewer. Non-web (raw TCP) services aren't usable here — the loopback
                    // tunnel dies the moment the app backgrounds to reach another app — so we
                    // don't list them.
                    self.services = result.services.filter { $0.isWebService }
    ...
    private func updateBrowseState() {
        ...
        if activeTunnels[id] != nil {
            return   // active but no browsable URL yet — keep waiting
        }
