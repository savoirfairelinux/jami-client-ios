...
class PeerSharingViewModel: ObservableObject {
    ...
    /// Dismisses the open browse session when its backing tunnel disappears externally
    /// (peer disconnect, daemon close, or app-background `closeAllTunnels`), so the WebView
    /// never lingers on a dead loopback URL.
    ...
    private func presentWebView(for service: PeerServiceInfo) {
        guard let tunnel = activeTunnels[service.id],
              let url = tunnel.browsableURL else { return }
        presentedBrowseSession = PresentedPeerService(service: service, url: url)
        // Intent fulfilled — clear it so a later same-id tunnel can't auto-present.
        pendingPresentationServiceId = nil
    }
