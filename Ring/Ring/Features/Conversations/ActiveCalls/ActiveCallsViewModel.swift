...
class ActiveCallsViewModel: ObservableObject, Stateable {
    ...
    private func updateCallViewModels(from trackersByAccount: [String: AccountCallTracker]) {
        // A swarm shared across several linked accounts mirrors the same
        // remote call into each account's tracker — dedup by full remote
        // identity so the popup shows one row. Iterate the current account
        // first so its accountId is stamped on the surviving row, which is
        // the local account used when the user accepts the call.
        ...
        for accountId in orderedAccountIds {
            ...
            for call in tracker.incomingNotAcceptedNotIgnoredCalls() {
                guard seen.insert(call.remoteIdentity).inserted else { continue }
