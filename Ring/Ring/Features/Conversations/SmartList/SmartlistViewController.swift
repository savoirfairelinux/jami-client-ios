...
class SmartlistViewController: UIViewController, StoryboardBased, ViewModelBased, UISearchControllerDelegate {
    ...
    func searchBarActive() {
        if let customNavBar = self.navigationController?.navigationBar as? CustomNavigationBar {
            ...
            guard let account = self.viewModel.currentAccount else { return }
            let accountSip = account.type == AccountType.sip
            let qrCodeButton = creatSwarchBarButtonWithImage(named: "qrcode")
            qrCodeButton.rx.tap
                .subscribe(onNext: { [weak self] in
                    self?.viewModel.showQRCode()
                })
            ...
            if accountSip {
                let bookButton = creatSwarchBarButtonWithImage(named: "book.circle")
                ...
                bookButton.rx.tap
                    ...
                    .disposed(by: customNavBar.disposeBag)
                let dialpadCodeButton = creatSwarchBarButtonWithImage(named: "square.grid.3x3.topleft.filled")
                dialpadCodeButton.rx.tap
                    .subscribe(onNext: { [weak self] in
                        self?.viewModel.showDialpad()
                    })
                    .disposed(by: customNavBar.disposeBag)
                customNavBar.addCustomSearchView(with: [qrCodeButton, bookButton, dialpadCodeButton])
            } else {
                let swarmButton = creatSwarchBarButtonWithImage(named: "person.3")
                swarmButton.rx.tap
                    .subscribe(onNext: { [weak self] in
                        self?.createGroup()
                    })
    ...
    func creatSwarchBarButtonWithImage(named imageName: String) -> UIButton {
        let button = UIButton()
        let configuration = UIImage.SymbolConfiguration(pointSize: 40, weight: .semibold, scale: .large)
