...
class SmartlistViewModel: Stateable, ViewModel {
    ...
    lazy var accounts: Observable<[AccountItem]> = {
        return self.accountsService.accountsObservable.asObservable().map({ accountsModels in
        ...
        return profileImageForCurrentAccount.share()
            .map({ profile in
                ...
                guard let account = self.accountsService.currentAccount else {
                    return UIImage(asset: Asset.icContactPicture).withAlignmentRectInsets(UIEdgeInsets(top: 4, left: 4, bottom: 4, right: 4))
                }
...
//    func defaultAvatarFor(profileName: String?, account: AccountModel)-> UIImage {
