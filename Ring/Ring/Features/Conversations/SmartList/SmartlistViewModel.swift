...
class SmartlistViewModel: Stateable, ViewModel {
    ...
    lazy var accounts: Observable<[AccountModel]> = {
        return self.accountsService.accountsObservable.asObservable()
    }()
    ...
    lazy var profileImage: Observable<UIImage> = { [unowned self] in
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.01, execute: {
            if let account = self.accountsService.currentAccount,
                let jamiId = AccountModelHelper(withAccount: account).ringId {
        ...
        return profileImageForCurrentAccount.share()
            .map({ profile in
                ...
                guard let name = profile.alias else {return self.defaultAvatarFor(profileName: nil)}
                let profileName = name.isEmpty ? nil : name
                return self.defaultAvatarFor(profileName: profileName)
            })
        }()
    ...
    func reloadDataFor(jamiId: String) {
        self.profileService.getProfile(ringId: jamiId)
    ...
    func defaultAvatarFor(profileName: String?)-> UIImage {
        let image = UIImage(named: "ic_contact_picture")!
        guard let account = self.accountsService.currentAccount else {return image}
    ...
    required init(with injectionBag: InjectionBag) {
        ...
        self.accountsService.currentAccountChanged
            .subscribe(onNext: { [unowned self] account in
                if let currentAccount = account, let jamiId = AccountModelHelper(withAccount: currentAccount).ringId {
                    self.reloadDataFor(jamiId: jamiId)
                }
            }).disposed(by: self.disposeBag)
