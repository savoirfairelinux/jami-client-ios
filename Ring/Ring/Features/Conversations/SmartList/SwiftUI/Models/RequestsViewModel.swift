...
class RequestRowViewModel: ObservableObject, Identifiable, Hashable {
    ...
    private var senderProfileName: String?
    private var senderRegisteredName: String?
    ...
    private func subscribeSenderProfile() {
        ...
        profilesService.getProfile(uri: profileUri, createIfNotexists: false, accountId: request.accountId)
            ...
            .subscribe(onNext: { [weak self] profile in
                guard let self = self else { return }
                let trimmedAlias = profile.alias?.trimmingCharacters(in: .whitespacesAndNewlines)
                self.senderProfileName = (trimmedAlias?.isEmpty == false) ? trimmedAlias : nil
                let newMember = GroupAvatarMember.resolve(
                    profilePhoto: profile.photo,
                    profileName: self.senderProfileName,
                    registeredName: self.senderFallbackName(),
                    jamiId: jamiId
                )
    ...
    private func subscribeSenderRegisteredName() {
        // Only look up if the request carries a display name; no name means no registration to resolve.
        guard !request.name.isEmpty,
              let jamiId = request.participants.first?.jamiId else { return }

        nameResolver.nameService.usernameLookupStatus.asObservable()
            .filter { lookupNameResponse in
            ...
            .subscribe(onNext: { [weak self] lookupNameResponse in
                guard let self = self else { return }
                guard lookupNameResponse.state == .found,
                      !lookupNameResponse.name.isEmpty else { return }
                self.senderRegisteredName = lookupNameResponse.name
                self.updateSenderMemberName()
                self.setAvatar()
            })
            .disposed(by: disposeBag)

        nameResolver.nameService.lookupAddress(withAccount: request.accountId, nameserver: "", address: jamiId)
    }
    ...
    private func updateSenderMemberName() {
        ...
        guard senderProfileName == nil else { return }
        guard let fallbackName = senderFallbackName() else { return }
        let updated = GroupAvatarMember(image: current.image, name: fallbackName)
        guard updated != current else { return }
        senderMember = updated
    }

    private func senderFallbackName() -> String? {
        if let senderRegisteredName = senderRegisteredName, !senderRegisteredName.isEmpty {
            return senderRegisteredName
        }
        if request.name.isEmpty {
            return nameResolver.bestName.isEmpty ? nil : nameResolver.bestName
        }
        return nil
    }
...
class RequestsViewModel: ObservableObject {
    ...
    func accept(requestRow: RequestRowViewModel) {
        processRequest(requestRow, action: .accept) {
            if requestRow.request.isCoredialog(), let jamiId = requestRow.request.participants.first?.jamiId {
