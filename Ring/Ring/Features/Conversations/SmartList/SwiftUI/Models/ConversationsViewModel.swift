...
class ConversationsViewModel: ObservableObject, FilterConversationDataSource {
    ...
    @Published var publicDirectoryTitle = "Public directory"
    ...
    init(injectionBag: InjectionBag, stateSubject: PublishSubject<State>) {
        ...
        self.accountsModel = AccountsViewModel(accountService: injectionBag.accountService, profileService: injectionBag.profileService, nameService: injectionBag.nameService, stateSubject: stateSubject)
        ...
        self.accountsService.currentAccountChanged
            ...
            .subscribe(onNext: { [weak self] account in
                if let account = account {
                    if account.isJams {
                        self?.publicDirectoryTitle = "Search Result"
                    }
                }
            })
            .disposed(by: self.disposeBag)
