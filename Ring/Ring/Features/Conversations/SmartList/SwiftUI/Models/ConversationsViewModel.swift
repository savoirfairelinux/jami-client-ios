...
class ConversationsViewModel: ObservableObject, FilterConversationDataSource {
    ...
    func open(withConversationViewModel conversationViewModel: ConversationViewModel) {
    }


    func newMessage() {
        self.stateSubject.onNext(ConversationState.presentNewMessage(model: self))
    }
