...
struct ConversationsView: View {
    ...
    var body: some View {
        ForEach(model.conversations) { conversation in
            Button(action: {
                // Activate navigation to MessagesListView for the selected conversation
                selectedConversation = conversation
            }) {
                ConversationRowView(model: conversation)
                    .conditionalSwipeActions(conversation: conversation, model: model)
            }
            // Invisible NavigationLink to manage navigation programmatically
            .background(NavigationLink(destination: MessagesListView(model: conversation.swiftUIModel), isActive: .constant(selectedConversation?.id == conversation.id)) {
                EmptyView()
            }.hidden())
        }
        .onAppear {
