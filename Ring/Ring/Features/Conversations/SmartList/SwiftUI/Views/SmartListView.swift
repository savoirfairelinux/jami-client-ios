...
struct SearchableSmartList: View {
    ...
    var body: some View {
        SmartListContainer(model: model, isSearchBarActive: $isSearchBarActive, selectedConversation: $selectedConversation)
...
struct SearchableNewMessage: View {
    @ObservedObject var model: ConversationsViewModel
    @SwiftUI.State private var searchText = ""
    @SwiftUI.State private var isSearchBarActive = false
    @Binding var selectedConversation: ConversationViewModel?
    var body: some View {
        NewMessageContainer(model: model, isSearchBarActive: $isSearchBarActive, selectedConversation: $selectedConversation)
            .navigationBarSearch(self.$searchText, isActive: $isSearchBarActive)
            .onChange(of: searchText) { _ in
                model.performSearch(query: searchText)
            }
...
struct SmartListView: View {
    @ObservedObject var model: ConversationsViewModel
    @SwiftUI.State var preseningNewMessage = false
    @SwiftUI.State var selectedConversation: ConversationViewModel?
    var body: some View {
        if #available(iOS 16.0, *) {
            NavigationStack {
                SearchableSmartList(model: model, selectedConversation: $selectedConversation)
                    ...
                    .sheet(isPresented: $preseningNewMessage) {
                        NewMessageListView(model: model, selectedConversation: $selectedConversation)
                    }
...
struct NewMessageListView: View {
    @ObservedObject var model: ConversationsViewModel
    @Binding var selectedConversation: ConversationViewModel?
    var body: some View {
        if #available(iOS 16.0, *) {
            NavigationStack {
                SearchableNewMessage(model: model, selectedConversation: $selectedConversation)
                    .navigationBarTitleDisplayMode(.inline)
