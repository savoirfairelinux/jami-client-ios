...
class MediaRecordViewModel: ObservableObject, Stateable, ViewModel {
    ...
    private var sliderPositionSink: ((Float) -> Void)?
    ...
    func startRecording() {
        ...
        let nameForRecordingFile = Self.recordingDateFormatter.string(from: Date()) + "_" + String(UInt64.random(in: 0...9999))
    ...
    private func transition(to state: RecordingState) {
        DispatchQueue.main.async { [weak self] in
            ...
            case .sent, .cancelled:
                self.isRecording = false
                self.isReadyToSend = false
                self.dismissHandler.dismissView()
                self.stopTimer()
            }
        }
    }
    ...
    private func startTimer() {
        ...
        // Schedule on the main run loop so timer callbacks update @Published properties on the main thread.
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            self.recordingTimer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { [weak self] _ in
                ...
                let minutes = (self.recordingSeconds / 60) % 60
                let hours = self.recordingSeconds / 3600
                self.recordDuration = hours > 0
    ...
    private func createPlayer() {
        // Stop camera frames and drop any previous player subscription before binding to the new player.
        ...
        newPlayer.playerPosition.asObservable()
            .subscribe(onNext: { [weak self] position in
                DispatchQueue.main.async { self?.sliderPositionSink?(position) }
            })
            .disposed(by: playerDisposeBag)
    }
}
