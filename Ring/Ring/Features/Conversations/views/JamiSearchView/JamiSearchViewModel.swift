...
//            .disposed(by: disposeBag)
    ...
    private func isConversation(_ conversation: ConversationViewModel, contains text: String) -> Bool {
        /*
    ...
    private func isCoreDialog(conversation: ConversationViewModel, for text: String) -> Bool {
        guard conversation.model().isCoredialog() else {
            return false
        }
        if text.isSHA1() {
            return conversation.model().getParticipants().first?.jamiId == text
        }
        if conversation.model().isSwarm() {
            return conversation.swarmInfo?.hasParticipantWithRegisteredName(name: text) ?? false
        }
        return conversation.userName.value == text
    }

    private func isConversationExists(for text: String)  -> Bool {
        let coreDialog = self.dataSource.conversationViewModels
            .filter({conversation in
                self.isCoreDialog(conversation: conversation, for: text)
            }).first
    ...
    private func addTemporaryConversationsIfNeed(text: String) {
        ...
        if self.isConversationExists(for: text) {
            return
        }
        /*
