...
class JamiSearchViewModel: ObservableObject {
    ...
    func isConversation(_ conversation: ConversationViewModel, match searchQuery: String) -> Bool {
        ...
        if conversation.model().isOnlyLocalParticipant() {
            if searchQuery.isSHA1() {
                return conversation.model().getLocalParticipants()?.jamiId == searchQuery
            }
            let registeredName = self.accountsService.currentAccount?.registeredName ?? ""
            return registeredName.lowercased() == searchQuery.lowercased()
        }
    ...
    private func convertToConversations(from searchModels: [JamiSearchViewModel.JamsUserSearchModel], accountId: String) -> [ConversationViewModel] {
        ...
        for model in newContacts where !blockedIds.contains(model.jamiId) {
            self.cacheProvisionalProfile(for: model, accountId: accountId)
            jamsSearch.append(self.createTemporaryJamsConversation(with: model, accountId: accountId))
        }
        return jamsSearch
    }

    private func cacheProvisionalProfile(for user: JamsUserSearchModel, accountId: String) {
        ...
        self.injectionBag.profileService
            .cacheProvisionalProfile(uri: uriString,
                                     accountId: accountId,
