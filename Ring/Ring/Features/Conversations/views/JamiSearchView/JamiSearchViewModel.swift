...
class JamiSearchViewModel {
    ...
    init(with injectionBag: InjectionBag, source: FilterConversationDataSource) {
        ...
        self.nameService
            ...
            .subscribe(onNext: { [weak self] nameSearchResponse in
                guard let self = self,
                      let results = nameSearchResponse.results as? [[String: String]] else { return }
                self.log.debug("[UserSearch][JamiSearchViewModel] userSearchResponseShared: \(nameSearchResponse.results.isEmpty)")
    ...
    fileprivate func search(withText text: String) {
        ...
        if self.accountsService.isJams(for: currentAccount.id) {
            ...
            self.log.debug("[UserSearch][JamiSearchViewModel] searchUser id: \(currentAccount.id), query: \(text)")
            return
        }
