...
class JamiSearchViewModel: ObservableObject {
    ...
    private func createTemporarySwarmConversation(with hash: String, accountId: String, userName: String? = nil) -> ConversationViewModel {
        ...
        let youSuffix = isSelf ? " \(L10n.Conversation.youSuffix)" : ""
    ...
    private func createTemporaryJamsConversation(with user: JamsUserSearchModel, accountId: String) -> ConversationViewModel {
        ...
        if account?.jamiId == user.jamiId {
            ...
            newConversation.userName.accept("\(currentUserName) \(L10n.Conversation.youSuffix)")
            if let displayName = newConversation.displayName.value, !displayName.isEmpty {
                newConversation.displayName.accept("\(displayName) \(L10n.Conversation.youSuffix)")
            }
        }
