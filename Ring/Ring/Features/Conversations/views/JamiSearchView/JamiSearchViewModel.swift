...
class JamiSearchViewModel {
    ...
    lazy var searchResults: Observable<[ConversationSection]> = Observable<[ConversationSection]>
                       ...
                       resultSelector: { temporaryConversation, filteredResults, jamsTemporaryResults in
                        ...
                        if !jamsTemporaryResults.isEmpty {
                            sections.append(ConversationSection(
                                header: L10n.Smartlist.results,
                                items: jamsTemporaryResults
                            ))
                        } else if let temporaryConversation = temporaryConversation {
                            sections.append(ConversationSection(
                                header: L10n.Smartlist.results,
                                items: [temporaryConversation]
                            ))
                        }
                        if !filteredResults.isEmpty {
                            sections.append(ConversationSection(
                                header: L10n.Smartlist.conversations,
    ...
    init(
        ...
        searchOnlyExistingConversations: Bool
    ) {
        ...
        isSearching = searchBarText.asObservable()
            .map { text in
                !text.isEmpty
            }
            .observe(on: MainScheduler.instance)
    ...
    static func removeFilteredConversations(from conversationViewModels: [ConversationViewModel],
                                            with filteredResults: [ConversationViewModel])
    ...
    func updateSearchStatus() {
        if !jamsTemporaryResults.value.isEmpty {
            searchStatus.onNext(.foundJams)
        } else if temporaryConversation.value != nil {
            searchStatus.onNext(.foundTemporary)
        } else {
            searchStatus.onNext(.noResult)
        }
    }
    ...
    private func temporaryConversationCreated(tempConversation: ConversationViewModel?) {
        temporaryConversation.accept(tempConversation)
        if let delegate = delegate {
            delegate.temporaryConversationCreated(conversation: tempConversation)
        }
    }

    func isConversation(_ conversation: ConversationViewModel,
                        contains searchQuery: String) -> Bool {
        /*
    ...
    private func getFilteredConversations(for searchQuery: String) -> [ConversationViewModel]? {
        let conversations =
            dataSource.conversationViewModels
    ...
    func temporaryConversationExists(for jamiId: String) -> Bool {
        guard let temporaryConversation = temporaryConversation.value else { return false }
    ...
    func isConversation(_ conversation: ConversationViewModel, match searchQuery: String) -> Bool {
        ...
        if conversation.model().isSwarm() {
            return conversation.swarmInfo?
                .hasParticipantWithRegisteredName(name: searchQuery) ?? false
        }
        return conversation.userName.value == searchQuery
    }

    func isConversationExists(for searchQuery: String) -> Bool {
        let coreDialog = dataSource.conversationViewModels
            .filter { [weak self] conversation in
                guard let self = self else { return false }
    ...
    private func createTemporaryConversation(searchQuery: String,
                                             account: AccountModel) -> ConversationViewModel {
        switch account.type {
        case .sip:
            return createTemporarySipConversation(with: searchQuery, account: account)
        case .ring:
            return createTemporarySwarmConversation(with: searchQuery, accountId: account.id)
        }
    }
    ...
    private func search(withText searchQuery: String) {
        ...
        addTemporaryConversationsIfNeed(searchQuery: searchQuery)
    }
    ...
    private func addTemporaryConversationsIfNeed(searchQuery: String) {
         ...
         */
        if accountsService.isJams(for: currentAccount.id) {
            performLookup(searchQuery: searchQuery, accounId: currentAccount.id, isJams: true)
            return
        }
        // If conversation already exists we do not need to create temporary conversation.
        if isConversationExists(for: searchQuery) {
            searchStatus.onNext(.notSearching)
            return
        }
        /*
        ...
        if currentAccount.type == .ring, !searchQuery.isSHA1() {
            performLookup(searchQuery: searchQuery, accounId: currentAccount.id, isJams: false)
            return
        }
        let tempConversation = createTemporaryConversation(
            searchQuery: searchQuery,
            account: currentAccount
        )
        temporaryConversationCreated(tempConversation: tempConversation)
    }

    private func performLookup(searchQuery: String, accounId: String, isJams: Bool) {
        searchStatus.onNext(.searching)
        ...
        nameService.usernameLookupStatus
            ...
            .subscribe(onNext: { [weak self] lookupResponse in
                ...
                guard lookupResponse.state == .found,
                      lookupResponse.name == self.searchBarText.value
                else {
                ...
                let tempConversation = self.createTemporarySwarmConversation(
                    with: lookupResponse.address,
                    accountId: account.id,
                    userName: lookupResponse.name
                )
        ...
        nameService
            ...
            .subscribe(onNext: { [weak self] nameSearchResponse in
                ...
                let users = results.map { result in
                    ContactsUtils.deserializeUser(dictionary: result)
                }
                .compactMap { $0 }
            ...
            .disposed(by: disposeBag)
        if isJams {
            nameService.searchUser(withAccount: accounId, query: searchQuery)
        } else {
            nameService.lookupName(withAccount: accounId, nameserver: "", name: searchQuery)
        }
    }

    private func cleanUpPreviousSearch() {
        temporaryConversationCreated(tempConversation: nil)
        jamsTemporaryResults.accept([])
        filteredResults.accept([])
    }

    private func convertToConversations(
        from searchModels: [JamiSearchViewModel.JamsUserSearchModel],
        accountId: String
    ) -> [ConversationViewModel] {
        var jamsSearch: [ConversationViewModel] = []
        for model in searchModels {
            let newConversation = createTemporaryJamsConversation(with: model, accountId: accountId)
            jamsSearch.append(newConversation)
        }
        return jamsSearch
    }

    private func createTemporarySwarmConversation(
        with hash: String,
        accountId: String,
        userName: String? = nil
    ) -> ConversationViewModel {
        let uri = JamiURI(schema: URIType.ring, infoHash: hash)
    ...
    private func createTemporarySipConversation(with searchQuery: String,
                                                account: AccountModel) -> ConversationViewModel {
        let trimmed = searchQuery.trimmedSipNumber()
        let uri = JamiURI(schema: URIType.sip, infoHash: trimmed, account: account)
        let conversation = ConversationModel(withParticipantUri: uri,
                                             accountId: account.id,
                                             hash: trimmed)
    ...
    private func createTemporaryJamsConversation(with user: JamsUserSearchModel,
                                                 accountId: String) -> ConversationViewModel {
        let uri = JamiURI(schema: URIType.ring, infoHash: user.jamiId)
