...
struct PlayerView: View {
    ...
    var onVideoTap: ((_ frame: CGRect, _ frameProvider: @escaping () -> CGRect) -> Void)?
    ...
    var body: some View {
        ZStack {
            ...
            // Full-screen tap layer sits below the controls so buttons remain tappable.
...
private extension View {
    ...
    func applyGlassButtonBackground() -> some View {
        glassCircleBackground()
            .overlay(Capsule().stroke(style: .init(lineWidth: 1, lineCap: .round, lineJoin: .round)))
    }
}
