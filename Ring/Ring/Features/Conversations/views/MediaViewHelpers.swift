...
// MARK: - Auto-Hide Task Holder

/// Holds a cancellable `Task` used to auto-hide UI controls after a delay.
/// Shared by `MediaPreviewView` and `PlayerCoordinator`.
final class HideTaskHolder {
    var task: Task<Void, Never>?

    /// Schedule hiding `controlsVisible` after `delay` seconds.
    /// Any previous pending task is cancelled first.
    func schedule(delay: UInt64 = 4_000_000_000,
                  animation: SwiftUI.Animation = .spring(response: 0.35, dampingFraction: 0.8),
...
extension Float {
    ...
    var durationString: String {
        ...
        return Int(self / 1_000_000).durationString
    }
}

extension Int {
    /// Converts a duration in whole seconds to a human-readable `mm:ss` or `hh:mm:ss` string.
    var durationString: String {
        let seconds = self % 60
