...
class MessageContentModel: ObservableObject, MessageBased {
    ...
    required init(message: MessageModel, stateSubject: PublishSubject<State>) {
        ...
        if self.type == .fileTransfer {
            ...
            self.textColor = Color(UIColor.secondaryLabel)
            self.secondaryColor = Color(UIColor.tertiaryLabel)
            self.updateTransferActions()
            self.stateSubject.onNext(MessageState.size(transferId: message.daemonId, message: self))
        }
    }
    ...
    private func updateTransferActions() {
        if self.fileSize == 0 {
            self.stateSubject.onNext(MessageState.size(transferId: self.message.daemonId, message: self))
        }
        DispatchQueue.main.async {
            ...
            if self.transferStatus == .success, self.image == nil {
                self.stateSubject.onNext(MessageState.getImage(transferId: self.message.daemonId, message: self))
            }
            self.updateFileInfo()
        }
    }
