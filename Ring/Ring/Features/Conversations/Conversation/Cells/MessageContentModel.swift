...
class MessageContentModel: ObservableObject {
    ...
    @Published var fileName = "file.txt"
    ...
    convenience init() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            self.fileProgress = 10
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                self.fileProgress = 20
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    self.fileProgress = 30
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                        self.fileProgress = 40
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                            self.fileProgress = 50
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                                self.fileProgress = 60
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                                    self.fileProgress = 70
                                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                                        self.fileProgress = 80
                                    }
                                }
                            }
                        }
