...
class MessageCellLocationSharingSent: MessageCellLocationSharing {
 ...
 //            .observeOn(MainScheduler.instance)
            .subscribe({ [weak self] location in
                ...
                if let imageData = conversationViewModel.myOwnProfileImageData, let circledImage = UIImage(data: imageData)?.circleMasked {
                    self.myLocationMarker.image = circledImage
                } else {
                    self.myLocationMarker.image = UIImage(asset: Asset.fallbackAvatar)!
                }
    ...
    private func setupStopSharingButton() {
        let constraint = NSLayoutConstraint(item: self.stopSharingButton!,
                                            attribute: NSLayoutConstraint.Attribute.centerY,
                                            relatedBy: NSLayoutConstraint.Relation.equal,
                                            toItem: self.stopSharingButton.superview,
                                            attribute: NSLayoutConstraint.Attribute.centerY,
                                            multiplier: 1,
                                            constant: 87)
        self.stopSharingButton.superview?.addConstraint(constraint)
