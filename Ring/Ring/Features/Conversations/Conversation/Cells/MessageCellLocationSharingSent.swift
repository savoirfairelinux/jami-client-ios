...
class MessageCellLocationSharingSent: MessageCellLocationSharing {
    ...
    //var myLocation = BehaviorRelay<MaplyCoordinate>(value: MaplyCoordinate())
    var myLocationMarker = BehaviorRelay<MaplyScreenMarker>(value: MaplyScreenMarker())
 ...
 //            .observeOn(MainScheduler.instance)
            .subscribe({ [weak self] location in
                ...
                if let imageData = conversationViewModel.profileImageData.value, let circledImage = UIImage(data: imageData)?.circleMasked {
                    self.myLocationMarker.value.image = circledImage
                }
                self.myLocationMarker.value.loc = maplyCoordonate
                self.myLocationMarker.value.size = CGSize(width: 32, height: 32)
        ...
        self.maplyViewController?.addScreenMarkers([myLocationMarker.value], desc: nil)
    }
