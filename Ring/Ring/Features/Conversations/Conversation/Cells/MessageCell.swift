...
class MessageCell: UITableViewCell, NibReusable, PlayerDelegate {
    ...
    private func prepareForReuseTapGesture() {
        ...
        if self.buttonsHeightConstraint != nil && self.buttonsHeightConstraint!.relation == .equal {
            self.buttonsHeightConstraint?.constant = 31
            changeRelation(constraint: self.buttonsHeightConstraint!, relation: .greaterThanOrEqual)
        }
    }
    ...
    private func prepareForTapGesture() {
        if self.buttonsHeightConstraint != nil && self.buttonsHeightConstraint!.relation != .equal {
            ...
            changeRelation(constraint: self.buttonsHeightConstraint!, relation: .equal)
        }
    }
