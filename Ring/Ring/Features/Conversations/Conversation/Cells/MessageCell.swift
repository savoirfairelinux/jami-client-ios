...
class MessageCell: UITableViewCell, NibReusable, PlayerDelegate {
    ...
    private func changeRelationToEqual(constraint: NSLayoutConstraint) -> NSLayoutConstraint {
        return NSLayoutConstraint(item: constraint.firstItem!, attribute: constraint.firstAttribute, relatedBy: .equal,
    ...
    private func prepareForTapGesture() {
        if self.buttonsHeightConstraint != nil && self.buttonsHeightConstraint!.relation != .equal {
            self.buttonsHeightConstraint?.constant = self.bubble.frame.height

            let new = changeRelationToEqual(constraint: self.buttonsHeightConstraint!)
            self.removeConstraint(self.buttonsHeightConstraint!)
            self.addConstraint(new)
            self.layoutIfNeeded()
            self.buttonsHeightConstraint = new
        }
    }
