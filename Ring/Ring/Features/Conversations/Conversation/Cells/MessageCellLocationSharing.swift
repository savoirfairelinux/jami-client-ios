...
extension MessageCellLocationSharing {
    private func expandOrShrink() {
        ...
        self.changeHeight(shouldExpand)
        self.changeWidth(shouldExpand)
        ...
        //self.layoutIfNeeded()
        let expandingNextValue = !self.locationTapped.value.1
        self.locationTapped.accept((true, expandingNextValue))
        self.locationTapped.accept((false, expandingNextValue))
    }

    private func changeHeight(_ shouldExpand: Bool, extendedHeight: CGFloat = 350) {
    ...
    private func changeWidth(_ shouldExpand: Bool) {
        ...
        } else if self as? MessageCellLocationSharingReceived != nil {
    ...
    func expandHeight(_ shouldExpand: Bool, _ height: CGFloat) {
        if shouldExpand {
            self.changeHeight(shouldExpand, extendedHeight: height * 0.86)
        }
        self.layoutIfNeeded()
    }
}
...
extension MessageCellLocationSharing {
    private func setupXButton() {
        ...
        xButton.cornerRadius = xButton.frame.height / 2.0
