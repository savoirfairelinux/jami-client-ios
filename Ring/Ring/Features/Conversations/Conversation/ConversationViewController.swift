    ...
    private func deleteCellSetup(_ cell: MessageCell) {
        cell.deleteMessage
            ...
            .subscribe(onNext: { [weak self, weak cell] (shouldDelete) in
                ...
                } else if cell as? MessageCellLocationSharingSent != nil {
                    self.tableView.isScrollEnabled = true
                }
    ...
    private func locationCellSetup(_ item: MessageViewModel, _ cell: MessageCell) {
        ...
        cell.locationTapped
            ...
            .subscribe(onNext: { [weak self, weak cell] (locationTapped) in
                ...
                self.log.debug("[ConversationViewController] locationCellSetup executing")

                if let index = self.tableView.indexPath(for: cell) {
                    cell.expandHeight(locationTapped.1,
                                      self.tableView.frame.height - self.tableView.contentInset.top - self.tableView.contentInset.bottom)
                    self.tableView.performBatchUpdates({ self.tableView.updateConstraintsIfNeeded() }, completion: nil)
                    self.tableView.scrollToRow(at: index, at: UITableView.ScrollPosition.top, animated: true)
                    self.tableView.isScrollEnabled = !locationTapped.1
                } else {
                    self.log.error("[ConversationViewController] locationCellSetup, something went wrong")
                }
            })
            .disposed(by: cell.disposeBag)
    }
