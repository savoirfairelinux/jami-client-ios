...
class ConversationViewController: UIViewController, UITextFieldDelegate, StoryboardBased, ViewModelBased {
    ...
    func applyBubbleStyleToCell(toCell cell: MessageCell,
        ...
        if cellRowShowsTime(cellRow: indexPath.row + 1) {
            cell.bubble.backgroundColor = UIColor.red
        }

//        if indexPath.row + 1 < (self.messageViewModels?.count)! {
//            let nextmsgVM = self.messageViewModels?[indexPath.row + 1]
//            if (nextmsgVM?.showsTime)! {
//                cell.bubble.backgroundColor = UIColor.red
//            }
//        }

        //let previousIndexPath = IndexPath(item: indexPath.row + 1, section: 0)
        //if let previousCell = getCellAtRow(cellForRowAt: previousIndexPath) as? MessageCell {

//            if previousCell.timeLabel.isHidden == false {
//                cell.bubble.backgroundColor = UIColor.red
//            }
        //}
    ...
    func cellRowShowsTime(cellRow row: Int) -> Bool {
//        if row >= 0 && row < shownTimes.count {
//            return self.shownTimes[row] != nil
//        }
//        return false

        if row >= 0 && row < (self.messageViewModels?.count)! {
            return (self.messageViewModels?[row].showsTime)!
        }
        return false
    }

    func getCellAtRow(cellForRowAt indexPath: IndexPath) -> UITableViewCell? {
    ...
    func formatCell(withCell cell: MessageCell,
        ...
        applyBubbleStyleToCell(toCell: cell, cellForRowAt: indexPath, withChaining: chaining, withContent: messageVM.content, withType: type)
