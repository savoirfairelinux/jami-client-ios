    ...
    func formatCell(withCell cell: MessageCell,
        ...
        if messageVM.bubblePosition() == .sent {
            messageVM.status.asObservable()
                ...
                .map { value in
                    self.log.debug("id: \(messageVM.id), status \(value.rawValue)")
                    return value == MessageStatus.sending ||
                            value == MessageStatus.unknown ? true : false
                }
                .bind(to: cell.messageLabel.rx.isHidden)
                .disposed(by: self.disposeBag)
        }
    }
