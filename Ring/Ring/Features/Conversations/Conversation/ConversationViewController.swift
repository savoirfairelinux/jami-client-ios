...
extension ConversationViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        self.log.warning("cellForRowAt: \(indexPath.row)")
        if let item = self.messageViewModels?[indexPath.row] {
            ...
            if item.isTransfer && item.bubblePosition() == .received {
                ...
                self.log.warning("item.initialTransferStatus: \(item.initialTransferStatus)")
                changeTransferStatus(cell, nil, item.initialTransferStatus, item, viewModel)
