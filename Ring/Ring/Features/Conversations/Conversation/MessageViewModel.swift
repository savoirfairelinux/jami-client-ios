...
class MessageViewModel: Identifiable {
    ...
    var messageRow: MessageRowModel
    ...
    var getAvatar: ((String) -> Void)? {
        didSet {
            self.messageRow.getAvatar = self.getAvatar
        }
    }

    var getlastRead: ((String) -> Void)? {
        didSet {
            self.messageRow.getlastRead = self.getlastRead
        }
    }
    ...
    var shouldShowTimeString: Bool = false {
        didSet {
            self.messageRow.timeString = shouldShowTimeString ? MessageViewModel.getTimeLabelString(forTime: self.receivedDate) : nil
    ...
    init() {
        ...
        self.messageRow = MessageRowModel()
        self.message = MessageModel(withInfo: [String: String](), accountJamiId: "")
        self.displayReadIndicator = BehaviorRelay(value: true)
        self.stateSubject = PublishSubject<State>()
        self.messageContent = MessageContentModel(message: message, stateSubject: self.stateSubject)
        self.initialTransferStatus = .unknown
    }

    init(withInjectionBag injectionBag: InjectionBag,
         withMessage message: MessageModel, isLastDisplayed: Bool, convId: String, accountId: String, stateSubject: PublishSubject<State>) {
        ...
        self.messageContent = MessageContentModel(message: message, stateSubject: self.stateSubject)
        self.messageRow = MessageRowModel()
        self.messageRow.timeString = MessageViewModel.getTimeLabelString(forTime: self.message.receivedDate)
        self.messageRow.incoming = self.message.incoming
        self.messageRow.partisipantId = self.message.authorId
        self.messageRow.messageId = self.messageId
        //        self.messageContent.image = self.getTransferedImage(maxSize: 450, conversationID: convId, accountId: accountId, isSwarm: true)
        // self.messageContent.state = self.state
