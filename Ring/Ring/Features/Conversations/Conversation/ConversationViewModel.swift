...
//    }()
    ...
    func shouldDisplayInvitationView() -> Bool {
        return self.conversation.value.isRequest || self.conversation.value.needsSyncing || self.contactsService.contact(withUri: self.conversation.value.getParticipants().first?.uri ?? "") == nil
    }
...
//            }
            ...
            if filterParicipants.count == 1,
               let contact = self.contactsService.contact(withUri: filterParicipants.first?.uri ?? "") {
