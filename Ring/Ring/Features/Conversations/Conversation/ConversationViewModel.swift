...
//            }
            ...
            let showInv = self.conversation.value.needsSyncing || self.contactsService.contact(withUri: self.conversation.value.getParticipants().first?.jamiId ?? "") == nil
            ...
            if filterParicipants.count == 1,
               let contact = self.contactsService.contact(withUri: filterParicipants.first?.jamiId ?? "") {
...
//                                                        contactUri: self.conversation.value.participantUri)
    ...
    func openInvitationView(parentView: UIViewController) {
        ...
        } else if self.conversation.value.conversationId.isEmpty {
            // send invitation for search result
            self.stateSubject.onNext(ConversationState.openOutgoingInvitationView(displayName: name, contactJamiId: self.conversation.value.hash, accountId: self.conversation.value.accountId, parentView: parentView, invitationHandeledCB: handler))
        }
    }
}
