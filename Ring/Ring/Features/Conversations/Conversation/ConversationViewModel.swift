...
class ConversationViewModel: ViewModel {
        ...
        didSet {
            ...
            self.presenceService
                ...
                .subscribe(onNext: { [unowned self] presenceUpdateEvent in
                    if let status: Int = presenceUpdateEvent.getEventInput(.presenceStatus) {
                        self.recipientPresence.onNext(status > 0 ? true : false)
                    }
                })
    ...
    var recipientPresence = BehaviorSubject(value: false)
