...
class ConversationViewModel: ViewModel {
    ...
    var conversation: ConversationModel! {
        didSet {
            ...
            self.contactsService.contactStatus.map ({ contactN -> (ContactModel?) in
                if contactN.ringId == self.conversation.recipientRingId {
                ...
                .subscribe(onNext: { contact in
                    if let contact = contact {
                        ...
                        if contact.confirmed {
                            self.generateMessage(ofType: GeneratedMessageType.contactRequestAccepted).subscribe().disposed(by: <#T##DisposeBag#>)
                        }
                    }
                }).disposed(by: self.disposeBag)
