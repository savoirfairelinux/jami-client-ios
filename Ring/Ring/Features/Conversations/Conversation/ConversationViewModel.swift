...
//    }()
    ...
    func haveCurrentCall() -> Bool {
        return self.callService.call(participantURI: self.conversation.value.hash, accountID: self.conversation.value.accountId) != nil
    }
    ...
    lazy var showCallButton: Observable<Bool> = { [unowned self] in
           return self.callService
               ...
               .asObservable()
            .filter({ (call) -> Bool in
                call.participantUri.replacingOccurrences(of: "@ring.dht", with: "") == self.conversation.value.hash && call.accountId == self.conversation.value.accountId
            })
               .map({ call in
                   let callIsValid = self.callIsValid(call: call)
