...
extension ConversationViewModel {
    ...
    func getTransferProgress(transferId: String) -> Float? {
        return self.dataTransferService.getTransferProgress(withId: transferId, accountId: self.conversation.value.accountId, conversationId: self.conversation.value.id, isSwarm: self.conversation.value.isSwarm())
    }
