...
class ConversationViewModel: Stateable, ViewModel {
    ...
    func sendMessage(withContent content: String, contactURI: String? = nil) {
        ...
        let receipientURI = (contactURI != nil) ? contactURI! : self.conversation.value.participantUri
...
extension ConversationViewModel {
    ...
    private func changeConversationIfNeeded(items: [ConferencableItem]) {
        guard let selectedItem = items.first else { return }
        guard let selectedItemURI = selectedItem.contacts.first?.uri,
            items.count == 1,
            selectedItemURI != self.conversation.value.participantUri else { return }
        self.stateSubject.onNext(ConversationState.replaceCurrentconversationWithConversationFor(participant: selectedItemURI))
    }
    ...
    private func shareMessage(messageId: Int64, with selectedContacts: [ConferencableItem]) {
        guard let message = self.messages.value.filter({ $0.messageId == messageId }).first,
            !message.message.isGenerated,
        ...
        let image = message.getTransferedImage(maxSize: 200, conversationID: conversationId, accountId: accountId)
...
extension ConversationViewModel {
    ...
    func sendFile(filePath: String, displayName: String, localIdentifier: String? = nil, contactHash: String? = nil) {
        ...
        let hash = (contactHash != nil) ? contactHash! : self.conversation.value.hash
    ...
    func sendAndSaveFile(displayName: String, imageData: Data, contactHash: String? = nil, conversation: String? = nil) {
        guard let accountId = accountService.currentAccount?.id else { return }
        let hash = (contactHash != nil) ? contactHash! : self.conversation.value.hash
        let conversationId = (conversation != nil) ? conversation! : self.conversation.value.conversationId
    ...
    func sendImageFromPhotoLibraty(image: UIImage, imageName: String, localIdentifier: String?, contactHash: String? = nil) {
        ...
        self.sendFile(filePath: localCachePath.path,
                      ...
                      localIdentifier: localIdentifier, contactHash: contactHash)
    }
