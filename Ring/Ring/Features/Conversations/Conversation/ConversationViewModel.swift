...
extension ConversationViewModel {
    ...
    private func setupPeerSharingAvailability() {
        ...
        requeryPeerServicesOnPresenceOnline(accountId: accountId, peerId: peerId)
    }
    ...
    private func requeryPeerServicesOnPresenceOnline(accountId: String, peerId: String) {
        contactPresence
            .distinctUntilChanged()
            .skip(1)
            .filter { $0 == .available || $0 == .connected }
            .flatMapLatest { [peerSharingService] _ in
                peerSharingService.queryPeerServices(accountId: accountId, peerId: peerId)
            }
            .observe(on: MainScheduler.instance)
            .subscribe(onNext: { [weak self] result in
