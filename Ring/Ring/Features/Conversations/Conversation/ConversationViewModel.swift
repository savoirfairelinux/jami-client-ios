...
extension ConversationViewModel {
    ...
    private func changeConversationIfNeeded(items: [String]) {
        ...
        let contactsURIs = items.map { [weak self] item in
            self?.conversationsService.getConversationForId(conversationId: item, accountId: accountId)
        }
        .compactMap({ $0 })
        guard let selectedConversation = contactsURIs.first else { return }
        self.stateSubject.onNext(ConversationState.replaceCurrentWithConversationWith(conversation: selectedConversation))
    }
    ...
    private func shareMessage(message: MessageContentVM, with conversationId: String, fileURL: URL?, fileName: String) {
        ...
        if let url = fileURL {
            if let jamiId = conversationModel.getParticipants().first?.jamiId, accountService.currentAccount?.jamiId == jamiId {
...
extension ConversationViewModel {
    ...
    func sendFile(filePath: String, displayName: String, localIdentifier: String? = nil, contactHash: String? = nil, conversationModel: ConversationModel? = nil) {
        let conversation = conversationModel ?? self.conversation.value
