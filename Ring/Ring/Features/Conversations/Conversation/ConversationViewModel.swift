...
//        guard let conversation = accountService.currentAccount?.id else { return }
...
//            self.conversationsService.transferStatusChanged(DataTransferStatus.error, for: transferId, accountId: currentAccount.id, to: peerInfoHash)
    ...
    func getTransferProgress(transferId: String, accountId: String) -> Float? {
        return self.dataTransferService.getTransferProgress(withId: transferId, accountId: accountId, conversationId: self.conversation.value.id)
    }
    ...
    func getTransferSize(transferId: String, accountId: String) -> Int64? {
        guard let info = self.dataTransferService.dataTransferInfoNew(withId: transferId, accountId: accountId, conversationId: self.conversation.value.id) else { return nil }
