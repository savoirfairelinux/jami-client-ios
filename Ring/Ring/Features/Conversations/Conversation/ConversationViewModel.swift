...
class ConversationViewModel: Stateable, ViewModel, ObservableObject, Identifiable {
    ...
    func joinActiveCall(call: ActiveCall, withVideo: Bool) {
        let callURI = "rdv:" + call.conversationId + "/" + call.uri + "/" + call.device + "/" + call.id
        if withVideo {
    ...
    func startCall() {
        ...
        let uri = self.conversation.isDialog() ? jamiId : "swarm:" + self.conversation.id
        self.closeAllPlayers()
        self.stateSubject.onNext(ConversationState.startCall(contactRingId: uri, userName: self.displayName.value ?? self.userName.value))
    }
