...
class ConversationViewModel: Stateable, ViewModel {
    ...
    func acceptTransfer(id: Int64) -> NSDataTransferError {
        guard let transferId = self.conversationsService.dataTransferMessageMap.filter( { $0.value == id } ).first?.key else {
