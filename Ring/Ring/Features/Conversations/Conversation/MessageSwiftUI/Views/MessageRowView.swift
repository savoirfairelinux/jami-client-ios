                    ...
                    if model.shouldDisplayAavatar,
                       let factory = avatarFactory {
                        Spacer()
                            ...
                            AvatarSwiftUIView(source: factory.provider(for: jamiId, size: Constants.AvatarSize.small.points))
                        Spacer()
                            .frame(width: 10)
                    } else {
                        Spacer()
                ...
                HStack(alignment: .top, spacing: -3) {
                    ...
                    ForEach(readIds.indices, id: \.self) { index in
                        if let factory = avatarFactory {
                            AvatarSwiftUIView(source: factory.provider(for: readIds[index], size: Constants.AvatarSize.tiny.points))
