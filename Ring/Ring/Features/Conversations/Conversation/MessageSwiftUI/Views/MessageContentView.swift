...
struct MessageContentView: View {
    ...
    var body: some View {
        ZStack(alignment: emojiAlignment) {
            VStack(alignment: messageModel.replyTarget.alignment) {
                ...
                MessageBubbleView(messageModel: messageModel, model: model, onLongPress: onLongPress)
                    ...
                    .background(GeometryReader { preferences in
                        Color.clear
                            .preference(key: BubbleWidthKey.self, value: preferences.size.width)
                    })
            }
            ...
            GeometryReader { proxy in
                Color.clear
                    .onAppear { self.bubbleWidth = max(self.bubbleWidth, proxy.size.width) }
            }
        }
    ...
    private func replyIndicator() -> some View {
        ...
        let width = bubbleWidth > 0 ? bubbleWidth : messageWidth
