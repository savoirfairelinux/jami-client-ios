...
struct LocationSharingView: View {
    ...
    @Binding var coordinates: [LocationSharingAnnotation]
    ...
    //    @SwiftUI.State private var mapView: MapView?
    ...
    var mapWidth: CGFloat {
        return model.isMapOpened ? UIScreen.main.bounds.size.width : 250
    }

    var mapHeight: CGFloat {
        return model.isMapOpened ? (UIScreen.main.bounds.size.height - navigationBarHeight) : 150
    }

    var labelHeight: CGFloat {
        return model.isMapOpened ? 60 : 0
    }
    ...
    var body: some View {
        HStack {
            ZStack(alignment: model.isMapOpened ? .center : .bottom) {
                VStack(spacing: 0) {
                    ...
                    .frame(width: mapWidth, height: labelHeight)
                    .background(Color(UIColor.darkGray))
                    .cornerRadius(radius: viewCornerRadius, corners: [.topLeft, .topRight])
                    ZStack(alignment: .bottom) {
                        MapView(annotations: $coordinates, showZoomButton: $model.isMapOpened)
        ...
        //                self.mapView = MapView(coordinates: $coordinates, showZoomButton: $model.isMapOpened)
        //            }
        //        })
