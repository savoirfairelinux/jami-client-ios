...
struct LocationSharingView: View {
    ...
    @SwiftUI.State var isMapOpened = false
    ...
    var mapWidth: CGFloat {
        return isMapOpened ? UIScreen.main.bounds.size.width : 250
    }

    var mapHeight: CGFloat {
        return isMapOpened ? (UIScreen.main.bounds.size.height - navigationBarHeight) : 150
    }

    var labelHeight: CGFloat {
        return isMapOpened ? 60 : 0
    }

    var body: some View {
        HStack {
            ZStack(alignment: isMapOpened ? .center : .bottom) {
                VStack(spacing: 0) {
                    HStack {
                        Spacer()
                            .frame(width: 20)
                    ...
                    ZStack(alignment: .bottom) {
                        MapView(annotations: $model.coordinates, showZoomButton: $isMapOpened)
                            .cornerRadius(isMapOpened ? 0 : viewCornerRadius)
                            .onTapGesture {
                                if !isMapOpened {
