...
class ContextMenuVM {
    ...
    func calculateOffsets() {
        ...
        let maxMessageHeight = screenHeight - (menuSize.height + navBarHeight + 20)
        messageHeight = min(maxMessageHeight, messageFrame.height)
        let offsetY = max(0, messageOffsetY)
        let diff = screenHeight - (offsetY + menuSize.height + navBarHeight + messageHeight)
