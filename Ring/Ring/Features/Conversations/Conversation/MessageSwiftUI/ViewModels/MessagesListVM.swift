            ...
            .subscribe(onNext: { [weak self] pair in
                if let self = self, let msg = pair.values.first, let messageId = pair.keys.first, !msg.isEmpty, !messageId.isEmpty, let separatorIndex = msg.firstIndex(of: ":") {
                    ...
                    if shouldAccept {
                        ...
                        self.conversationService.sendSwarmMessage(conversationId: self.conversation.id, accountId: self.conversation.accountId, message: "", parentId: reactionMsgId, flag: 1)
                    }
                }
            })
