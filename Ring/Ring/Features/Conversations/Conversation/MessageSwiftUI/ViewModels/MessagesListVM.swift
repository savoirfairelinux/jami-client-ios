                ...
                if let self = self, !event.isEmpty {
                    ...
                    case ReactionCommand.apply.toString():
                        if let dat = event["data"], let mId = event["parentMessageId"] {
                            ...
                            log.error(message: "[MessagesListVM] Invalid data provided while trying to add a reaction a message.")
                        }
                    case ReactionCommand.revoke.toString():
                        if let rId = event["reactionId"] {
                            ...
                            log.error(message: "[MessagesListVM] Invalid message ID provided while trying to revoke a reaction from a message.")
                        }
                    default: break
                    }
                }
