...
class MessagesListVM: ObservableObject {
    ...
    private func insert(newMessage: MessageModel, fromHistory: Bool) -> Bool {
        ...
        let container = MessageContainerModel(message: newMessage, contextMenuState: self.contextStateSubject, localJamiId: localJamiId)
    ...
    private func computeSequencing() {
        ...
        for (index, model) in self.messagesModels.enumerated() {
            ...
            model.shouldDisplayName = shouldDisplayName(container: model)
        }
    }
    ...
    private func shouldDisplayName(container: MessageContainerModel) -> Bool {
        return (container.sequencing == .firstOfSequence || container.sequencing == .singleMessage) && container.message.incoming && !container.message.isReply()
    }
