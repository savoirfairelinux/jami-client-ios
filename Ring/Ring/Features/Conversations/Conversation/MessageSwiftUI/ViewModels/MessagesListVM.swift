...
class MessagesListVM: ObservableObject {
                ...
                .subscribe { [weak self] messages in
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                        if self.shouldScroll() {
                            self.scrollToId = self.messagesModels.last?.message.id
                        }
                        self.loading = false
                    }
                } onError: { _ in
    ...
    private func shouldScroll() -> Bool {
        /*
         scroll should be performed when a new message received while previous last message for conversation
         was visible on the screen
         */
        if visibleRows.isEmpty { return true }

        // check if previous message was visible on screen
        if self.messagesModels.count < 2 {
            return true
        }
        let previousMessage = self.messagesModels[1]
