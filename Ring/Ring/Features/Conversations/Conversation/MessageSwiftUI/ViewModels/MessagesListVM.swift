    ...
    private func nameLookup(id: String) {
        self.nameService.usernameLookupStatus
            ...
            .subscribe(onNext: { [weak self] lookupNameResponse in
                ...
                let observableAvatar = self.avatars.get(key: id) as? BehaviorRelay<UIImage?>
                if let username = self.names.get(key: id) as? BehaviorRelay<String>,
                   !username.value.isEmpty,
                   observableAvatar?.value == nil {
                    let image = UIImage.createContactAvatar(username: username.value, size: CGSize(width: 30, height: 30))
