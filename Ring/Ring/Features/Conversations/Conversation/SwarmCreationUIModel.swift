...
class SwarmCreationUIModel: ObservableObject {
    ...
    required init(with injectionBag: InjectionBag, conversation: ConversationModel) {
        self.swarmInfo = SwarmInfo(injectionBag: injectionBag, conversation: conversation)
        injectionBag
            .contactsService
            .contacts
            .asObservable()
        ...
        self.swarmInfo.participants.subscribe { infos in
            for info in infos {
