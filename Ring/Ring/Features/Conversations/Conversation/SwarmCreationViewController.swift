...
class SwarmCreationViewController: UIViewController, ViewModelBased, StoryboardBased {
    ...
    override func viewDidLoad() {
        ...
        let model = SwarmCreationUIModel(with: self.viewModel.injectionBag)
        self.viewModel.itemList
            .observe(on: MainScheduler.instance)
            .subscribe(onNext: { contactList in
                model.contactItems = contactList
            })
            .disposed(by: self.disposeBag)
        // model.contactItems = self.viewModel.contactItems
