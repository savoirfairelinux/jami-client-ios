...
enum InvitationStatus {
    ...
    case inSynchronization // conversation request in synchronization
    case added
    case refused
    case invalid
}
...
class InvitationViewModel: ViewModel {
    ...
    func setInfoForRequest(request: RequestModel, displayName: String, invitationHandeledCB: @escaping ((_ conversationId: String) -> Void)) {
        ...
        profileImageData.accept(request.avatar)
        if let participantId = request.participants.first?.jamiId, request.participants.count == 1 {
        ...
        self.request!.inSynchronization
            .startWith(self.request!.inSynchronization.value)
            .subscribe(onNext: { [weak self] inSynchronization in
                if inSynchronization {
                    self?.invitationStatus.accept(.inSynchronization)
                }
            }, onError: { _ in
            })
    ...
    private func listenConversationStatusForContact() {
        self.conversationsService.conversationReady
            .subscribe { [weak self] conversationId in
                ...
                if conversation.conversationType == .oneToOne && conversation.containsParticipant(participant: self.contactJamiId) {
                    self.invitationStatus.accept(.added)
                    self.invitationHandeledCB(conversationId)
                }
            } onError: { _ in
    ...
    func acceptRequest() {
        ...
        if request.type == .contact || (request.participants.count == 1 && self.contactsService.contact(withUri: request.participants.first!.jamiId) == nil) {
    ...
    func sendRequest() {
        self.contactsService
            .sendContactRequest(toContactRingId: self.contactJamiId,
