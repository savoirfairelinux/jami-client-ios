...
class InvitationViewController: UIViewController, StoryboardBased, ViewModelBased {
    ...
    override func viewDidLoad() {
        ...
        self.contactAvatar.addSubview(AvatarView(profileImageData: self.viewModel.profileImageData.value, username: self.viewModel.displayName.value, size: 90))
        self.viewModel.profileImageData
            .observe(on: MainScheduler.instance)
            .subscribe { [weak self] data in
                guard let self = self, let data = data else { return }
                self.contactAvatar.addSubview(AvatarView(profileImageData: data, username: self.viewModel.displayName.value, size: 90))
            } onError: { [weak self] _ in
    ...
    func setSynchronizationContactView() {
        buttonsContainer.isHidden = true
        inviteButton.isHidden = true
        invitationLabel1.isHidden = true
        invitationLabel3.isHidden = false
