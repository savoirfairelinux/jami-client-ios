...
class MessagesListModel: ObservableObject, Stateable {
    ...
    @Published var scrolToLast = false
    ...
    init (bag: InjectionBag, convId: String, accountId: String, conversation: ConversationViewModel) {
        ...
        self.dataTransferService = bag.dataTransferService
    ...
    func getTransferedImage(maxSize: CGFloat,
        ...
        if message.incoming && message.transferStatus != .success {
            return nil
        }
        let name = message.daemonId
    ...
    func subscrabeState() {
        self.stateSubject
            .subscribe(onNext: { [weak self] (state) in
                ...
                switch state {
                case .accept, .cancel, .openPlayer(player: _):
                    break
                case .progress(let transferId, let message):
                    if let progress = self.getTransferProgress(transferId: transferId) {
                        message.fileProgress = CGFloat(progress)
                    }
                case .size(transferId: let transferId, message: let message):
                    if let size = self.getTransferSize(transferId: transferId) {
                        message.fileSize = size
                    }
                case .getImage(transferId: let transferId, message: let message):
                    guard let conv = self.conversationModel?.conversation.value else { return }
                    guard let messageM = self.messagesModels.filter({ model in
                        model.id == message.message.id
                    }).first else { return }
                    let image = messageM.getTransferedImage(maxSize: 450, conversationID: conv.id, accountId: conv.accountId, isSwarm: conv.isSwarm())
                    let shouldScroll = image != nil && message.image == nil
                    self.updateLastVisibleRow()
                    let sasds = self.lastMessageOnScreen == message.message.id
                    message.image = image
                    if shouldScroll && sasds {
                        self.scrolToLast = true
                    }
                }
            })
    ...
    private func insert(message: MessageModel, bag: InjectionBag, convId: String) -> Bool {
        ...
        let model = MessageViewModel(withInjectionBag: bag, withMessage: message, isLastDisplayed: false, convId: convId, accountId: accountId, stateSubject: self.stateSubject)
    ...
    func messageTaped(message: MessageViewModel) {
        if message.message.type == .fileTransfer, let player = message.player {
            self.stateSubject.onNext(MessageState.openPlayer(player: player))
        }
    }

    func getTransferProgress(transferId: String) -> Float? {
        guard let conv = self.conversationModel else { return nil }
        return self.dataTransferService.getTransferProgress(withId: transferId, accountId: conv.conversation.value.accountId, conversationId: conv.conversation.value.id, isSwarm: conv.conversation.value.isSwarm())
    }

    func getTransferSize(transferId: String) -> Int64? {
        guard let conv = self.conversationModel else { return nil }
        guard let info = self.dataTransferService.dataTransferInfo(withId: transferId, accountId: conv.conversation.value.accountId, conversationId: conv.conversation.value.id, isSwarm: conv.conversation.value.isSwarm()) else { return nil }
