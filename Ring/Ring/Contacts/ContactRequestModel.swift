...
class ContactRequestModel {
    ...
    fileprivate let fromKey = "from"
    fileprivate let payloadKey = "payload"
    fileprivate let receivedKey = "received"
    ...
    init(withDictionary dictionary: [String : String], accountId: String) {
        ...
        if let ringId = dictionary[fromKey] {
            self.ringId = ringId
        } else {
            self.ringId = ""
        }

        if let vCardString = dictionary[payloadKey] {
            do {
        ...
        if let receivedDateString = dictionary[receivedKey] {
